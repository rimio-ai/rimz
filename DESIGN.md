...
The problem RimZ answers is attention. A fleet emits far more than one person can read (prompts, tool calls, completions, failures, token burn, rate-limit stalls). RimZ turns that wall of activity into "this pane, right now, needs you," and otherwise stays quiet. A good harness spends your attention only where a run needs a human, and a good loop removes the need for it between runs; every choice below serves that economy.
...
- Two symbols carry every call for attention: `?` needs your answer, `!` needs a look. They differ by shape, so the signal survives `NO_COLOR` and color-blind eyes; color only reinforces. The fixed cockpit line (`? 2  ! 1 …`) compresses the fleet: a row of zeros means nothing needs you, skip the scan.
...
- Presence is live and facts are durable. A row exists because a pane runs right now and clears itself when the agent exits; everything the agent did stays in the store. Stats enrich display; the store and explicit events decide state and correctness.
...
- **Shared language servers are on-demand enrichment.** Off until configured, one server serves each checkout/server key read-only, bounded by agent leases, memory pressure, and idleness. Required policy gates launch admission, not agent survival after a server stops ([language servers](./docs/internals/lsp.md)).
...
- **One account per provider per room.** A room launches each provider's agents under one account, a provider home chosen at birth and changed only by `rimz reset`, and a session never resumes under another account. A conversation's credentials, settings, and transcript live in the home it was born in, so crossing accounts would silently continue it under someone else's login.
...
- **Pane I/O is explicit, and it enriches rather than decides.** `pane capture` and `pane send` are public primitives for humans and scripts, and `message` routes human-authored text through the same send path, delivering only at open turn boundaries and held while the agent waits on your answer. Pane contents and transcripts decorate rows; the store, hooks, and explicit events decide permissions, state, and correctness.
