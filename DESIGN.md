...
- **A small state vocabulary the agent owns.** Each agent rolls up to one of five states — `running`, `waiting`, `idle`, `success`, `failed`. Rimz observes that state; the one state it derives is `paused`, a display park for an agent that stopped mid-turn on a provider limit, lifted by provider recovery, window reset, or the next hook event ([docs/internals/agent.md](./docs/internals/agent.md)).
