...
- **A reborn room offers recovery.** When a session must be reborn — reboot, multiplexer crash, or a stuck room — Rimz offers to re-seed the prior agents from the durable rollup into `#channel` tabs grouped by their channel label, each restored idle (`claude --resume`, `codex resume`, `pi --session`). Empty named channels reopen as tabs from the channel registry. The prompt defaults to recovery and non-interactive starts recover. Agents deliberately closed while the room remains live record an ended trace and stay out of recovery. Continuity is Rimz-owned and transcript-based: the clean rebirth is guaranteed, the conversation transcript including asks and answers is durable, and the provider resume rides on top as best-effort enrichment.
