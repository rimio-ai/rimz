...
- **A reborn room offers recovery after reboot.** When the machine rebooted since the room was last alive — detected by boot id, or by boot time where the boot id is unreadable — Rimz offers to re-seed the prior agents from the durable rollup into `#channel` tabs grouped by their channel label, each restored idle (`claude --resume`, `codex resume`, `pi --session`). A host with no readable boot signal starts bare. Empty named channels reopen as tabs from the channel registry. The prompt defaults to recovery and non-interactive starts recover. Agents deliberately closed while the machine stays up — a tab/pane or the whole room — stay out of recovery, and a multiplexer crash without a reboot starts bare. Continuity is Rimz-owned and transcript-based: the clean rebirth after reboot is guaranteed, the conversation transcript including asks and answers is durable, and the provider resume rides on top as best-effort enrichment.
