...
- The column arrives triaged. Attention rises by unread inbox first, then read attention oldest-first, then fresh calm work, then process rows, then calm rows past the one-hour attention ceiling. A row becomes unread when it enters `waiting`, `failed`, `paused`, or `success` — including rows already in that state when you attach — and stays unread until you focus it or `rimz sidebar mark-read` clears it. A per-worktree cap trims only the idle/process tail.
