...
- A context meter, token totals, live dollar cost, diff stats, todo progress, and a last-activity age ride the rows: which agent is burning toward a rate limit, which is one approval from done. The provider dashboard carries the pace — spend for today, the week, and the month per provider, with 5h/7d budget bars draining in real time. These enrich display; the ledger and explicit events decide state and correctness.
...
- **A reborn room offers recovery.** When a session must be reborn — reboot, multiplexer crash, or a stuck room — Rimz offers to re-seed the prior agents from the durable rollup into `#channel` tabs grouped by worktree, each restored idle (`claude --resume`, `codex resume`, `pi --session`). The prompt defaults to recovery and non-interactive starts recover. Agents deliberately closed while the room remains live record an ended trace and stay out of recovery. Continuity is Rimz-owned and transcript-based: the clean rebirth is guaranteed, and the provider resume rides on top as best-effort enrichment.
