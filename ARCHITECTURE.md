...
- `src/agents/spending.rs` — today/week/month/all-time spend and token (`input + output`) aggregation over agent transcript history (the `SpendTally` type, cache types, cross-file Claude dedup); dispatches to the per-provider parsers in `transcript/`.
