...
`build.rs` embeds the ignored generated pricing snapshot (`pricing/litellm-pricing.json`) at compile time as compressed bytes, network-free; `cargo xtask pricing-refresh` regenerates it, and `cargo xtask dist` runs that refresh before release builds ([provider.md → Token pricing](./docs/internals/agents/provider.md#token-pricing)). Hidden subcommands are machinery, not humans — the `sidebar` and `statusline` helper APIs, the `claude refresh-usage` helper, and the `codex` helpers (`refresh-context`, `refresh-rate-limits`, and the `app-server serve` broker hosted in the `rimzd` daemon view) — listed in [cli.md](./docs/reference/cli.md#commands-rimz-calls-for-you).
