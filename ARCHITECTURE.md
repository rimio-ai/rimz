...
Top-level domain modules keep their `//!` headers as the detail entry point: `workspace` (project identity), `worktree` (Rimz-owned Git worktrees and cleanup; [worktrees.md](./docs/internals/agents/worktrees.md)), `tab_layout` (agent-tab layout DSL and IR; [worktrees.md](./docs/internals/agents/worktrees.md)), `trust` (executable-surface hash and grant state; [trust.md](./docs/internals/sidebar/trust.md)), `feed` (item lifecycle, surfaces, statuses), `target` (agent target parsing and snapshot resolution), `message` (deferred per-agent text queue; [messages.md](./docs/internals/agents/messages.md)), `run` (supervised one-shot run records and lifecycle completion; [run.md](./docs/internals/agents/run.md)), `bridge` (per-request and per-run sockets, nonce validation), `ids` (typed identifier newtypes), `resume` (resume-on-rebirth planner), `remote_control` (agent remote-control launch), `osc` (terminal notification OSC/BEL bytes), `config` (per-machine settings, with config-family leaves under `src/config/`, including `animation.rs` for sidebar status-head overrides), `agent_activity` (liveness hints), `proc` (`/proc` reader), `reload` (binary-upgrade convergence and re-exec target resolution).
