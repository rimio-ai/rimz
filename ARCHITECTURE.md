...
- **Project identity** — `workspace` (root/worktree resolution and env pinning), `channel` (durable named lanes; [message.md § Channels](./docs/internals/agents/message.md#channels)), `worktree` (Rimz-owned Git worktrees and seeding; [worktree.md](./docs/internals/agents/worktree.md)), `storage` (symlink-safe Rimz-owned disk measurement), `forge` (pure PR-number/URL/remote-host parsing).
- **Agent orchestration** — `agents_spec` (the inline layout DSL/IR and team/profile resolution; [harness.md](./docs/internals/agents/harness.md#the-layout-ir)), `target` (the `@handle#channel` address grammar and its inverse renderer; [harness.md](./docs/internals/agents/harness.md#the-address)), `launch` (default-shell, startup-argv, and PATH resolution), `message` (durable per-agent message queue and delivery pipeline; [message.md](./docs/internals/agents/message.md)), `run` (supervised runs; [harness.md](./docs/internals/agents/harness.md#supervised-runs)), `schedule` (pure loop schedule parsing/due evaluation; [harness.md](./docs/internals/agents/harness.md#scheduled-turns-loop)), `loop_instances` (Rimz-owned one-shot/self-wake/poll-until state; [harness.md](./docs/internals/agents/harness.md#scheduled-turns-loop)), `resume`/`remote_control` (resume-on-rebirth, agent remote-control launch), `petname` (stable human-friendly handles).
