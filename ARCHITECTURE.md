...
An agent launched in the current pane has a signal-aware placement parent for the command's lifetime. It owns only the temporary mux tab title and restores the original name and naming policy before returning to the user's shell ([fleet.md](./docs/internals/harness/fleet.md#placement)).
