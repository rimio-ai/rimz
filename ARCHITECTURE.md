...
| Subtree | Owns | Detail |
| --- | --- | --- |
| `src/cli/` | command parsing and per-subcommand handlers; one `run(...)` per subcommand, with oversized command internals split under matching leaves such as `cli/run/` and `cli/remote/` | [cli.md](./docs/reference/cli.md) |
...
| `src/agents/` | the agent integration layer: adapter trait, registry, per-provider adapters, spend/pricing/account | [contract](./crates/rimz/src/agents/AGENTS.md) · [hooks.md](./docs/internals/agents/hooks.md) |
...
Top-level domain modules keep their `//!` headers as the detail entry point: `workspace` (project identity), `worktree` (Rimz-owned Git worktrees and cleanup; [worktrees.md](./docs/internals/agents/worktrees.md)), `tab_layout` (agent-tab layout DSL and IR; [worktrees.md](./docs/internals/agents/worktrees.md)), `launch` (default-shell resolution, shell-startup argv construction, and final PATH probing), `trust` (executable-surface hash and grant state; [trust.md](./docs/internals/sidebar/trust.md)), `feed` (item lifecycle, surfaces, statuses), `target` (agent target parsing and snapshot resolution), `message` (deferred per-agent text queue; [messages.md](./docs/internals/agents/messages.md)), `run` (supervised one-shot run records and lifecycle completion; [run.md](./docs/internals/agents/run.md)), `bridge` (per-request and per-run sockets, nonce validation), `sock` (shared AF_UNIX socket budget and remedy), `ids` (typed identifier newtypes), `resume` (resume-on-rebirth planner), `remote_control` (agent remote-control launch), `osc` (terminal notification OSC/BEL bytes), `config` (per-machine settings, with config-family leaves under `src/config/`, including `color.rs` for typed theme colors and `animation.rs` for sidebar status-head overrides), `agent_activity` (liveness hints), `proc` (`/proc` reader), `reload` (binary-upgrade convergence and re-exec target resolution).

`build.rs` embeds the checked-in pricing snapshot (`pricing/litellm-pricing.json`) at compile time, network-free; `cargo xtask pricing-refresh` refreshes it ([pricing.md](./docs/internals/agents/pricing.md)). Hidden subcommands are machinery, not humans — the `sidebar` and `statusline` helper APIs, the `claude refresh-usage` helper, and the `codex` helpers (`refresh-context`, `refresh-rate-limits`, and the `app-server serve` broker hosted in the `rimzd` daemon view) — listed in [cli.md](./docs/reference/cli.md#commands-rimz-calls-for-you).
