...
- CI runs three jobs: `checks`, `externals`, and `tests`. The `tests` job runs the `gate`, `live-tmux`, `live-zellij`, and `journey` profile slices as sequential `cargo xtask test --profile <tier>` steps in one container; the `live-*` and `journey` steps run under `if: ${{ !cancelled() }}` so one failing tier still reports the others, and running the tiers sequentially keeps the latency-sensitive live backends from co-scheduling.
