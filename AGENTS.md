...
- `agents/` — the `AgentAdapter` trait, `state.rs` rollup, per-kind adapters (Claude, Codex, Pi, OpenCode), the shared `plugin/` process adapter, spend/pricing/account.
...
- [agent-support.md](./docs/reference/agent-support.md) — per-agent status, integration surface, and permission-mode mapping for Claude, Codex, Pi, OpenCode; [agent-plugins.md](./docs/reference/agent-plugins.md) — external bundle, canonical wire, and probe contracts.
