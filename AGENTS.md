...
- CI compiles nextest binaries once with `cargo xtask test-archive`, then a matrix of test jobs, one per tier (`gate`, `live-tmux`, `live-zellij`, `journey`), checks out the repository and runs each profile's slice of the archive; the checkout keeps fixture paths baked by `env!("CARGO_MANIFEST_DIR")` valid.
