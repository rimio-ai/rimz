...
- CI compiles nextest binaries once with `cargo xtask test-archive`, then two test jobs download the archive and run profile slices — a `gate` job and a `live` job that runs the `live-tmux`, `live-zellij`, and `journey` tiers as sequential steps; both check out the repository so fixture paths baked by `env!("CARGO_MANIFEST_DIR")` stay valid.
