...
- Run the pre-PR / pre-hand-off stack with `cargo xtask gate`: it auto-formats, then runs invariants, docs-links, lint, doctests, and the fast nextest subset that excludes live-backend and journey tiers; `cargo xtask test -P live` is the exact excluded complement.
...
- CI runs three jobs: `checks`, `externals`, and `tests`. The `tests` job compiles the nextest archive with `cargo xtask test-archive`, then runs the `gate`, `live-tmux`, `live-zellij`, and `journey` profile slices from it as sequential steps in one container; the `live-*` and `journey` steps run under `if: ${{ !cancelled() }}` so one failing tier still reports the others, and running the tiers sequentially keeps the latency-sensitive live backends from co-scheduling.
