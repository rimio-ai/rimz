...
- CI runs three jobs: `checks`, `externals`, and `tests`. The `tests` job runs the `gate`, `live`, and `journey` profile slices as `cargo xtask test --profile <tier>` steps in one container; `live` runs both mux backends in one nextest process so they co-schedule while per-backend groups bound concurrency, and the `live` and `journey` steps run under `if: ${{ !cancelled() }}` so one failing tier still reports the others.
