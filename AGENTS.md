...
- **`agents/`** — integrate, operate, and house coding agents.
  ...
  - [autoping.md](./docs/internals/agents/autoping.md) — scheduled window-priming pings: the lowest-effort `ping`→`pong` supervised turn that starts a provider's sliding budget window on your schedule, the OS-scheduler installer (systemd user timer or crontab, consent-previewed and idempotent), the per-machine `[autoping]` config, and the `--root`-carried workspace.
