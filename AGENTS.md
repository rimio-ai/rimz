...
- Run the pre-PR / pre-hand-off stack with `cargo xtask gate`: it auto-formats, then runs invariants, docs-links, lint, doctests, and the fast nextest subset that excludes live-backend and journey tiers.
...
- Escalate to journey, live-backend, performance, dependency gates, or full CI (`cargo xtask ci`) when the change touches those surfaces, their fixtures, or shared infrastructure.
