...
**`CommandChanged` is the load-bearing event for RimZ**: it pushes the foreground-command handoff with the full argv, the foreground bit, and the focused clients — exactly the live process state `list-panes -j` does not report ([caveats](../../internals/multiplexers.md#zellij-backend-caveats)). `CwdChanged` and `get_pane_pid` exist at RimZ's Zellij 0.44.0 floor; `CommandChanged` first appears after 0.44.0, so the existing 0.44.0 foreground gap remains until the user upgrades Zellij. A cached permission grant produces **no** `PermissionRequestResult`; application state flowing is the proof of grant (verified live, 0.44.3).
...
`is_focused` is an upstream projected mark, not a uniqueness guarantee. In Zellij 0.44.3 a reconnect-churned SSH room with one listed client has been observed reporting multiple durable `is_focused: true` terminal panes in one tab. RimZ accepts the field only at the Zellij API boundary and does not project, retain, diagnose, bind, list, or repair from it. The full `(client_id, PaneId)` map returned by `list_clients()` is the attached-focus authority; plugin pane ids remain in that map for ambiguity and client counting, while the host-facing `viewed_panes` shape contains terminal ids only. `is_held` means the command pane sits at the `Press ENTER to run` banner — the resurrection fingerprint RimZ's pre-attach gate keys on. There is **no pid, cwd, or live-command field** — those are the request/response host commands above, or `CommandChanged`/`CwdChanged` pushes.

`ListClients` replies carry no request token. RimZ therefore permits one request in flight and retains one newest queued purpose: general sampling, immediate switch observation, or settled switch confirmation. The in-flight slot expires on the keepalive cadence; a reply after expiry is folded as a general sample and the replacement purpose is re-issued, so a dropped or late reply cannot resolve a superseded switch or stall sampling indefinitely.

Zellij exposes no causally confirmed actuator for the sampled human client. The control-plane `action focus-pane-id` can move the visible pane and route subsequent terminal input while `ListClients` continues to report the exact pre-action pane. Plugin `focus_terminal_pane` routes with the invoking plugin instance's `env.client_id`; a headless session-lifetime instance therefore targets its transient/plugin client rather than the human client whose sample authorized repair. RimZ keeps focus-repair actuation in the visible renderer through the CLI and records command acceptance as an applied action intent rather than observed human focus. The presence plugin's `ChangeApplicationState` grant is used only for a mechanical fullscreen toggle on a host-selected pane id, which is independent of plugin client identity. Native client observations remain authoritative and resolve focus intent through confirmation, supersession, or an unchanged-observation fence.
...
| Panes | `new-pane [--direction right\|down] [--floating] [--in-place] [--cwd] [--name] [--close-on-exit] [--start-suspended] [--stacked] [--tab-id] [--near-current-pane] [--no-focus] [--borderless true\|false] [--plugin url] [--blocking \| --block-until-exit[-success\|-failure]] [-- cmd…]` · `close-pane [--pane-id]` · `rename-pane [--pane-id]` / `undo-rename-pane [--pane-id]` · `move-pane [dir]` / `move-pane-backwards` · `resize [dir\|+\|-]` · `clear` · `toggle-fullscreen` · `stack-panes -- id…` |
...
**Outer-terminal title** — the server emits OSC 0 for the focused pane as `<session> | <pane title>` (omitting the separator when the title is empty). A non-empty explicit pane name wins over the terminal grid title learned from application OSC 0/1/2; only an unnamed pane uses that application-controlled value. RimZ therefore names every pane it creates. See upstream [`make_terminal_title`](https://github.com/zellij-org/zellij/blob/v0.45.0/zellij-utils/src/shared.rs#L70-L82), [`TerminalPane::render_terminal_title`](https://github.com/zellij-org/zellij/blob/v0.45.0/zellij-server/src/panes/terminal_pane.rs#L650-L659), and the [client render emission](https://github.com/zellij-org/zellij/blob/v0.45.0/zellij-server/src/ui/pane_contents_and_ui.rs#L295-L313).
