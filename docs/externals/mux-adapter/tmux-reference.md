...
- **`read-only` restricts key input only — stdin commands still execute.** The man's "only keys bound to detach-client or switch-client have any effect" governs key bindings; a `-r` control client can still mutate the server through commands. The presence watch attaches writable with `ignore-size` so tmux 3.7 `send-keys` can resolve a writable client on headless sessions; its safety property is the command allowlist, which writes only `refresh-client -B`.
