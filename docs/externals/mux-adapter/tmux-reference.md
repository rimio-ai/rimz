...
Since 3.7, a floating pane's cell is a child of the root cell (`layout.c`, `layout_floating_pane`), so the tiled tree lists it as one more leaf of the root group, with its own offsets. `layout_dump` then appends every floating pane again after the tree inside angle brackets, one leaf cell each, front to back: `<WxH,X,Y,<pane>,WxH,X,Y,<pane>>`. Only the suffix marks a leaf as floating; the root group gives no marker (probed on 3.7c: `29ff,200x50,0,0{100x50,0,0,0,99x50,101,0,1,40x10,4,2,2}<40x10,4,2,2>` for floating `%2`). A zoomed window writes no suffix, because zoom clears every pane's layout cell while `window_layout` still dumps the saved root with the floating leaves inside it; read `pane_floating_flag` there. The checksum covers the whole string. tmux's own parser does not read the suffix: `layout_parse` builds the tiled tree and fails `invalid layout` on anything after it, so `select-layout` rejects the string `window_layout` prints for a window with a floating pane, and a client parser that stops at the tiled tree fails the same way. `CHANGES FROM 3.6b TO 3.7` lists restoring custom layouts with floating panes as not yet available. 3.8-rc replaces this format with a JSON subset for clients that opt in ([unreleased](#unreleased-in-38-rc)).
