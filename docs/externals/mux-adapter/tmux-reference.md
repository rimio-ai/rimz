...
| `terminal-features[240]` | server | **`*:sync`** | tells tmux the outer terminal honours synchronized output, so bracketed frame writes forward as atomic redraws for pixel pets and full-screen TUIs; the fixed index makes repeated writes idempotent, and RimZ purges exact `*:sync`/`*:extkeys` duplicates at other indices while preserving unrelated user entries |
