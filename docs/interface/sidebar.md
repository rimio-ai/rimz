...
| token | reads as |
|-------|----------|
| `▣ ━━━━╺━──── 38.2%` | context meter — how full the window is; the bar fills as used, `▢` hollow at 0%. Small windows draw linearly, and the fill curve grows with the window. The fill also shows *where* the window went: a wide cache-read run in the meter's health tone, with gap-fronted `╺` runs for cache-write and fresh-input accents; the half-cell floor keeps every nonzero component visible when the fill has room |
