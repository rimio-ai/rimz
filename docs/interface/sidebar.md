...
The team badge leads, followed by ` · ` and the track. The dots follow declared stage order: passed, current, then future, with the current stage's name immediately after its dot. Passed dots are green and future dots are muted. Flipping backward moves the current dot back. At `Done`, every dot is passed and the last becomes the green done seal, followed by `Done` to keep the meaning visible without color. A board stage outside the declared pipeline shows its name without a track. The current dot and stage name take the visible stage owner's status color and animation phase, or stay muted when that owner has no visible card. The line never creates `!`, unread state, attention ranking, tab status, or a notification.

The muted, glyph-free clock is pinned right: stage elapsed / run total, such as `18m / 1h`. Each duration uses its highest whole unit (`45s`, `18m`, `1h`, `2d`), without zero-padding. If only one duration is known it appears alone; if neither is known the clock is absent. At `Done`, only the total appears. The stage clock starts at the latest parseable entry into that stage in the board's `## Progress` (or `## Progress log`) ledger, including an `opened` entry. A backward flip restarts the stage it enters; a same-stage re-flip does not. The total starts at the first parseable ledger entry (or the latest flip out of `Done`) and runs until now, or until the last `-> Done` entry at `Done`; a `Done -> Done` re-flip does not move the stop. A missing or future start omits that duration, and `Done` without a valid stop omits the clock. Clicking the line focuses the visible stage owner's pane, otherwise the first actionable team member, otherwise the group's first visible row.
