...
| `▰▱` / `▱▱` / `ex` / `api` | provider account bar: included-window budget (`5h`/`7d`/`30d`, fill = left), unknown budget as a dim empty track, paid extra usage as `ex`, API-key spend as `api`; `∞` means uncapped, not reported, or a limit is currently lifted |
| `↻ 2h06m`         | when a provider budget resets; soft at a sustainable burn pace, heating toward red as spend outruns the window or cooling toward green when spend runs well under it once enough of the window has elapsed — the duration form of `↻`, next to a budget bar (the count forms are compactions on a card and reset credits in the Codex header) |
