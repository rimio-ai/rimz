...
| `╱` | partition tick inside a provider bar: fill to its left is still available to the capped model; from the tick to the fill edge is available only to other models; shown only when the model's cap is tighter than the parent budget |
...
A model sub-cap folds onto its parent window's bar as a **partition tick**, not another row: for Claude's Fable weekly cap, the fill left of `╱` is what Fable may still spend, while the fill from the tick onward belongs to other models. The tick replaces one cell inside the fill, never in the empty track, and appears only when the model's remaining allowance is strictly less than the parent's. Its tone follows the model's own remaining percentage; a spent model cap places a red tick at cell 0 while the row keeps the parent's fill, label tone, and reset countdown. It has no label or countdown of its own, does not exhaust other windows or trigger paid-extra substitution, and follows the parent row when `ex` moves it. An unknown model reading or an exhausted parent row draws no tick. For example, 37% weekly usage and 58% Fable usage of a 50% share leave 63% overall and 21% for Fable.
