...
| `#91 ✓` / `#91 ✕` / `#91 ◌` | an open pull request's CI verdict beside its linked number: passing in green, failing in red, or still running in amber |
...
A linked pull request shows its `#N` beside the worktree name in the cool link tone. An open PR adds `✓`, `✕`, or `◌` beside the number for passing, failing, or pending CI; identity and CI stay beside the name while the PR state stays on the right marker.
