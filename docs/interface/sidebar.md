...
The budgets are account-scoped — every session of a provider shares one account's budget — so they pin to a panel at the bottom rather than riding the rows. How accounts stack or tab, and in what order, is display config (`provider_tabs`, `provider_list`, `max_provider_blocks`; [theme.md → Display](../guide/theme.md#display)).
