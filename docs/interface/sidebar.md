...
**The cap.** Each active worktree shows a capped number of rows with a dim `+K more`. Click `+K more` to expand that group in place; the expanded group shows every row and a dim `− less` line that collapses it. The ordinary cap trims only the idle/process tail; active, blocked, paused, finished, unread, and focused rows stay visible. A finished group collapses to its header and one dim roster receipt, hiding unread success too: each hidden member keeps its status glyph, and its name wears the provider's softened brand tone like a calm card; overflow and process rows fold into `+n`, and tracked cost pins right when it rounds to a cent. Narrow or process-only receipts fall back to `+K done`; the toggle or `s` status filter reveals the full roster:
