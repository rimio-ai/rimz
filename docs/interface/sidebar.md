...
| `$1.27` / `≈$0.42` | spend — dollar green, two decimals; `≈` marks a locally priced card-only estimate excluded from aggregate spend and budget enforcement; omitted while a session's cost rounds to zero |
...
- **Headline spend.** The room's workspace-scoped spend for the configured headline window, pinned to the right of the live-agents line, **counting up** in an eased odometer roll the moment any in-scope agent's cost moves — every jump lands inside 1.2s of 200ms clicks, big first steps easing into a penny-sized landing on the exact figure, with a brief brighten as it settles. Locally priced `≈$` card-only estimates stay out of this aggregate and budget enforcement. The value is always present, starting at `$0.00`; within one headline-window epoch it ratchets upward and resets when the configured spend window rolls.
