...
- **The context meter (`▣`/`▢`).** The resting card's one bar: `▢` hollow at 0%, `▣` once anything fills it, the value always the percent *used*. Supported kitty-graphics paths move the fill edge at pixel precision; every other path rounds to the nearest half cell. The fill also shows *where* the window went — a dominant cache-read run in the current health tone, with gap-fronted `╺` runs for cache-write (`◍`) and fresh-input (`↘`) accents. Their half-cell floor keeps every nonzero component visible when the fill has room; a row with no per-call split yet paints one flat run. The health bands and pixel tier are tunable ([theme.md → Display](../guide/theme.md#display)).
