...
| token | reads as |
|-------|----------|
| `▣ ━━━━◦━──── 38.2%` | context meter — how full the window is, bar fills as used; an empty 0% window reads the hollow `▢`. Glyph, bar, and the `▤` head below share one health ramp — a healthy green at rest, then a continuous OKLab sweep through gold and orange to alarm red, bands configurable via `[sidebar.context]`. The dominant cache-read run carries the sweep; `◦`-separated accents trail it for cache-write and fresh input |
...
| `◇` `↘` `↗` `◌` / `◍` | token markers: fleet lines read total · input (including cache creation) · output · cache-read; the card context line adds `◍` for per-call cache-write. Marker tones stay stable: `◇` total and `◍` cache-write share soft-violet but never share a line (`◇` is fleet vocabulary, `◍` belongs to the card context line), with `◌` cyan · `↘` blue · `↗` green — so the card's context line legends the bar and the cockpit/dashboard/ledger lines speak the same vocabulary; the figures beside them read at the soft middle tier on the fleet lines and subagent rows, and a step deeper at the dim chrome on the card's context line, so the colored markers carry it |
...
- **Summary — who's here and what today burned.** Two lines, each a colored glyph and soft-tier count on the left with today's numbers pinned right. Line 1 is the day at a glance: `◎` (teal) the sessions (threads) that have run today in this room, with the room's accumulated token breakdown — `◇` total · `↘` input, including cache creation · `↗` output · `◌` cache-read, each marker in its one color — pinned to the right edge in the coarse integer form (it drops when today recorded no tokens, leaving `◎ N` alone). Line 2: `¤` (the agents' working clay) the live agents in the room right now, followed by a breathing unread count like `(2)` when unseen rows exist, with the room's spend pinned right. The counts read from the live room and the workspace-scoped JSONL tally's today window. An empty room reads `◎ 0` over `¤ 0`.
...
- **The context meter (`▣`/`▢`).** The resting card's one bar — `▢` hollow at an empty 0% window, `▣` once anything fills it. The bar fills as the window is *used*; its value is always the percent used. Glyph, bar, and the `▤` head wear one health amount — a healthy green at rest, then a continuous OKLab sweep through gold and orange to alarm red, bands tunable via `[sidebar.context]` ([configuration.md](../reference/configuration.md#sidebar-bands)). The bar shows *where* the window went at every fill level: the dominant cache-read run carries the gradient sweep up to the current severity, then `◦`-separated flat accents trail it for cache writes (`◍`) and fresh input (`↘`). A row with no per-call split yet sweeps one continuous gradient.
