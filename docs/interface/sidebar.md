...
Below 46 columns the delegation line uses the legend shorthand `⧉ N · ⧖ M`, keeping both counts and the cost visible. Either half keeps this shorthand when shown alone. At 46 columns and wider the full `subagents (N)` and `waits (M)` labels return. Selection never changes which form the current width uses. Clicking the delegation line toggles its entries without focusing the agent's pane, even on an unselected card. When current child entries are visible, prior-turn finished children fold behind a clickable `  +K more` tail; opening it reveals that history and drops the tail. With no current child entry, no tail shows and the delegation line alone toggles the history. Clicking the delegation line again restores the selection/density view, so a selected card still shows current entries. The toggle expires at the parent's next user-authored prompt; messages from other agents and RimZ's automatic deliveries do not expire it.
