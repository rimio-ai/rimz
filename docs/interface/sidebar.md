...
| token | reads as |
|-------|----------|
| `▣ ━━━━──── 38.2%` | context meter — how full the window is, bar fills as used; an empty 0% window reads the hollow `▢`. Glyph, bar, and the `▤` head below share one severity ramp — calm blue, then a continuous OKLab warn → caution → alarm heat scale, bands configurable via `[sidebar.context]` |
...
- **The context meter (`▣`/`▢`).** The resting card's one bar — `▢` hollow at an empty 0% window, `▣` once anything fills it. The bar fills as the window is *used*; its value is always the percent used. Glyph and bar wear one severity — calm blue, then a continuous OKLab warn → caution → alarm heat scale, bands tunable via `[sidebar.context]` ([configuration.md](../reference/configuration.md#sidebar-bands)). While the meter rests calm the fill splits into colored segments showing *where* the tokens went (`◌` cache reads · `◍` cache writes · `↘` fresh input); once it warms the bar goes one solid severity run.
