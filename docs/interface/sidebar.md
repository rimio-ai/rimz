...
| `◔ 5m`            | last-activity age — shown once it crosses five minutes, so a card stays quiet through normal churn; the face fills by the quarter hour (`◔`≤15m · `◑`≤30m · `◕`≤45m · `●`≤60m · `◉` past) and heats to red by the hour, where resuming likely re-reads the whole context uncached. A successful finished card holds this clock at the muted resting tone. On a running subagent line the same face reads the child's elapsed work as a fixed `m`/`h` label (`<1m` under a minute) |
...
| `✓ #91` / `✕ #91` / `◌ #91` | an open or merged pull request's CI verdict leading its linked number: passing in green, failing in red, or still running in amber |
...
While a [make-up bucket](#zone-1--the-cockpit), the unread lens, or the open-PR lens is picked, the body shows every matching card: non-matching rows, process rows, worktree groups left empty, and the `+K more` line all step aside until the pick clears, and the unfiltered cap does not apply.
...
A linked pull request shows its `#N` beside the worktree name in the cool link tone. An open or merged PR leads the number with `✓`, `✕`, or `◌` for passing, failing, or pending CI; identity and CI stay beside the name while the PR state stays on the right marker.
