...
- **Headline spend.** The room's workspace-scoped spend for the configured headline window, pinned to the right of the live-agents line, **counting up** in an eased odometer roll the moment any in-scope agent's cost moves — every jump lands inside 1.2s of 200ms clicks, big first steps easing into a penny-sized landing on the exact figure, with a brief brighten as it settles. The value is always present, starting at `$0.00`; within one headline-window epoch it ratchets upward and resets when the configured spend window rolls.
...
- **Line 1 — identity.** The animated leading cell, then the agent's handle — its team role, explicit name, or profile, else the kind — in the provider brand color (so a team reads `planner` / `coder` / `reviewer`; unknown kinds mid-gray), then capability: model · one reasoning-configuration token (effort or `thinking`) · window token, dim metadata under the brand-colored name. Capability degrades by width — wide carries model · reasoning configuration · window, medium drops reasoning configuration, narrow keeps just the name; the window token rides non-idle cards only. The session `$cost` pins right and joins the line once the session has actually spent (an idle agent at `$0.00` shows nothing), counting up in the same eased odometer roll as the cockpit headline; a locally priced estimate carries the `≈$` prefix and does not join the headline aggregate.
