...
```
○ claude · Opus 4.8 · xhigh                              ← line 1 — glyph · name · model · effort
...
- **Line 1 — identity.** The animated leading cell, then the agent's handle — its team role, explicit name, or profile, else the kind — in the provider brand color (so a team reads `planner` / `coder` / `reviewer`; unknown kinds mid-gray), then capability: model · effort · window token, dim metadata under the brand-colored name. Capability degrades by width — wide carries model · effort · window, medium drops effort, narrow keeps just the name; the window token rides non-idle cards only. The session `$cost` pins right and joins the line once the session has actually spent (an idle agent at `$0.00` shows nothing), counting up in the same eased odometer roll as the cockpit headline.
