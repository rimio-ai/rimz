...
A working agent gone silent past the stall window (`[agents.attention]`, 30 minutes by default) escalates to `!` — unless its provider window is spent without reset, which pauses (`⏸`) instead. A parent waiting on subagents is exempt: its quiet wave is the children's work. A fresh idle agent collapses to identity plus any RimZ-authored description. Selecting it always opens the empty `▢` context meter below its description, or below an animated ellipsis compose affordance when it has no description; moving selection away restores the collapsed shape.
...
| `#91 ✓` / `#91 ✕` / `#91 ◌` | an open or merged pull request's CI verdict beside its linked number: passing in green, failing in red, or still running in amber |
...
A linked pull request shows its `#N` beside the worktree name in the cool link tone. An open or merged PR adds `✓`, `✕`, or `◌` beside the number for passing, failing, or pending CI; identity and CI stay beside the name while the PR state stays on the right marker.
