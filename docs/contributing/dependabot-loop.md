...
`run` takes the repository-common lock, checks the live agent cards for an occupied repair lane, runs `git worktree prune` and `git fetch --prune origin`, deletes empty batch branches, and re-plans. An empty batch branch is a `deps/repair-*` branch whose tip is already in `origin/<default_base>`, that no worktree holds and no open PR heads. It is deleted on origin with `git push --force-with-lease=refs/heads/<branch>:<tip> origin :refs/heads/<branch>`, or locally with `git branch -D`, and each deletion prints one `pruned_batch_branch` JSON line. A worker that stops without writing therefore leaves nothing behind for the next plan. `plan` never deletes anything. When the plan calls for repair, it resolves the attempt checkout in this order, printing one `attempt_checkout` JSON line (`resumed` or `created`, with `path` and `branch`):
...
Worktrees on other `deps/repair-*` branches are reported as `stale_attempt_checkouts` and left alone: `rimz gc` reclaims them once their work is landed, or a human clears them. List them with `rimz worktree list`; discard a rejected batch's tree with `rimz worktree remove deps/repair-N --force`. To abandon a batch branch that holds work but never got a PR, remove its tree, then run `git push origin --delete deps/repair-N`: while such a branch covers a closed source, the coordinator refuses to plan.
