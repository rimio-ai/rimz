...
2. **Learn what it does.** `inspect --module <target> --brief --out /tmp/<target>.md` is the dossier a subagent brief carries; `--json --out /tmp/<target>.json` then `jq` narrows the full report. The verdict says whether the interface is wide or deep; the record says what the module claims to be; the surface table says what each item can narrow to; call shapes and repeated assembly quote the assembly callers pay, and `also wires` says which other modules the same caller assembles; flags name the parameters one caller bends; pass-throughs are the deletion test's first candidates; pins list the tests a narrowing moves. A shape proves reach, not assembly: brief a subagent with the heaviest caller and the top call shape and ask whether the caller constructs and wires the module's internals or merely names several of its items.
