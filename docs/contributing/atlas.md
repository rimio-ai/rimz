...
If `rust-analyzer` itself panics, Atlas retries the same exact SCIP export once with cache priming serialized. The retry neither substitutes heuristics nor falls back to `--no-index`; if it also fails, Atlas reports the panic cause and both attempts. This recovery exists because the analyzer bundled with the pinned toolchain computes an unused inlay-hint facet while exporting SCIP, and that upstream inference path can panic on a valid workspace.
