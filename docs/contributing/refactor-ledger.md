...
| `harness` | landed pass-5; pass-7; pass-8; pass-14; pass-20a; pass-23b | — | — | policy over agent and store records, reaching down; interiors have their own rows below. Two account-cache writer sites stay until the `agents::account` lift. Pass 23b reviewed the flat root files as one group: the auto-redeem policy constants and verdict, the subagent spec filter, the reminder struct, the two team-prompt accessors, the board-run scan and the two sweep enforcers narrowed; every error and scan type that stayed `pub` is the return of an entry point the binary calls, recorded item by item in `refactor-target.toml`. |
| `harness/schedule` | holds; landed pass-14; pass-23b | `54bb292a5` | 30 | one arming rule, compiled armed receipt, exit codes from `RunStatus`; `RunLockInfo`, `SignalSelector`, `LoopRunRecord::new` public by verdict. Pass 23b re-reviewed the interior after 179 escaping items: six narrowed (`LOOP_TASK_ENV` and the pending-wait scan to the crate, `SignalSelector::family` to the harness, `watch_interval`, `StrikesError` and `CheckOutcome::new` inside their own tree), and the pass-14 `ArmingError` verdict still holds on `pub fn pause`/`disable`. The catalog, arm, team and signal error and outcome types are all signature floors. |
...
| `harness/rebirth` | holds; landed pass-23b | `54bb292a5` | 30 | `RebirthOutcome` deleted: it wrapped a `ResumePlan` beside a death marker no caller read, so `materialize` returns the plan and stamps the marker in place. The three entry points and `record_boundary` are `pub(crate)`; `RebirthPlan` and `RebirthErr` are floored by the held `room::RoomContext::inspect_rebirth`. |
| `harness/auto_continue` | holds; landed pass-23b | `54bb292a5` | 30 | `ResumeArm` and `resume_park` file-private, `arm_budget_park` at harness reach; the `message` admission on `ResumeUnrecovered` holds by intent. The backoff-ramp absorbing stays an open deferral. |
...
| `worktree` | holds; landed pass-8; pass-23b | `54bb292a5` | 30 | the interior reviewed: `content_landed` and `on_trunk_first_parent` are the landed proof at crate reach for the sidebar group and removal. `RequestedName`, `WorktreeMerge` and `RemovalRetirement` stay `pub` because `cli/worktree.rs` names all three, and `PushDestination` is a pub field of the `CreatedWorktree` that command renders. |
...
- `xtask/src/atlas/contract.rs` rejects `kind = "module"` with `max-production-sloc-delta = 0`, but [refactor-program.md](./refactor-program.md) prescribes a ceiling of 0 for a narrowing-only pass. Pass 23b cleared it by landing two vestigial deletions, so its ceiling went negative; the next narrowing-only module pass with nothing to delete has to pick one side. The gate and the doc move together.
