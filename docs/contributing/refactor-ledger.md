...
`store` owns every record it persists and reaches nothing in `harness`; `harness` is policy over store records and reaches down. `store::run` owns the run schema, codec, terminal transition and wake sender; `store::event` owns the persisted signal vocabulary; `store::message` owns submitted-prompt alignment and the shared notice-header spelling. Handle minting and validation live in `agents::petname`. Harness retains run policy and workspace-lock wrappers, the run waiter, signal selectors and firing, and the message-header composer. Stored records, wire frames, grammar and observable behavior stay unchanged; only the impossible run-wake serialization error path is deleted.
...
| `store` → `diag` | — | keep | reset shares the diagnostic frames-directory layout through `frames_dir_under`; a diag-below-store re-layer is a later seam if the survey ranks it |
| `store` → `diag::record` | — | keep | diagnostic record vocabulary depends only on `ids` and `remote::link::LinkTier`; a diag-below-store re-layer is a later seam if the survey ranks it |
| `store` → `remote::link` | — | close: later seam | `LinkTier` is a value type; `remote::link` depends on `mux::CommandSpec`, so the type moves down, not the module |
| `store` → `disk_usage` | — | close: later seam | `dir_size` is a disk fact; `disk_usage.rs` depends only on `disk::paths` and belongs beside `disk` |
...
| 2026-09-06 | pass-7 seam #7 `store` → `harness` | `crates/rimz`; `docs`; `refactor-target.toml`; `AGENTS.md`; `CLAUDE.md`; `ARCHITECTURE.md` | `97a947974` | rehome ×15 (+ the terminal transition onto `RunRecord`), delete ×1, deepen ×1 | −22 | +2 | +8 internal | Measured with `cargo xtask atlas diff --expect /tmp/pass-7-contract.toml`: test SLOC +153, Rust files +0; all 15 rehomes, the delete and six zero-site dependency rows landed. Store → harness 17→0; source ceiling 0 retained (D1 spike +15, final −22). Escaping surface +2 pays for four shared items minus two deleted wrappers; internal sites +8 include the split imports. Carried `d904fa4bd` first. Deferred: remote link vocabulary and disk usage as later seams, and diag re-layering if the survey ranks it. Kept `harness::run::{create, load, list}` wrappers and public `WakeupFrame` for binary callers. Tightened harness 536, run 31, run_wake 9; store 336 and agents 988. |
