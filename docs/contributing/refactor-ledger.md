...
| 7 | `store` → `harness` upward dependencies (signal vocabulary, petname, prompt alignment, run schema, run wake sender): close the direction | pass-6 admission intent `store → harness::schedule::signal` close: later seam; survey at `97a947974`: 17 sites into harness, cycle harness ↔ store 47/17 cross-layer | in flight store-below-harness |
...
| `store` → `harness::schedule::signal` | 5 | close: later seam | added by `a8e4c4afa` after pass 4 (`store/event.rs:9`, `store/follow.rs:10`, `store/writer/signal.rs:3`); pass 6 defers a split moving `SignalName`/`SignalNameErr`/`SignalSource` below store and making `append_signal` take `SignalEventPayload` |
