...
| 8 | `store` → `remote::link` and `store` → `disk_usage` remaining non-diag upward dependencies | survey at `6d13498b3`: store upward sites 6 = diag 4, disk_usage 1, remote::link 1; worktree → disk_usage 1 | landed pass-8 |
...
| `store` → `diag::record` | — | keep | diagnostic record vocabulary depends only on `ids` and `store::snapshot::LinkTier`; a diag-below-store re-layer is a later seam if the survey ranks it |
...
| 2026-09-06 | pass-8 seam #8 remaining non-diag `store` upward edges | `crates/rimz/src/{store,remote,diag,sidebar,sidebar_pane,disk,cli}`; `disk_usage.rs`; `worktree.rs`; `lib.rs`; `docs`; `refactor-target.toml`; `AGENTS.md`; `CLAUDE.md` | `6d13498b3` | rehome ×6, delete ×1 (manual Ord/PartialOrd/tier_rank), re-layer ×1 | −17 | +0 | +0 internal | Measured with `cargo xtask atlas diff --expect /tmp/pass-8-contract.toml`: test SLOC +38, Rust files +0; six rehomes landed and three dependency rows 1→0. Three pins green before structure changes; disk body and four tests moved unchanged. Deferred: FileIdentity/FileStamp/temp_sweep identity unification (three different rules), remote module interior, and store → diag re-layering if the survey ranks it. Tightened remote 177, lib 69; destination surfaces store 337 and disk 122. Global tighten also pruned the already-unused runner → proc allowance. |
