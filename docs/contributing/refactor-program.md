...
**Plan.** One self-standing ordered plan for a coder who has only the plan: title, the target in full, the contract (behaviour-preserving, net-subtractive, bugs found are reported rather than fixed), the line budget, prerequisites (tests to pin, landed as the first commit and green on the base), tests that move, per-file steps led by their verb, and verification commands. Derive the tests-that-move table mechanically before the plan ships: for every symbol a `[[delete]]` row, a narrowing, or a signature change names, `rg -n '<symbol>' crates/rimz/src crates/rimz/tests crates/rimz/benches` and list every hit outside the owning module; and re-read the declaration line of every signature, derive, and literal the Target or a step quotes. A row that quotes a shape from a subagent report and not from the line is a guess: pass 15b shipped four such rows, each a coder stop. The line budget prices what a no-shim move adds as well as what it deletes: one line per file that imported the old and new homes together, and a header, imports, and `mod` lines per new module file; measure the largest move on a spike with `cargo xtask atlas diff --expect /tmp/<pass>/draft-contract.toml` before locking the ceiling, which the contract requires to be negative for a module pass (a seam contract's ceiling is flat: [Seam passes](#seam-passes)). The draft contract pins the merge-base SHA and lists every edited path, callers and tests included; use its totals row for the line budget, not the single-valued `--path` measurement. Run `cargo xtask atlas diff --expect` on that draft against the clean base before the plan ships: every row must resolve, and the printed base counts are the ceilings' starting figures. Count `esc` from `inspect --json` items plus the `pub use` names, never from memory. Set the ceiling from the target written out, not from the candidate list: each new signature in rustfmt form (a method that replaces a predicate costs its formatted lines, not one), the `?` error boundary each shared helper crosses (a helper over two error types is a generic or a new enum, never a free line), the escaping-item delta as an explicit sum of narrowings minus additions, and every pin's expected value read from the code path, not from the grammar as described (pass 14 wrote a `fri-mon` pin from the grammar and the sort at the end of `parse_days` reordered it). Pass 14 corrected its phase-2 budgets five times, each one a coder stop. If a sandboxed spike cannot write the worktree's shared `.git`, use an independent clone (`git clone --no-hardlinks <repo> /tmp/<pass>-spike`) and a separate build directory (`export CARGO_TARGET_DIR=/tmp/<pass>-spike-target`). Before locking the contract, read the code that enforces each row kind it uses rather than a model of it: the schema is `xtask/src/atlas/contract.rs`, every `diff --expect` row is judged in `xtask/src/atlas/diff.rs`, the per-commit ratchet is `xtask/src/atlas/conform.rs`, and what counts as production SLOC and as a dependency site is decided in `xtask/src/atlas/sources.rs` and `syntax.rs`. The [Caveats](./atlas.md#caveats) in atlas.md index the surprises already found; the code is the authority. Pass 1 wrote four plan defects (a zero budget for a new rule, a non-negative ceiling, a `pub use` counted as a definition site, and cfg accounting) from a model of atlas; ten minutes of reading would have saved two coder stops. The pass contract goes in the plan verbatim as TOML, written to a scratch path outside the worktree:
...
1. A pass row in the ledger's pass log: date, scope, base SHA, the verbs landed, the deltas copied from `diff --expect` on that commit (production SLOC, `esc`, dependency sites), and every candidate deferred with the reason. A deferred narrowing copies the `narrow to` column from `inspect --section surface` on the record commit, never a guessed visibility; a deferred deletion names the gate that must allow it (for example, clippy's `new_without_default` under `-D warnings` keeps `Default` beside `new`).
2. A module verdict row for every module the pass reviewed, carrying the reviewed source SHA and the churn threshold that reopens it (default: 30 scoped commits, `git log --oneline <sha>.. -- <path> | wc -l`): `holds` when nothing landed, `holds; landed pass-N` when the pass rethought the module's interior, since only a `holds` status demotes the module in the next survey. The SHA is the post-rebase source tip, the record commit's parent (a commit cannot carry its own hash); re-stamp it in the amended record commit when a post-review rebase rewrites the branch, and have the reviewer's delta round check that it resolves on the rebased branch. A seam pass that reviewed a module's edges and left its interior a candidate writes a bare `landed pass-N`.
...
5. The tightened `refactor-target.toml` from `conform --tighten --only <owned-path>` (repeat `--only` for each owned path); the diff carries only the pass's own rules.
6. Doc updates the change implies: the module's `AGENTS.md`, its internals page, the [code map](../../AGENTS.md#code-map) when a module moved, and [ARCHITECTURE.md](../../ARCHITECTURE.md) when the runtime shape changed. The plan's docs step lists the items whose visibility changed beside the items that moved, and the sweep covers the prose that names their reachers, not only the links to their old paths: pass 14 narrowed `wait_log_path` to `pub(super)` and its `AGENTS.md` still named the CLI among its callers at review. `CHANGELOG.md` stays untouched.

If a review fix or loosened contract lands after the row is written, re-measure and update the row in that same commit. A rebase that moves the merge base also re-measures and amends the record commit. Pass 1 wrote its row at step 7 (−2), review removed two more lines, and a tenth commit with its own review round existed only to correct the number to −4.
