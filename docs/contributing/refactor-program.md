...
A pass ends with one commit (or the last commit of the branch) that records it:

1. A pass row in the ledger's pass log: date, scope, base SHA, the verbs landed, the measured deltas from `diff` (production SLOC, `esc`, dependency sites), and every candidate deferred with the reason.
