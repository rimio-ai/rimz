...
`cargo xtask screenshot state <empty|fleet|provider> [--width W] [--height H] [--output PATH]` renders deterministic fixture frames through the same headless sidebar renderer used by tests.
...
Live capture reads the multiplexer grid and re-renders it through `freeze`; content, layout, ANSI colors, and glyphs are the review target. `freeze` writes an SVG and `rsvg-convert` rasterizes it so the installed Nerd Font is used for sidebar symbols. Client-side terminal details such as subpixel hinting and Ghostty font thickening remain outside this capture path.
