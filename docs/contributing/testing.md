...
- The one exception is the Codex app-server daemon, which fires hooks with no mux pane env so its session is unstamped (no pane id): an unstamped in-pane `codex` session binds the live `codex` pane in its own worktree by cwd (`paneless_codex_agent_binds_to_its_worktree_pane`), never a pane in another worktree (`paneless_codex_agent_in_other_worktree_stays_a_process_row`) and never a nested-worktree pane (`paneless_codex_agent_does_not_capture_a_nested_worktree_pane`); the cwd bind is `codex`-only (`paneless_codex_does_not_bind_a_non_codex_pane`, `paneless_claude_agent_is_never_rescued_by_cwd`), and two unstamped Codex sessions in one worktree bind the most-recently-active (`two_paneless_codex_in_one_worktree_bind_most_recent`).
