...
- `cargo xtask atlas <survey|brief|rank|seams|api|shapes|diff|inspect|conform>` is the bounded, versioned refactor-program instrument: survey a scope, brief candidate modules, inspect per-function assembly, write target schema v4, then conform and ratchet. All views roll up one per-file/per-item facts model; exact references come from rust-analyzer's cached SCIP export (`scip`/`protobuf`), while complexity uses `rust-code-analysis-cli` (`cargo install rust-code-analysis-cli --locked`). Reference-aware verbs fail fast when `rust-analyzer` is unavailable unless `--no-index` explicitly omits reference-derived output. `rank` auto-splits oversized rows; `diff --base` compares a base revision with the working tree; `inspect` quotes the heaviest function crossing one module boundary; `seams` joins `use`, exact-reference, caller, and rename-folded co-change evidence; `api` reports exact production/test referrers; `shapes` and the survey detectors expose repeated choreography; `conform` checks v4 grouped layers, admitted upward imports or exact allow-lists, surface budgets, and stranglers, while `conform --status` reports distance without changing the ratchet. `--path` scopes analysis except that `conform` accepts it only with `--init`; tables are top-N with complete totals and JSON is atlas v4. Measurement stays outside gates; only `conform --ratchet` runs in `gate` and `checks`, passing when no target file exists.
...
`cargo xtask atlas conform --ratchet` is distinct: it reads temporary refactor-program rules as data from root `refactor-target.toml` and fails only when current imports, escaping surface, or strangler counts exceed the checked-in baseline. `--tighten` only lowers baselines after improvement; raising one requires a deliberate target-file edit. The gate is inert when the target file is absent.
