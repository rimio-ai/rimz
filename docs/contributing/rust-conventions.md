...
`cargo xtask <task>` is the entry point for contributor automation; new automation lands in `xtask/`, and the only tracked hook script is `.githooks/pre-commit`, which routes back to it. Tasks: `build`, `build-plugin`, `plugin-refresh`, `install`, `install-dev`, `stage-install`, `dist`, `brew-formula`, `profile-build`, `hooks`, `fmt`, `lint`, `check`, `test`, `test-archive`, `deps`, `deny`, `vet`, `semver`, `externals`, `coverage`, `perf`, `complexity`, `survey`, `invariants`, `docs-links`, `gate`, `checks`, `ci`, `pricing-refresh`, `theme-refresh`, `screenshot`.

Four deserve a note:

- `cargo xtask complexity [--top N] [--code|--tests] [--path <prefix>] [--min-score S] [--json]` measures tracked source and test functions against the same cyclomatic, cognitive, and SLOC refactor thresholds via `rust-code-analysis-cli` (`cargo install rust-code-analysis-cli --locked`), groups actionable functions into separate source and test rankings with a top 10 default for each, counts inline `#[cfg(test)]` functions as tests, and lists the largest source and test files by LOC. `--path` limits analysis to tracked Rust files under one root-relative directory or file. Rankings hide file groups below score 2 by default; `--min-score 0` restores the full ranking. `--json` emits the v3 agent contract with `code` and `tests` sections, per-section untruncated filtered totals, largest-file lists, and the same section filters; the task remains outside every gate.
- `cargo xtask survey [--path <prefix>] [--deps] [--json]` maps tracked Rust SLOC into source and inline-test totals by top-level Rust module (a single-file `foo.rs` and directory `foo/` share one row), compares each module's rename-folded share of non-merge commits over fixed lifetime, 25%, and 10% windows, and reports retired-path history. `--path` defaults to `crates/rimz/src`; `--json` emits the v1 agent contract. `--deps` adds weighted cross-module edges, mutual pairs, and fan-in/fan-out via `cargo-modules` (`cargo install cargo-modules --locked`); that analysis can take minutes and holds Cargo's target-directory lock, so do not run it beside another Cargo command. The task remains outside every gate.
