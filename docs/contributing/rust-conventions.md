...
- `cargo xtask atlas <rank|seams|api|shapes|conform>` is the bounded, versioned refactor-program instrument. `rank` combines source/test SLOC, raw public items, effective escaping surface, rename-folded commit pace, and calibrated complexity (`rust-code-analysis-cli`, installed with `cargo install rust-code-analysis-cli --locked`); `seams` reports `use` edges, consumer and provider surface, Rust-source co-change with root dispatchers folded to one hub row, and import divergence; `api` reports declared/effective/implied reach with separate production and test name-match evidence; `shapes` clusters large functions by shared domain-call choreography; `conform` reads target schema v2 from root `refactor-target.toml` or a selected `--file`, reports import allow-lists, escaping `surface-budget` values, and stranglers, and can `--ratchet`, atomically `--tighten`, or `--init` a current-tree baseline. Analysis follows `--path`, tables are top-N plus complete totals, and every `--json` payload is atlas v2. The measurement verbs stay outside every gate; only `conform --ratchet` runs in-process in `gate` and `checks`, passing when no target file exists.
