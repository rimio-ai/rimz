...
- `cargo xtask atlas <rank|seams|api|shapes|conform>` is the bounded, versioned refactor-program instrument. `rank` combines source/test SLOC, boundary-public items, rename-folded commit pace, and calibrated complexity (`rust-code-analysis-cli`, installed with `cargo install rust-code-analysis-cli --locked`); `seams` reports `use` edges, consumer and provider surface, Rust-source co-change with root dispatchers folded to one hub row, and import divergence; `api` reports public shape and explicitly heuristic outside-module identifier occurrences (`occ`); `shapes` clusters large functions by shared domain-call choreography; `conform` reads root `refactor-target.toml` by default or a selected `--file`, reports generic import/public/strangler budgets, can `--ratchet` or atomically `--tighten` them, and can `--init` a current-tree baseline. Analysis follows `--path`, tables are top-N plus totals, and every `--json` payload is atlas v1. The measurement verbs stay outside every gate; only `conform --ratchet` runs in-process in `gate` and `checks`, passing when no target file exists.
...
`cargo xtask atlas conform --ratchet` is distinct: it reads temporary refactor-program rules as data from root `refactor-target.toml` and fails only when current imports or counts exceed the checked-in baseline. `--tighten` lowers baselines after improvement; raising one requires a deliberate target-file edit. The gate is inert when the target file is absent.
