...
- `cargo xtask atlas <rank|seams|api|shapes|conform>` is the bounded, versioned refactor-program instrument. `rank` combines source/test SLOC, boundary-public items, rename-folded commit pace, and calibrated complexity (`rust-code-analysis-cli`, installed with `cargo install rust-code-analysis-cli --locked`); `seams` reports `use` edges, external surface, bounded-commit co-change, and their divergence; `api` reports public shape and explicitly heuristic outside-module identifier occurrences (`occ`); `shapes` clusters large function control-flow skeletons; `conform` reads root `refactor-target.toml`, reports generic import/public/strangler budgets, and can `--ratchet` or atomically `--tighten` them. Analysis follows `--path`, tables are top-N plus totals, and every `--json` payload is atlas v1. The measurement verbs stay outside every gate; only `conform --ratchet` runs in-process in `gate` and `checks`, passing when no target file exists.
