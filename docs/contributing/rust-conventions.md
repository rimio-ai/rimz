...
Refresh the image by editing `ci/Dockerfile`, then pushing that change to `main` or dispatching `.gitea/workflows/ci-image.yml`. The workflow uses the `REGISTRY_PUSH_TOKEN` secret to build and push a new immutable `rimz-ci:<tag>` image to the Gitea container registry, then repoints the repository variable `RIMZ_CI_IMAGE`; consuming workflows read only that variable.
