...
Each toggle is independent: when it is set *and* that agent can start a host, `rimz start` launches it into one shared background view — a `rimz-rc` tab on Zellij, a `rimz-rc` window on tmux — out of your working pane and idempotent on that name (a second `rimz start` is a no-op). Both hosts live in that one view, side by side in separate panes.
