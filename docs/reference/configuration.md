...
- **`[zellij]`** carries a few invariants Rimz always applies — locked mode, click-through with focus-follows-mouse off, no session serialization (Rimz owns rebirth), and auto-layout — plus optional keys (`pane_frames`, `copy_clipboard`, …) that apply only when you set them and otherwise fall through to your `~/.config/zellij/config.kdl`. The sidebar pane is always borderless so its hit-testing stays stable regardless of `pane_frames`.
