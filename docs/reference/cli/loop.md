...
A `<kind>-ping` agent is the window-primer: the run skips when the provider's window is already counting down. `qwen-ping` binds a fresh managed launch to its exact Alibaba region and API key, permits a cold matching cache so that account can start its own window, suppresses a running 5-hour window, and uses the matching 7-day reset for `--every reset`; a missing binding or mismatched cache fails closed. Other `--every reset` tasks fire one minute after the provider's longest observed budget window resets, then use the ping turn's own cache refresh as the next occurrence.
