...
Signal deliveries carry `Type: SIGNAL` from `@rimz`, park at the receiver's next `done` boundary, and stay hidden in rendered transcripts while `--json` keeps them. This differs from self-only timer and command wakes, which steer.

Adding `--signal --wake` with the same target session, selector, normalized matches, and resolved root as a live instance row prints `already subscribed as <name>` and writes nothing. The existing name, prompt, provenance, and overlays remain unchanged. Session end, loss, or stop retires pinned rows; gc is the backstop.

The fired task's prompt opens the way a wake does ([the delivered message](./wake.md#the-delivered-message)): `waited on ci.failed on feat-x (PR #91)`, then `fired [<task>]`, with no elapsed because a `loop` row records no arming stamp. The payload follows as one compact JSON line, then the task's own `--prompt` verbatim; `--wake` tasks may omit the prompt entirely and deliver those lines alone. The run record keeps the signal name and its payload, replacing a payload over 4 KiB with a truncated rendering.
...
rimz loop add merged --signal pr.merged --once --wake @me
```
