...
When the selected records share one RimZ-created checkout, the panel prints a `since <timestamp>` line above the groups, including when the scope has no records left, using the configured local time zone at minute resolution (`YYYY-MM-DD HH:MM`). JSON preserves the exact RFC 3339 boundary as `scope.since`; `--md` omits the timestamp. A scope spanning several checkouts, an unmarked checkout, or a removed one leaves the boundary absent.
