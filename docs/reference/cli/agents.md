...
- `--agent <KIND>` launches every agent cell through that registered provider while retaining profile and team identity. Profile model, effort, prompt sources, and raw `args` carry over, then the other command-line flags win on top. Adapter-incompatible typed fields fail before RimZ creates a pane. This is a fresh-launch override: it conflicts with `--resume`, is not recorded as a profile edit, and a later `restart` refuses when the profile resolves back to a different provider.
...
- `--append-system-prompt-file <PATH>` is repeatable. RimZ composes the files in flag order after `--system-prompt-file` (or the profile's base prompt), with blank lines between pieces, and replaces the provider's system prompt with the result. A non-empty command-line list replaces the profile's whole `append-system-prompt-files` list. Claude, Codex, Qwen, and process plugins declaring a replacement flag support it; other adapters fail fast and name raw `args` as the escape hatch.
