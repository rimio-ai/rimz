...
- `--agent <KIND>` launches every agent cell through that registered provider while retaining profile and team identity. Profile model, effort, system-prompt path, and raw `args` carry over, then the other command-line flags win on top. Adapter-incompatible typed fields fail before RimZ creates a pane. This is a fresh-launch override: it conflicts with `--resume`, is not recorded as a profile edit, and a later `restart` refuses when the profile resolves back to a different provider.
- `--model`, `--effort`, `--budget <AMOUNT[/day]>`, and `--system-prompt-file` carry the same meaning and resolution rules as the [profile fields](../../guide/configuration.md#profiles) of the same names. Repeat `--append-system-prompt-file <PATH>` to replace the inherited fragment list in command-line order. A bare budget caps the session; `/day` resets at the configured local day boundary.
