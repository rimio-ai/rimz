...
`--model <MODEL>`, `--system-prompt-file <PATH>`, `--append-system-prompt-file <PATH>`, and `--effort <LEVEL>` broadcast to every agent cell like `PROMPT`, and each adapter renders them into its native flags: `--model` selects the provider model (Claude `--model <MODEL>`; Codex `--model <MODEL>`; Pi `--model <MODEL>`; OpenCode `-m/--model <provider/model>`), `--system-prompt-file` replaces the agent's base system prompt (Claude `--system-prompt-file <PATH>`; Codex `-c model_instructions_file=<PATH>`), `--append-system-prompt-file` appends rules to the base prompt where supported (Claude `--append-system-prompt-file <PATH>`), and `--effort` sets reasoning effort (Claude `--effort <LEVEL>`; Codex `-c model_reasoning_effort=<LEVEL>`; Pi `--thinking <LEVEL>`; OpenCode `--variant <LEVEL>`). The launcher resolves prompt files to absolute paths and refuses a missing file before launch. Levels are provider-specific — Claude takes `low|medium|high|xhigh|max`, Codex takes `minimal|low|medium|high|xhigh`, Pi takes `off|minimal|low|medium|high|xhigh`, and OpenCode accepts provider-specific variants — and a param without a native flag fails the launch with the offending flag named: Pi and OpenCode for prompt files, Codex for append. A configured profile preset renders first, so an explicit `--model` or `--effort` on the command line wins.
