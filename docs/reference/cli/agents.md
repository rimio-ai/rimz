...
Permission-mode cells exist where the adapter supports them: `-auto`, `-ask`, `-plan`, and `-yolo` set the permission posture, with `claude-plan` and `antigravity-plan` passing native plan mode while `codex-plan` and `grok-plan` add no argv and keep the default posture. The built-in set is `claude-{auto,ask,plan,yolo}`, `codex-{auto,ask,plan,yolo}`, `cursor-{auto,ask,plan,yolo}`, `antigravity-{auto,ask,plan,yolo}`, `grok-{auto,ask,plan,yolo}`, and `pi-{ask,plan}`. Grok Ask maps to `--permission-mode default`, Auto to `--permission-mode auto`, and Yolo to `--yolo`. On the command line, `--ask` keeps native prompts and `--yolo` passes the adapter's bypass flags; with neither, each provider keeps its own prompting.
