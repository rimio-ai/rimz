...
The default panel groups members by team and shows provider, model and effort, estimated active time, calls, messages, transcript-priced cost, token detail, and subagents as labelled lines. A pane-backed child launched through `rimz subagents` is grouped by launch profile in the subagents line, but contributes nothing to the member's own figures or to group and document totals; without a retained parent, the child is omitted. Asks lead the calls line. Messages read `{n} from you · {n} from teammates · {n} to teammates`; the Total line counts received messages once and excludes RimZ-authored automation. The final subagents line groups children as `{count} × {type} ({cost})`, using provider-reported type for native children and launch profile for pane-backed children, and folds description-like or missing types into `other`. Provider-native child spend is already inside the parent transcript, so its subagent figure breaks down the member cost; launched-child spend is separate from the member cost. Any unavailable or empty labelled figure is omitted rather than printed as an unknown placeholder.
