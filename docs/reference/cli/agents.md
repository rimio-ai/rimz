...
Fresh launches that open a new pane or tab print the absolute checkout path and each member's minted handle, provider, and resolved model (`-` when unset). If you supplied a prompt, a shortened echo names its recipient. A copy-ready `Reach` command targets the resolved leader, or the first minted member when no leader resolves. Startup remains asynchronous: the receipt records launch inputs, not provider readiness. Named teams also print the [team receipt](./teams.md#launch-a-team) with stages, board path, and inspection/wait hints; in-place launches replace the command pane directly and print no receipt.
...
The scope selects the lane; the lane's current worktree decides what counts inside it. A worktree RimZ created carries a creation timestamp in its [marker](./worktree.md), and attribution admits only the records registered at or after it, so a checkout removed and recreated under the same name credits the current incarnation alone rather than every generation the name ever had. A record whose checkout no longer exists counts nowhere, which is how removing a worktree removes its attribution; nothing is deleted, the audit rollup and provider transcripts keep the records, and `rimz agents show` still reports an excluded session when you address it directly. A checkout RimZ did not create has no timestamp to compare against, so the main working copy, a worktree you made by hand, and a directory below a managed checkout all keep the full retained history they report today. Roots and the pane-backed children they launched are each admitted by their own checkout and their own registration time. Attribution infers no commit or time window of its own; JSON timestamps let callers apply a narrower one.

When the selected records share one RimZ-created checkout, the boundary is on the page: the panel prints a `since <timestamp>` line above the groups, including when the scope has no records left, `--md` appends ` · since <timestamp>` to the summary line, both using the configured local time zone at minute resolution (`YYYY-MM-DD HH:MM`), and JSON preserves the exact RFC 3339 boundary as `scope.since`. A scope spanning several checkouts, an unmarked checkout, or a removed one leaves it absent.
