...
Creation uses the current directory's main Git repository, including when the command runs from one of its linked worktrees. If that repository differs from the room root, RimZ shows both paths and asks before proceeding. A non-interactive launch refuses that ambiguity; pass `--root <current-git-root>` to name the intended repository explicitly, or run the command from the room's checkout. When the current directory is not in a Git repository, the room root remains the creation root.
