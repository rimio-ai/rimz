...
`--md` emits a collapsed, linked `<details>` receipt for a pull-request body. Its summary identifies aggregate `agent-time`, cost, and the human contribution as author messages and answered asks. Detail lines link attribution jargon to its documentation, say `from the author` rather than addressing the PR reader as `you`, and end with linked `rimz agents attribution` provenance. An empty scope emits no Markdown. `--json` emits a schema 3 document, and `--json` conflicts with `--md`.
