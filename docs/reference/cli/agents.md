...
When the selected records share one RimZ-created checkout, the boundary is on the page: the panel prints a `since <timestamp>` line above the groups, including when the scope has no records left, `--md` appends ` · since <timestamp>` to the summary line, and JSON carries it as `scope.since`. A scope spanning several checkouts, an unmarked checkout, or a removed one leaves it absent.
