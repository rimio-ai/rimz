...
- **`gc`** removes stale runtime state older than `--older-than` (default `24h`), sweeps orphaned atomic-write temp files (`*.tmp.<pid>.<nonce>`) across the state and runtime trees, removes stale provider probe-throttle markers (`*-probe.*`) from the runtime shared dir, abandons pending feed items whose owner has exited, abandons queued messages for missing sessions, repairs a corrupt event-log tail, prunes provably dead workspace ledgers, and sweeps clean Rimz-marked worktrees whose work has landed with no live pane inside. It prints live progress and reports reclaimed disk grouped by category.
