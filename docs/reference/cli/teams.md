...
A member leaving a stage its role owns for a stage it does not own, `Done` included, needs a clean worktree: when `git status --porcelain --untracked-files=all` lists any path, the flip is refused before the board is touched and names the paths. Team memory files are git-excluded at launch, so they never count; a worktree outside Git has nothing to check. User flips, same-stage re-fires, moves between self-owned stages, and flips of another role's stage are exempt, so a human can still force a hand-off.
