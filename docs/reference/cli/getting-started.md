...
- `--web` runs remote `rimz web open --print --json`, starts a supervised SSH local-forward tunnel to the remote Zellij web server, prints `web: http://127.0.0.1:<port>/<session>`, opens the local browser best-effort, and stays in the foreground until Ctrl-C. `--web-token` creates and relays a one-time Zellij web login token before opening; without it, Rimz reminds you that this browser's first visit asks for a token. `--web-port <port>` pins the local browser origin; otherwise Rimz derives a stable port from the session name in `8300..8399`.
