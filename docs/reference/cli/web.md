...
`rimz web` is `rimz web open`. `open` starts from `PATH` or `.` and births the Rimz room with Zellij `web_sharing on` if needed; for an existing room, it asks the presence plugin to enable sharing at runtime. `--session <name>` targets an existing Rimz workspace session by exact session name. `--print` skips browser launch, `--no-start` refuses when `zellij web` is offline, and `--json` emits the `rimz.web.v1` payload instead of human output.
