...
Examples include `rimz sidebar snapshot`, `rimz sidebar serve`, `rimz statusline feed`, `rimz hooks feed`, `rimz queue deliver`, `rimz agents exec`, `rimz worktree cleanup`, `rimz claude ...`, and `rimz codex ...`. The owning internals docs describe the protocols: [ledger](../internals/sidebar/ledger.md), [state](../internals/sidebar/state.md), [agent](../internals/agents/agent.md), [provider](../internals/agents/provider.md), and [harness](../internals/agents/harness.md).
