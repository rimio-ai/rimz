...
A scoped window carrying `duration_mins` is a sub-cap: it folds onto an unscoped window of the same duration as a partition tick rather than taking a row, and remains hidden when that parent is absent. Optional `share_pct` is the sub-cap's size as a percentage of the parent's allowance; `used_percentage` remains on the sub-cap's own 0–100 axis. For example, `scope: {"id":"model:fable","label":"Fable"}`, `duration_mins: 10080`, `share_pct: 50`, and `used_percentage: 58` leave 21% of the parent allowance. An absent `share_pct` means the same axis as the parent (100% share); omit it for plain temporal windows and durationless named quotas. Scoped readings never refill through display projection, even when they carry a parent duration: an elapsed reset leaves usage unknown until the provider reports it again.
