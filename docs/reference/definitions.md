...
Host isolation skips skill checks at config load and ignores the invocation policy at launch; list syntax and the `Skill`-tool requirement still apply. The check reads the host's skill roots, so it runs only from a host process: inside a sandboxed pane, where an agent's own unlisted skills appear as RimZ's user-only copies, config loading and `rimz agents validate` skip it (validate says so on stderr), and the host-side exec wrapper refuses a launched profile whose definition fails there. From a host shell, `rimz agents validate` checks listed skills against both roots regardless of isolation. Validation resolves the provider root from the ambient environment; a named account whose login home holds a different skill set is checked when that account launches. Sandbox launch also requires provider support for skill views. See [configuration: skills](../guide/configuration.md#skills).
...
| Skills | Invalid names, missing `Skill` tool, a listed skill in neither the provider root nor the library, invalid policy metadata, or a listed user-only skill (including automatically added `reflect`). |
