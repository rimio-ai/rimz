...
Antigravity 1.1.2 combines safe native `PreInvocation`, `PostToolUse`, and `Stop` hooks with the stock workspace conversation cache and `brain/<conversation-id>/.system_generated/logs/transcript.jsonl`. Its wrapped custom statusline supplies model, version, plan/account identity, context usage, and a read-only permission-wait marker that raises the card and routes focus to the pane. A private read-only service on a verified same-uid running `agy` process pairs account status and quota on one endpoint, publishes conservatively normalized authoritative `5h`/`7d` bars under a hashed owner key, and invalidates stale windows on an account switch; RimZ starts no helper, reads no credential, and claims no credits or account dollars. Hook-bound sessions and exact `agy --conversation <id>` resumes bind independently of the latest cache, so a lagging workspace entry cannot hide a transcript-backed native question. RimZ leaves `PreToolUse` uninstalled because every documented response changes native permission policy, so the permission decision and all question/artifact waits remain in Antigravity's own pane. No 1.1.2 Stop error has a twice-captured typed recovery discriminator: every Antigravity error remains terminal, and quota exhaustion alone never preserves supervised work or arms auto-continue.
...
Antigravity CLI 1.1.2 documents observer-neutral `{}` output for invocation and post-tool hooks and a non-`continue` `Stop` decision that permits termination. RimZ installs those events and wraps the custom statusline, while deliberately excluding `PreToolUse`; the provider's UI remains the only permission and question decision surface. Validated transcript questions project native waiting cards, a hook-captured prompt stays sticky when a long bounded transcript tail no longer contains it, and a new in-place conversation ID takes ownership of the same guarded pane. Priceable statusline model/current-token values provide a local API-rate estimate for the live card without claiming historical or provider billing, the provider dashboard shows active session count alone when no cumulative ledger exists, and the verified running local service fills account and quota between turns without OAuth or a spawned `agy`. `rimz hooks install antigravity --dry-run` shows both `hooks.json` and `settings.json` changes before consent, and uninstall restores the prior statusline command.
