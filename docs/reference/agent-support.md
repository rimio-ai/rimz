...
Antigravity 1.1.2 combines safe native `PreInvocation`, `PostToolUse`, and `Stop` hooks with the stock workspace conversation cache and `brain/<conversation-id>/.system_generated/logs/transcript.jsonl`. Its wrapped custom statusline supplies model, version, plan/account identity, context usage, and a read-only permission-wait marker that raises the card and routes focus to the pane. Hook-bound sessions and exact `agy --conversation <id>` resumes bind independently of the latest cache, so a lagging workspace entry cannot hide a transcript-backed native question. RimZ leaves `PreToolUse` uninstalled because every documented response changes native permission policy, so the permission decision and all question/artifact waits remain in Antigravity's own pane.
