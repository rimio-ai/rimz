...
| `selection_bg` | the selected card's background band, behind every line of the card; a subdued blend of the scheme's `colors.selection.background` toward the background, else a faint background tint. At truecolor depth the band recesses a flat step below this tone, so the selected card reads as one recessed panel |
