...
An agent launched in your current pane temporarily names that tab after its profile or agent kind: `rimz agents astra` shows `astra`, without the project suffix. Channel labels stay unchanged. The name stays readable while the agent runs, including when sandboxing is enabled. When the agent exits, the original tab name returns; tmux resumes automatic process-based naming if it was enabled before the launch, so a shell tab shows `zsh` again, for example.
