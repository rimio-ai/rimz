...
Per-agent and per-team lifetime figures group every continuation of one durable seat, select their transcript entries, and deduplicate replayed API calls across resumed sessions before pricing. `rimz agents show`, `rimz teams show`, finished sidebar receipts, and attribution use the same all-in dollar and token fold across the seat and every subagent it spawned. Attribution also labels durable conversation traffic by direction — `from you`, `from teammates`, and `to teammates` — and credits asks from the same log, while its subagent line breaks the all-in spend down by provider-reported type or launch profile. Expand a finished receipt and each member card shows its seat's lifetime cost, adding back to the pin; a live card shows its current provider session plus the sessions it launched.

Those four surfaces count a worktree's current life. RimZ stamps each worktree it creates with a creation time and counts the sessions registered at or after it, so removing a worktree and starting again under the same name gives the new work its own tally while the earlier sessions stay in your history and drop out of these figures. A checkout RimZ did not create, your main clone included, has no such stamp and keeps reporting everything it has retained.

Estimated active time comes from per-agent runtime sidecars: RimZ credits observed progress, closes the span when the agent stops, and grace-caps a live span when progress goes quiet. A team or attribution total is the sum of those per-agent spans, not the wall-clock time elapsed, so parallel work can make agent-time exceed the change's duration. Runtime sidecars age out under `rimz gc`, leaving older attribution records with no active-time figure.

The cache-hit percentage measures prompt input served from cache: `cache read / (cache read + cache write + fresh input)`, rounded to the nearest whole percent. Output is outside the denominator. A live agent card uses cumulative counters for that provider session, covering the same lifetime as its session dollar figure; the Models and Agents rows use the selected stats window. A missing or zero input-side denominator stays absent. Green means at least 90%, yellow means 70–89%, and red means below 70%.
