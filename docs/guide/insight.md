...
Per-agent and per-team lifetime figures group every continuation of one durable seat, select their transcript entries, and deduplicate replayed API calls across resumed sessions before pricing. `rimz agents show`, `rimz teams show`, and finished sidebar receipts also fold in pane-backed subagents, while `rimz agents attribution` lists their spend only on the subagents line and keeps it out of member and total figures. Attribution labels durable conversation traffic by direction — `from you`, `from teammates`, and `to teammates` — and credits asks from the same log, while child spend is grouped by provider-reported type or launch profile. The surfaces agree on root-seat dollars and token split; show, teams, and sidebar figures additionally include delegated sessions. Expand a finished receipt and each member card shows its seat's lifetime cost, adding back to the pin; a live card shows its current provider session plus the sessions it delegated.
