...
None of this can fail into a wrong-looking number. Spend is enrichment, so a missing binary, a logged-out account, or an unpriced model degrades to a blank or a zero, never a bad figure dressed up as a real one. A live card may carry a clearly marked `≈$` local estimate where no discoverable history exists; that estimate stays outside the cockpit, provider dashboard, `rimz stats`, and budgets. For the mechanism in full, the caches, the price-table precedence, and the window fusion, see [providers internals](../internals/agents/providers.md).
