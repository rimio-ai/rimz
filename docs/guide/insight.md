...
Per-agent and per-team lifetime figures group every continuation of one durable seat, select their transcript entries, and deduplicate replayed API calls across resumed sessions before pricing. `rimz agents show`, `rimz teams show`, and finished sidebar receipts also fold in pane-backed subagents. Attribution keeps those children out of the root seat's dollar figure but makes their spend explicit: their subagent rows say `launched`, and the member, group, and Total lines carry a separate `+... launched` figure. Provider-native rows say `native`, and their spend is already inside the root seat's cost. Attribution also labels durable conversation traffic by direction — `from you`, `from teammates`, and `to teammates` — and credits asks from the same log, while child spend is grouped by provider-reported type or launch profile. The surfaces agree on root-seat dollars and token split; show, teams, and sidebar figures additionally include launched sessions in their all-in figure. Expand a finished receipt and each member card shows its seat's lifetime cost, adding back to the pin; a live card shows its current provider session plus the sessions it launched.
