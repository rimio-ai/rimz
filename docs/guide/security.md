...
Stock agent CLIs share the host's `/tmp` and discover every installed user skill. On Linux, `rimz config set agents.isolation sandbox` uses bubblewrap to give each agent pane the room's shared `/tmp`, merge the RimZ skill library into its provider's skill root, and optionally limit automatic invocation through [profile skill lists](./configuration.md#profiles). It probes before enabling; start refuses when bubblewrap is missing or unusable.
