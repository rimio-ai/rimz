...
The **executable surface** is every project field that can cause a process to run: agent launch commands, project profile and team definitions, hook commands, PATH-affecting env overrides, and any future project command string. A single hash over all of these is what `rimz trust grant` pins, and the grant stores the surface itself so a `stale` report shows a field-level diff of what changed before you re-grant. Room layout (`[layout]`, including tmux status `#(...)` and popup commands) is per-machine config; a project config carrying it is refused with the move-it fix. Adding a new project command-running field that isn't in the hash is a CI invariant violation. Implementation detail in [`docs/internals/harness/trust.md`](../internals/harness/trust.md).
