...
`[web] auth_header` delegates the public decision to an authenticating reverse proxy. RimZ's gate validates the proxy source, requires the header on every request, strips client-supplied `Authorization`, and injects the machine's Basic credential before forwarding to ttyd. The proxy strips client-supplied copies of the identity header, injects its own value after authentication, and remains the only non-loopback source that can reach the gate.
