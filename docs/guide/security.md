...
On Zellij, rimz loads a small presence plugin into each session so the sidebar learns of pane changes by push instead of polling and tab switches land back on work instead of the sidebar ([internals](../internals/sidebar/multiplexers.md#zellij-presence-channel)). With `[web] enabled` true, Rimz seeds Zellij's own permission cache for its embedded plugin before load, keyed to the canonical plugin path Rimz materializes under the user data directory. The seeded grant covers **Access Zellij state** (it watches pane/tab shape), **Run commands** (it runs rimz-owned `rimz sidebar wake` and `rimz sidebar focus` argv), **Reconfigure** (it applies Rimz's room mouse options and, when configured, binds the [focus key](../reference/configuration.md#sidebar-rendering) to a runtime-only plugin pipe without writing your `config.kdl`), and **Start web server / share session** (it enables browser access to the room when you run `rimz web open` against an already-running session). The plugin reports a switched-to tab that restored focus to the sidebar; the matching renderer moves focus through the same host command used for an ordinary sidebar jump. Revoking the grant makes sidebar presence fall back to Zellij's prompt or Rimz's poll; setting `[web] enabled = false` stops Rimz from seeding new grants and makes web commands fail fast before changing room sharing. The plugin's argv, artifact, and configuration are all rimz-owned (never your `config.kdl`), it ships no pane content anywhere, and the grant stays in Zellij's own permission store where its plugin manager can revoke it.
