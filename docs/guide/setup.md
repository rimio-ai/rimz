...
rimz loop add follow-reset --spec claude-ping --at-reset   # re-prime when the window resets
```
...
The ping defaults its prompt to `ping` at the lowest effort, skips when the provider's window is already counting down, and fires only while a room for `root` is open. The same `[tasks]` table also schedules watchdogs and self-wakes — an agent turn on an interval, gated on a shell check such as `cargo test` or `gh run watch` — covered in [configuration → Loop tasks](../reference/configuration.md#loop-tasks) and [the loop CLI](../reference/cli/agents.md#schedule-turns-with-loop).
