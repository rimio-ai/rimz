...
Every other key, `pane_frames` and `copy_clipboard` among them, is passed only when you set it and otherwise falls through to your `~/.config/zellij/config.kdl`. Five behaviours are not keys at all: the room starts in locked mode, a directionless new pane splits the focused pane rather than the largest one, swap layouts are off, every view carries RimZ's own layout with its one-row compact bar in place of Zellij's built-in tab and status bars, and the sidebar pane is always borderless so its hit-testing stays stable whatever `pane_frames` says.
...
Three tmux behaviours are not keys either: window titles (`set-titles` with the `set-titles-string` that carries a pane's title to the tab), the three server `terminal-features` entries behind synchronized output, extended keys, and hyperlinks, and the three root key bindings for Shift+Enter, Alt+Enter, and RimZ's own `User240` signal.
