...
A profile is a named agent preset, addressable as `@<name>` once it is running. `agent` is its base, a built-in kind (`claude`, `codex`, …) or another profile that resolves to one, and the remaining **override fields** layer on top: `mode` (`auto` | `ask` | `plan` | `yolo`), `model`, `effort`, `budget`, `system-prompt-file`, and raw `args`. `budget = "5"` caps the session and `budget = "20/day"` resets at the configured local day boundary. Team roles expose the same fields; loop tasks expose the launch fields relevant to scheduled work. The template and `rimz config get` carry the current per-field defaults. Model, effort, and adapter-declared system-prompt path flags repeated in raw `args` are reconciled at launch: the typed field or launch flag wins and RimZ warns when its value differs, while a model set only in `args` becomes the launch model and suppresses the adapter default. Qwen's typed replacement is an environment variable rather than an argv flag, so a raw provider `--system-prompt` remains an explicit provider-specific override.

Inheritance flattens at launch to one concrete adapter kind, and **the nearest set value wins for every field, including `args`**: a child that sets `args` replaces the base `args`. `system-prompt-file` names one complete replacement system prompt. RimZ resolves the path to an absolute path and passes that same file directly to the adapter; it never reads, copies, or combines the contents. Claude, Codex, and Qwen support replacement, as do process plugins that declare a replacement path flag. An unsupported adapter fails the launch and points to raw `args` as the provider-specific escape hatch. A `~` expands to home and a relative path roots at the declaring config file, so a role in a drop-in `team.toml` can name a prompt beside that fragment and keep pointing at it wherever the team launches; the file must exist at launch, and a missing one fails with the path to fix.

For additive project or role guidance, use the context files the agents already load natively, such as `AGENTS.md`, `CLAUDE.md`, or `QWEN.md`. Those files remain additive context under each provider's own discovery rules; RimZ does not merge them into the system prompt. Keep a combined file yourself when the guidance must be system-role text. The removed `append-system-prompt-file` and `append-system-prompt-files` config fields fail with this guidance instead of being ignored. Command-line `--model`, `--effort`, `--budget`, and `--system-prompt-file` override the profile value.
