...
Symlinks in the provider's skill directory keep their targets, so skills can still import shared modules beside their canonical location. Only directories containing `SKILL.md` receive user-only markers; shared folders and instruction files are left alone. Rewritten copies appear read-only at the skill's canonical path inside the sandbox, even when that path is outside the provider's skill directory. If two names point to the same skill, list both or neither; a mixed policy refuses launch.
