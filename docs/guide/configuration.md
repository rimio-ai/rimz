...
Every other key, `pane_frames` and `copy_clipboard` among them, is passed only when you set it and otherwise falls through to your `~/.config/zellij/config.kdl`. Six behaviours are not keys at all: the room starts in locked mode, a directionless new pane splits the focused pane rather than the largest one, swap layouts are off, resizing stacks panes rather than shrinking them, every view carries RimZ's own layout with its one-row compact bar in place of Zellij's built-in tab and status bars, and the sidebar pane is always borderless so its hit-testing stays stable whatever `pane_frames` says.
...
Three tmux behaviours are not keys either: window titles (`set-titles` with the `set-titles-string` that carries a pane's title to the tab), and the server `terminal-features` entries for synchronized output and for hyperlinks. Two more are governed by `extended_keys` rather than standing alone: the `extkeys` terminal feature, which RimZ unsets when you turn the key off, and the three root key bindings for Shift+Enter, Alt+Enter, and RimZ's own `User240` signal, which it stops writing.
...
To try one launch the other way, pass `--isolation host` or `--isolation sandbox` to `rimz agents`, `rimz teams`, or `rimz subagents`. That agent keeps the choice through restart and resume, and its subagents inherit the override, but never run looser than their parent's effective isolation: a sandboxed parent refuses a child's host override and clamps host defaults to sandbox with a note. Inside the mount view a Codex agent runs with its own command sandbox off (`--sandbox danger-full-access`); host mode leaves it on.
...
On the host, Claude keeps unlisted skills user-only through `--settings skillOverrides`; Codex hides them through `-c skills.config`, including from explicit `$skill` invocation. Other providers warn and run unrestricted. Neither switch has a wildcard, so skills installed after launch stay callable until restart. Your Codex `config.toml` skill restrictions remain in effect. In sandbox mode, Antigravity, Amp, OpenCode, Kiro, Grok, and plugins refuse a list outright; remove the field to launch them. Changing a trusted project's skill list means granting trust again. The provider-by-provider table is [agent support](../reference/agent-support.md), and the exact merge rules and limits are in [sandbox internals](../internals/sandbox.md#host-mode).
...
Two fields enter the trust hash but do nothing yet: `launch_command` on an `[[agents]]` entry, and a `[[hooks]]` table. RimZ neither runs the command nor installs the hook today. One table is refused outright: a `[layout]` table fails the load with an error naming what replaced it, `placement` under `[agents]` for where a launch lands and a team definition for a whole room shape. Per-machine config has no `[layout]` either, so the table goes rather than moves. RimZ's own [`.rimz/config.toml`](../../.rimz/config.toml) is a working example of project tasks; its repository sync task assumes push rights on the remote.
