...
- **Codex.** `codex remote-control start` runs once, detached: it brings up Codex's per-user app-server daemon with remote control enabled and returns. The daemon is one per machine, shared by every room — the same daemon Codex's own TUI already routes through.
