...
> This is the vocabulary the rest of the docs lean on: how you launch agents, how one spec becomes a laid-out room, how a worktree isolates a line of work, and how a team puts named roles on one feature. The command surface behind every example is the [agent-control reference](../reference/cli/agents.md); the launch and worktree machinery is [harness.md](../internals/harness/harness.md) and [worktree.md](../internals/harness/worktree.md).
...
**A suffix sets the permission mode.** `-auto`, `-ask`, `-plan`, and `-yolo` render into the adapter's own flags, so `claude-plan` starts in plan mode and `codex-yolo` skips provider prompts. A `-ping` suffix opens the agent at lowest effort to keep the provider's budget window warm. The built-in set is `claude-{auto,ask,plan,yolo,ping}`, `codex-{auto,ask,plan,yolo,ping}`, and `pi-{ask,plan}`; a mode a given adapter has no equivalent for keeps that provider's default posture. On the command line the same choice is `--ask` (keep native prompts) or `--yolo` (pass the bypass flag).

```sh
rimz agents codex-yolo      # launch straight through provider prompts
rimz agents claude-plan     # start in plan mode
rimz agents claude --yolo   # the flag form of the same posture
```
...
rimz agents forge.reviewer          # re-add one role, same handle and lane
...
**Relaunch reconciles instead of duplicating.** Point `rimz agents <team> -w <name>` at a worktree that already holds that team, and Rimz reads the state first: a live team focuses its tab, a closed team with work in progress offers to resume it, and a clean merged tree offers to remove it and start fresh. `--resume` (alias `--continue`) forces the resume path — reopening the newest matching cohort by team name and role, or by cell order for an inline spec. Resume takes identity, cwd, and channel from the store, so it stands alone: no prompt, model, or channel flags ride with it.
...
That is enough to launch, re-add, and jump to agents. Routing text to them — parking at the turn boundary, steering the live turn, broadcasting to a lane — is the [Messaging guide](./messaging.md), which owns the full address grammar and delivery rules.
