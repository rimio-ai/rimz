...
The `COST` in `rimz teams show` and the team's collapsed finished sidebar receipt use the same lifetime fold across every resumed session of each role, including pane-backed subagents. An attribution footer uses the same root-seat dollars and tokens but lists pane-backed child spend only under `subagents`, so its totals can be lower. Expanding that finished receipt puts each role's lifetime cost on its card, and those cards add back to the receipt; live cards remain scoped to the current provider session.
