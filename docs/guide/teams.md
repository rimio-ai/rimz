...
The `COST` in `rimz teams show` and the team's collapsed finished sidebar receipt use the same lifetime fold across every resumed session of each role, including pane-backed subagents. An attribution footer keeps the root-seat dollars and tokens as its main figure, marks provider-native subagent rows as `native` because their spend is already included there, and marks pane-backed rows as `launched`. Pane-backed spend appears separately as `+... launched` on the member, team, and Total lines rather than disappearing into the root-seat figure. Expanding that finished receipt puts each role's lifetime cost on its card, and those cards add back to the receipt; live cards remain scoped to the current provider session.
...
At launch, adapters with reminder support (currently Claude, Codex, Qwen, and Droid) tell each member its worktree, team, role, channel, leader and teammates; whether the session is fresh or resumed; and which declared scratch files existed then, with line counts. On a reused worktree, RimZ reports leftover scratch files as earlier run state rather than touching them, so the member can read them before acting.
