...
At launch, every member learns its worktree, team, role, channel, leader and teammates; whether the session is fresh or resumed; and which declared scratch files existed then, with line counts. On a reused worktree, RimZ reports leftover scratch files as earlier run state rather than touching them, so the member can read them before acting.
