...
**Dollar budgets bound hands-off work.** `--budget 5` caps each fired run; `--budget-per-day 20` makes the scheduler sum that task's completed run costs in the configured local day and skip a fire that cannot fund its per-run cap. Room-fleet and provider-account daily caps gate the same fire before launch and record `budget skipped` with the exhausted scope. `rimz loop list` shows each task's spend against its daily cap, while `rimz loop show` reads back each run's cost and fresh input/output tokens plus the last-run and rolling ten-run average cost. A `/day` park reopens at the next local day; a human message can waive one interactive turn but never satisfies a loop gate.
