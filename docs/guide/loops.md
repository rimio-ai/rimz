...
> Loop engineering is where Rimz keeps the fleet moving while you sleep: you define the goal and the stopping condition, and a cycle of act, observe, decide, repeat runs without you at the keyboard. This page owns the four layers that cycle is built from — built-in recovery, scheduled turns, watchdogs, and notification handlers — plus the permission posture that makes an unattended run safe. The mechanics underneath are [harness.md → Scheduled turns](../internals/harness/harness.md#scheduled-turns-loop); the intelligence in the loop stays yours to build.
...
Two settings keep a live agent working through the interruptions that would otherwise park it until morning. Both are off by default and switch on with one `rimz config set`.
...
**Auto-continue** picks a parked turn back up on its own. A rate-limit or spend-limit park resumes the moment the provider's budget window resets, and a transient overload or API error retries on a bounded backoff ramp (`auto_continue_backoff_secs`, default `[180, 300]` — the first retry after 3 minutes, then every 5, capped by `auto_continue_max_retries`, default 13). Recovery types the nudge (`continue` by default) into the agent's live pane through the same path as a steer message, so the agent's next hook moves the row back to running. The tuning knobs are in [configuration.md → Resume](../reference/configuration.md#resume); the decision logic is [provider.md → Auto-continue](../internals/agents/provider.md#auto-continue).
...
`rimz loop` fires a supervised agent turn on a clock. Each task names one agent cell with `--spec` (a kind, a profile, or a virtual cell like `claude-ping`), carries a `--prompt`, and picks one firing shape. The room's elected sidebar elder keeps time while a room for the task's project is open, so tasks fire whenever that project's room is alive.
...
- **Calendar** — `--at 07:00` with an optional `--days` mask (`daily`, `weekdays`, `weekends`, a range `mon-fri`, or a list `mon,wed,fri`).
- **Interval** — `--every 15m`, `2h`, or `1d`, measured from the last fire.
- **Raw cron** — `--cron "*/15 * * * *"`, a five-field expression.
- **One-shot** — `--once` on a calendar or cron schedule, or `--in 30m` to fire once after a delay.
- **Window-reset** — `--at-reset` on a `<kind>-ping` spec, tied to the provider's budget-window reset (below).
- **Poll-until** — `--every` plus `--check`, `--on`, `--until`, and an agent action (below).
...
**Use the agent's bypass flag** for runs where you accept the tradeoff. `rimz agents <kind> "<prompt>" -p --yolo` passes the adapter's bypass flag (`claude --dangerously-skip-permissions`, `codex --dangerously-bypass-approvals-and-sandbox`), while `--ask` keeps the provider's prompts in place. Rimz still observes sessions, completions, and failures through lifecycle hooks; the tradeoff is that the agent skips permission events at the source, so the store holds what other hooks report rather than a per-decision audit trail.
