...
The tab bar carries the same glance when the sidebar itself is off-screen. RimZ appends the tab's most urgent live-agent glyph to its existing name: `#feat !` for a failure, `#feat ?` for an ask, `#feat ⏸` for a provider pause, `#feat ⢿` while work is running, and `#feat ✓` for a recent clean finish. Idle tabs keep their bare names, and `✓` clears after five minutes; the other marks stay until the underlying state changes. A manual tab rename is preserved — RimZ manages only the final glyph suffix — and the shapes follow your configured [glyph set](./theme.md#glyphs).
