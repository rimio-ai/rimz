...
The tab bar carries the same glance when the sidebar itself is off-screen. RimZ appends the tab's most urgent live-agent glyph to its existing name: `#feat !` for a failure, `#feat ?` for an ask, `#feat ⏸` for a provider pause, `#feat ⢿` while work is running, and `#feat ✓` for a recent clean finish. Idle tabs keep their bare names, and `✓` clears after five minutes; the other marks stay until the underlying state changes. A tab RimZ named after its agents goes back to the shell's name when they all exit; your own tab names are kept. The compact Unicode shapes stay fixed even when the sidebar uses a configured [glyph set](./theme.md#glyphs).
...
The body: one card per pane, grouped under the worktree it lives in. A worktree is total isolation, so each group reads as one bounded block, and the group header names a linked pull request (`#91`) when one exists and carries the work's git story on the right: commits ahead and behind the trunk, lines added and removed (uncommitted and untracked work counts too), and a marker for where the work stands, from a plain branch through an open, merged, or closed pull request. An open or merged PR supplies the `✓`, `✕`, or `◌` CI verdict beside its badge; a non-trunk branch without a PR shows no CI glyph, while the trunk shows its own HEAD-commit CI. Closed PRs show no CI glyph. The team label goes on the pipeline line when one is drawn, on the roster line when the group is folded, and on the header as `· forge` otherwise. It names the unique team among the group's team members, so roleless shells and stray agents do not hide it; two distinct teams leave the group unlabeled. The `#91` badge opens that pull request through the terminal's OSC 8 hyperlink support without extra multiplexer configuration: Zellij enables OSC 8 by default, and RimZ declares tmux's `hyperlinks` terminal feature (supported by tmux 3.4 or newer). Panes outside every project checkout fold into a dim `external` divider at the very bottom.

Cards arrive already triaged (the [ranking below](#how-the-column-is-ordered) decides the order), and a busy worktree caps at six rows with a `+K more` line that only hides idle agents and quiet shells while work remains active. A finished line of work with several agents collapses completely to a muted-bold header and a two-line dim receipt, including unread success cards: `▸` marks the expandable roster line, a shared team name leads the members when present, each member keeps its status glyph and softened provider-brand name, overflow and quiet shells fold into `+n`, and tracked cost pins right; the totals line shows the pod's token breakdown followed by its aggregate session cache-hit percentage and pins its muted finished age on the right. A narrow multi-agent roster falls back to count-only `▸ +K done`; click the header or either receipt line, or apply the `s` status filter, to reveal the roster, then click the header to collapse it. Process-only groups and groups with one agent keep their rows visible. A focused or order-held member reveals the whole roster, so the pod is never half-collapsed. The receipt's cost and tokens cover the sessions of this worktree's current life, counted the way [Token Insight](./insight.md#how-the-numbers-are-calculated) explains.

A staged team with a board shows its progress directly below the group header: green `●` passed stages, `◉` the current stage, muted `○` future stages, then the stage name, a parenthesized clock, and the team label. The clock measures time in the current stage, restarting when a flip enters a different stage. At `Done`, it freezes as the whole run's total. Click the line to reach the stage owner. It is progress, not another request for attention. A folded finished group keeps the stage beside its team name instead. The [interface reference](../interface/sidebar.md#worktree-headers) covers missing clocks, narrow panes, and click fallbacks.

### The provider dashboard

Budgets are account-scoped, one account shared by every session of a provider in the room, so they live in a pinned panel at the bottom rather than on the cards. Each provider block names the account and plan (`Claude Max`, `ChatGPT Pro`), and a room on a [named account](./accounts.md) labels the block with it (`Claude · work`) and drains a "mana" bar per budget window (5-hour, 7-day) toward its reset, so a glance reads how much of the plan is left. The block also carries that provider's spend, and two totals rows below sum the fleet across the trailing week and month. Reading those figures, and how every one is calculated, is [Token Insight](./insight.md).
...
- **What it is working on.** The session's name or task, falling back to its first prompt so the card stays stable between turns. Codex names sessions automatically, so its generated short title replaces the raw first prompt as soon as it is available. When a turn dies on a provider error, this line quotes the error (`API Error: Overloaded`) so the card says why without a jump.
...
- **The token line.** The absolute companion: tokens currently in the window, the same composition as markers, a `↻ N` count of completed context compactions, an amber `⟲ N` while a running agent is between the identical-call warning and attention thresholds, and, once the agent has been quiet for five minutes, its last-activity age pinned right, heating toward red as an hour approaches. The age is the agent's own quiet time: an agent waiting on its subagents keeps counting, because its own session is idle and its context will be re-read when it resumes. At the attention threshold the marker yields to `!` and the description carries `loop: <tool> ×<count>`; the next differing call clears the run and returns the card to its running state.
- **The subagents and waits line.** Once the session has spawned a child, its lifetime count and known lifetime cost stay on the card throughout the retained session history, sharing the line with any pending waits. The figure covers provider-native and RimZ-launched children, but is a breakdown rather than a second charge to add to the identity line: native cost is already there, and launched-child cost is the portion the identity line adds.

To check delegated work without leaving your pane, click the subagents and waits line to open or close its entries; the choice sticks, even on the selected card and across your next prompts. Selecting a card opens that section by default: the **subagent entries** appear underneath, each with its own live state, what the parent asked it to do, and, while it runs, tokens, model, and elapsed time. Running children come first in the order they started; finished ones follow newest first with how long ago they finished, while they finished in the last 15 minutes, up to five of them. A child launched with `rimz subagents` also shows its launch profile, cumulative displayed session tokens (excluding cache reads), and own session cost. When that agent belongs to a named team, every visible teammate's card expands with it, while only the selected card carries the highlight. Every other finished child folds behind `+K older` beneath the entries while its history is retained; the listed children plus `K` always match the count. Click that tail to append them below. When no child is running or recently finished, there is nothing to fold behind, so opening the section lists the older children straight away. The older list closes again at your next prompt to the parent, while messages from other agents and RimZ's automatic deliveries leave it open. The window and the cap are `recent_subagent_secs` and `max_recent_subagents` ([theme.md → Display](./theme.md#display)). Still-running children launched with `rimz subagents` remain visible across turns. Provider-native subagents are headless; a launched child has its own pane and transcript while it runs, then normally closes that pane while keeping the verdict here. `--keep` holds the pane instead. Both kinds stay nested under their parent rather than becoming duplicate top-level cards.

How much of the card shows at rest is yours to tune with `card_density` ([theme.md → Display](./theme.md#display)): `compact` trims resting cards, including the shared subagents and waits line, `expanded` opens subagent and wait entries everywhere. In compact mode, select the card first to expose the clickable count line. Narrow sidebars shorten the shared count line to `⧉ N · ⧖ M` so both counts and the cost fit.

The **waits count** covers armed one-shot waits: timers, existing processes, watched shell commands, and one-shot signals. It shares the subagents line and disappears when none remain. It stays visible on standard cards while the agent works as well as while it sleeps. Selecting the card or opening its count line lists each wait beneath the subagent entries. The icon shows what is happening and the text names the wait: `◷ in 12m` for a timer and `⌁ pr.merged · 2h left` for a signal hold still, while a shell job such as `cargo test` or `pid 16776` spins the working animation in violet as long as it runs. The elapsed time on the right shows how long each has been pending. A Claude agent's background shells (commands it left running with `run_in_background`, or that Claude moved to the background) join the count as shell jobs, with elapsed time since RimZ first saw them; one Claude described reads as that description with its command on a second line. Each disappears when Claude reports it finished.
...
You do not read where to go; you go. Press `n` (or `␣`) to jump to the next thing that needs you, oldest first, and RimZ focuses that agent's pane. The prompt and its safe defaults live in the agent's own UI, where the full context is, so you read and answer there; focusing the pane clears its unread mark, and `N` walks back. From any pane in the room, `Alt+p` focuses the sidebar and toggles back to your last working pane, so the whole loop runs without touching the mouse. The full key table is in [the interface reference](../interface/sidebar.md#keys-and-mouse).
