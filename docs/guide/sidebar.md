...
The sidebar follows its attached view at the explicit `[theme.display].width_percent`, otherwise 30% on views wider than 240 columns or when pets are enabled, and 25% otherwise, capped at `[theme.display].max_cols` before backend snapping. Press `a` to make it narrower, press `d` to make it wider, or drag the sidebar border with the mouse: each action sets one room-wide share of the screen and every tab follows within a tick. tmux preserves the selected column exactly when the share is set; Zellij snaps to the nearest 5% share rung, so the configured cap or dragged width may settle within half a step while its siblings land on the shared rung. A narrower press is ignored when Zellij's next reachable width would fall below 24 columns, while tmux lands exactly on that floor. The pinned share keeps its proportion through terminal resizes, reloads, and reattaches for the life of the session and may exceed `max_cols`; a newly created session starts again from the configured percentage policy and cap. These chords are configurable under `[sidebar.keys]`. Filters use `A` for all and `s` for success/done.
