...
6. **Merged or closed.** Zero attention, so it gets out of the way entirely: a non-dirty merged or closed line skips every clock, archives immediately, and collapses to its header plus a dim roster receipt that names each hidden member beside its final status glyph, folds overflow and shells into `+n`, and pins tracked cost right when it rounds to a cent. A narrow or process-only receipt falls back to `+K done`; click the header or the line to reveal it, while a fresh run or ask revives it.
