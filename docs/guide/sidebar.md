...
A staged team with a board shows its progress directly below the group header: `●` passed stages, `◉` the current stage, `○` future stages, the stage name, and a run clock. The clock measures the run from the board's first recorded hand-off, restarting after `Done` and freezing when the run reaches `Done` again. Click the line to reach the stage owner. It is progress, not another request for attention. A folded finished group keeps the stage beside its team name instead. The [interface reference](../interface/sidebar.md#worktree-headers) covers missing clocks, narrow panes, and click fallbacks.
