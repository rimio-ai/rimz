...
The sidebar follows its attached view at the explicit `[theme.display].width_percent`, otherwise 30% on views wider than 240 columns or when pets are enabled, and 25% otherwise, capped at `[theme.display].max_cols`. Press `a` to make it narrower or `d` to make it wider; either key sets one room-wide share of the screen. A settled mouse drag does the same once it moves beyond half a native step. On Zellij, a smaller drag (at most 2.5% of the view) leaves the shared share untouched because it would snap back to the same 5% rung. Every tab follows a changed share within a tick. tmux preserves the selected column exactly when the share is set; Zellij snaps to the nearest 5% share rung, so a dragged width may settle within half a step while its siblings land on the shared rung. A narrower press is ignored when Zellij's next reachable width would fall below 24 columns, while tmux lands exactly on that floor. The pinned share keeps its proportion through terminal resizes, reloads, and reattaches for the life of the session and may exceed `max_cols`; a newly created session starts again from the configured percentage policy and cap. These chords are configurable under `[sidebar.keys]`. Filters use `A` for all and `s` for success/done.
