...
Cards arrive already triaged (the [ranking below](#how-the-column-is-ordered) decides the order), and a busy worktree caps at six rows with a `+K more` line that only hides idle agents and quiet shells while work remains active. A finished line of work collapses completely to its header and one dim roster receipt, including unread success cards: each hidden member keeps its status glyph while its name wears the provider's softened brand tone like a calm card, overflow and quiet shells fold into `+n`, and the pod's cost pins right when tracked spend rounds to at least one cent. Narrow or process-only receipts fall back to count-only `+K done`; click the header or the line, or apply the `s` status filter, to reveal the roster.
...
6. **Merged or closed.** Zero attention, so it gets out of the way entirely: a non-dirty merged or closed line skips every clock, archives immediately, and collapses to its header plus a dim roster receipt that names each hidden member in its provider's softened brand tone beside its final status glyph, folds overflow and shells into `+n`, and pins tracked cost right when it rounds to a cent. A narrow or process-only receipt falls back to `+K done`; click the header or the line to reveal it, while a fresh run or ask revives it.
