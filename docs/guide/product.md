...
The room outlives the host too. The ledger is a directory of flat files under `~/.local/state/rimz/`, and when the session must be reborn after a reboot or a multiplexer crash, Rimz re-seeds every prior agent idle in its worktree's `#channel` tab (`claude --resume`, `codex resume`, `pi --session`), so the fleet is one prompt away from where it stopped. `rimz reset` and `--no-resume` start a clean room instead.
