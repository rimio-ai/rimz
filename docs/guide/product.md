...
The room outlives the host too. The ledger is a directory of flat files under `~/.local/state/rimz/`, and when the session must be reborn after a reboot or a multiplexer crash, Rimz offers to recover prior agents idle in their worktree `#channel` tabs (`claude --resume`, `codex resume`, `pi --session`), so the fleet is one prompt away from where it stopped. The recovery prompt defaults yes, non-interactive starts recover, and `rimz reset` and `--no-resume` start a clean room instead.
