...
Targets read like Slack: `@<agent>` names who — `@swift-otter`, `@codex`, or `@all` to reach everyone — and `#<worktree>` names the channel, defaulting to the one you are in. The grammar and delivery gates live in [the agent-control reference](../reference/cli/agents.md#steer-live-agents).
