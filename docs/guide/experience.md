...
> A walkthrough from the chair of someone meeting Rimz for the first time. [product.md](./product.md) is the five-minute tour and [DESIGN.md](../../DESIGN.md) holds the invariants; this doc follows the felt experience from the first keystroke to a ten-agent fleet. The frames below are illustrative sketches of the moment; the exact, machine-checked rendering of every glyph, meter, and zone is the [interface reference](../interface/sidebar.md), and renderer mechanics live in [docs/internals/sidebar.md](../internals/sidebar.md).
