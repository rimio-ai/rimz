...
The `external` catch-all holds scripts, CI, and panes outside any worktree; it renders as a dim `┄ external ┄` divider and sorts last unless it holds something waiting or failed. The room scales past one repo too: `rimz start` in `~/code` makes each child repo a pod with the same cockpit, ranking, and jump triage ([the fleet room](./product.md#many-repos-one-room)).
