...
`rimz message` is the same command whether you type it or an agent runs it, so a running agent hands work to a teammate exactly as you do. Every attributed delivery starts with a `Type` / `From` / `Content` header: agent sends use `AGENT_MESSAGE` and their handle, launched-child completion reports use `SUBAGENT_REPORT` and the child's petname, while your sends use `USER_MESSAGE` and `@user`. Agent deliveries and subagent reports land as first-class lines in the receiving agent's transcript, so a channel's cross-talk reads as a conversation; user headers are removed when the prompt is recorded. `--no-from` delivers verbatim without a header when a script wants the raw text.
