...
`rimz doctor` reads the whole machine in one pass: the multiplexer it selected and whether the version clears the floor, the agent CLIs it found and the state of their hooks, the terminal's color depth, and the health of RimZ's own files. A normal run writes nothing to disk and starts, stops, or moves nothing in the room, so run it as often as you like. Every row it can print, and what to do about a `✗`, is in [troubleshooting](./troubleshooting.md#reading-the-report).
