...
- `snapshots/latest.json` is a derived view, published by writers after the workspace lock releases — serialized through `locks/publish.lock`, atomic rename, no fsync. Queued publishers group-commit: each holder folds to the log's current end, so the last writer always lands the newest state. It is stamped with the log generation and byte offset it reflects; a reader trusts it exactly when the stamp matches the live log, folds the missing tail itself when writes outran it (O(delta bytes)), and rebuilds from scratch on any mismatch or parse failure.
...
2. Sends a small wakeup datagram (`{ "kind": "ledger_delta", "request_id": "...", "workspace_id": "...", "protocol_version": "rimz.plugin.v3" }`) to each `sock/sidebar.<instance_id>.sock`.
