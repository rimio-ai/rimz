...
| PR state per repo | One `gh` or `tea` open-set call per due repo, plus a per-branch read only when a PR leaves the open set | `PR_STATE_HOT_TTL` and `PR_STATE_TTL`; failures back off and keep the last known map; terminal `merged` states are pinned |
