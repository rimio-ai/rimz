...
8. **One producer per workspace, one renderer per tab.** The fetch cost above is per *production*, so production must stay at one while every tab keeps its own renderer (the documented per-tab model — see [sidebar.md → Launch model](./sidebar.md#launch-model)). The eldest live instance is elected the producer (UUIDv7 ids sort by birth, so the lowest id wins; `elder_sidebar_present`); it forks `list-panes`/git and publishes the shared cache. Every younger renderer reads that cache read-only (`rimz sidebar snapshot --no-produce`) — it never forks its own `list-panes`/git and it never exits, so a per-tab pane never goes dark. This is what bounds the mux server to one round-trip per workspace: production, not renderer count, is capped, so even a stray duplicate that slips the launch lock costs one cache read, not a second `list-panes`. A launch lock still serializes concurrent attaches so they don't each spawn a daemon (`launch_sidebar_if_needed`), and the orphan sweep reaps a SIGKILLed instance. Trust the heartbeat TTL exactly as the launch gate does: a just-SIGKILLed elder is honoured for at most one TTL before the next-eldest renderer takes over production.
