...
7. **Open the panes.** Each cell compiles to a hidden `rimz agents exec` wrapper invocation carrying an `ExecRequest`. Provider-process compilation maps any typed system-prompt path to the adapter's argv or environment mechanism, then the wrapper runs the stock provider CLI in the pane with the trailing prompt in its launch argv ([The exec wrapper](#the-exec-wrapper)).
...
| [`launch.rs`](../../../crates/rimz/src/harness/launch.rs) | The provider process: adapter argv for launch, resume, and fork; typed system-prompt path rendering; the hidden `ExecRequest` wire; launch environment composition; the login-shell wrapper; and preflight. |
...
`launch::compile_agent_process` is the provider-process seam. It maps the typed system-prompt path to the adapter's replacement argv or environment, selects launch, resume, or fork argv from the request, composes trusted project, adapter, prompt-path, RimZ identity, and RTK environment in that order, applies the login-shell wrapper, and retains the raw provider argv for PATH preflight.

### The exec wrapper

Every agent pane runs the hidden `rimz agents exec <kind>` wrapper rather than the agent directly.

The command line carries two visible arguments, the kind and an optional `--worktree-path`, which together form the process-classification envelope that pane discovery reads. Everything else travels in one hidden compact JSON `ExecRequest`: the typed action (`Launch`, `Resume`, or `Fork`), the identity, the prompt, an optional system-prompt file path, the run id, pane-lifetime flags, provider account binding state, and provider argv. The wrapper decodes and validates that request, cross-checking the payload kind and worktree against the visible envelope, before it launches anything. Backends never resolve agent kinds or worktrees; the wrapper does.

When `system-prompt-file` is set, process compilation gives the user's resolved absolute path directly to the adapter: Claude and plugins use a path flag, Codex a config key, and Qwen `QWEN_SYSTEM_MD`. It removes a matching raw replacement argument first, so the typed setting wins on fresh launch and every relaunch path. RimZ checks that the path is a file before mux effects but never reads or copies its contents; the provider owns the final open.
