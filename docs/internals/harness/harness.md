...
`launch::compile_agent_process` is the provider-process seam. It selects launch, resume, or fork argv from one typed request after the exec wrapper has rendered prompt replacement, composes trusted project, adapter, generated prompt-path, RimZ identity, and RTK environment in that order, applies the login-shell wrapper, and retains the raw provider argv for PATH preflight.
