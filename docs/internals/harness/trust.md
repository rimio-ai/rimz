...
5. launch-plan env: the materialized system-prompt env, the account-home override, `RIMZ_ISOLATION` (`ENV_ISOLATION`: `sandbox` when the plan wraps the provider in bubblewrap, `host` otherwise), and `RIMZ_SCRATCH` (`ENV_SCRATCH`: the host path of the launch's scratch dir; the sandbox pin layer replaces it with `/tmp/scratchpad`)
