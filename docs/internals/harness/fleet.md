...
7. **Open the panes.** Each cell compiles to a hidden `rimz agents exec` wrapper invocation carrying an `ExecRequest`. The wrapper materializes any system-prompt composition, then runs the stock provider CLI in the pane with the trailing prompt in its launch argv ([The exec wrapper](#the-exec-wrapper)).
...
| [`launch_plan.rs`](../../../crates/rimz/src/harness/launch_plan.rs) | Shared exec-wrapper and explain plan: `compile` decides prompt, reminders, process argv/env, and sandbox view without writes; `apply` materializes runtime artifacts, room tmp, and skill copies. |
| [`launch_reminders.rs`](../../../crates/rimz/src/harness/launch_reminders.rs) | One system-reminder block combining team context, the model line, and delegation guidance. |
...
1. **Resolve.** `plan::resolve_launch` reads the effective config (machine profiles and teams, merged with trust-filtered project config) and produces a `LayoutSpec` whose agent cells carry their profile-declared launch params and typed system-prompt file path. A launch-wide `--agent` override resolves as a profile-or-kind replacement base without changing cell profile or role identity. `spec::rebase_onto` takes the engine from the replacement base (kind, plus model and effort whenever the base sets them) and the role from the original chain: mode, budget, auto-compact, skills, and prompt files win over the base, which only fills their gaps. When the base leaves effort unset, the original's effort is kept on any re-base; an unset base model keeps the original's model only on a same-provider re-base. On a provider change, provider-specific model and raw args silently come only from the replacement base because the replacement is explicit.
...
3. **Finalize.** `plan::finalize_launch_layout` applies the launch-wide choices: permission posture, CLI model and effort, the typed prompt-file override, passthrough argv, budget, adapter-declared preset reconciliation and defaults, and supervised turn limits. Profile-declared model, effort, and `auto-compact` render through typed presets and reconcile against matching raw `args`; `auto-compact` has no CLI override. An args-only model is adopted as identity; an adapter default is stamped only when no model was selected at all.
4. **Resolve ancestry.** The launcher reads its stable launch identity and resolves the live durable caller. An ordinary agents or teams launch gets a parentless peer-generation stamp and checks `[agents] max-chain-length`; a subagents launch gets a direct-parent stamp and rejects a subagent caller. A stale caller, excessive chain length, or subagent caller refuses here, before provider preflight, worktree creation, store append, or mux action. A subagents launch then re-anchors its workspace at the caller's recorded checkout, so the child works where its parent works rather than where the launch command ran; a vanished checkout refuses at the same point ([subagents.md](./subagents.md#the-child-works-where-its-parent-works)).
5. **Identify.** Each cell becomes a launch request with a name, a channel, its cohort stamps, and its optional peer-generation or parent stamp, and the store mints provisional rows before any pane opens.
...
The command line carries two visible arguments, the kind and an optional `--worktree-path`, which together form the process-classification envelope that pane discovery reads. Everything else travels in one hidden compact JSON envelope: the typed action (`Launch`, `Resume`, or `Fork`), the identity, the launch prompt's path, typed system-prompt source paths, the run id, pane-lifetime flags, provider account binding state, and provider argv. Every launch prompt is materialized as a content-addressed `prompt/task.<digest>.md` beneath the private runtime root, beside the system-prompt artifacts. The wrapper validates the request and cross-checks the visible kind/worktree before any artifact I/O, then establishes launch/run cleanup context and reads the prompt into the in-memory `ExecRequest`. A missing or unreadable artifact fails the provisional launch and any nonterminal supervised run, waking its waiter rather than dropping the prompt or leaving the parent waiting. Backends never resolve agent kinds or worktrees; the wrapper does.

Keeping the prompt body out of the pane command avoids tmux's approximately 16 KiB command-frame ceiling. The provider still receives the full prompt as one argv element, so RimZ enforces a separate 120 KiB (122880-byte) argv safety limit during launch preflight, before launch and supervised-run records are created, and again in the wrapper. Larger prompts fail with the limit named and advice to move detail into a file the agent reads. The runtime artifacts are rename-atomic caches, not durable records; content addressing makes reentry idempotent, and runtime GC removes stale artifacts by age.

The wrapper delegates decisions to `launch_plan::compile`, which takes the request, paths, effective configuration, preflighted bubblewrap path, and ambient environment. It reads prompt sources and skill metadata, builds reminders and provider argv/env, and plans the sandbox view without writing files or store records. `LaunchPlan` retains the request, cwd, prompt plan, process stage, reminder channel, optional `SandboxPlan`, and warnings; the rendered reminder is retained on `plan.process().reminder`, even when no provider channel can deliver it. `launch_plan::apply` is the write boundary: it ensures runtime directories and writes composed system-prompt and reentry prompt artifacts; when a sandbox plan is present, it also ensures the room tmp layout and applies planned skill copies. Cwd changes, identity writes, pane operations, and provider execution remain outside compilation. The task-envelope planner stays private to `launch`; its deferred artifact writer is harness-only. These module dependencies and public surfaces are admitted explicitly in `refactor-target.toml`.

[`rimz agents explain`](../../reference/cli/agents.md#explain-a-launch) renders this same compiled plan and never calls `apply`. Its profile branch shares launch resolution/finalization and `launch_identity_requests` but leaves the name and launch id unminted; its `@handle` branch uses restart action selection and current-config `resolve_posture`, reporting degradation as warnings and refusing overrides. Both require successful sandbox and skill preflight. The environment supplied for inspection is the invoking process's, not the target pane's. This is launch inspection, not a reconstruction of hidden provider instructions or historical one-off flags.
...
The wrapper also collects what RimZ tells the agent about itself, as a `LaunchReminders` value. For a non-child request it derives the profile's allowed subagent catalog from effective project and machine configuration; a child request carries the no-delegation reminder instead. For a team member it resolves the effective team definition, which supplies the worktree, role and team handles, channel, leader and teammates, session posture, and declared scratch entries present at launch with their line counts. For every request it reads `model-reminder` from the launched profile itself, looked up by the exact `LaunchParams.profile` name in `[subagents.profiles]` for a child and `[agents.profiles]` otherwise, with no walk up the `agent = …` chain; unset, absent from config, or a launch with no profile all mean on. Because the lookup is by name, restart, fork, resume, and rebirth reproduce the same answer from the rebuilt request. A failed effective-config load warns and falls back to default reminders, which keeps the model line because it needs no configuration. Independently of that lookup, the wrapper sets `sandbox` from its successful bubblewrap preflight, so the sandbox paragraph also survives a config-load fallback.

[`launch_reminders.rs`](../../../crates/rimz/src/harness/launch_reminders.rs) then renders those parts into one `<system_reminder>` tag, and the process compiler merges that single text through the adapter's native append-system-text channel. Paragraphs are blank-line separated in a fixed order: the team member's identity and run state, the seat's channel rule, the sandbox view, then the subagent catalog or the child's no-delegation body. The team paragraph (`launch_context.rs`) is one sentence of identity (`You are @planner, leader of team `forge`, with teammates @coder and @reviewer, on Fable.` or `You are @coder in team `forge` on #feature, led by @planner, …`), one of session posture and worktree, then what the declared memory files and the board held at launch, with the declared patterns shown without their leading `/`; a fresh worktree collapses to one sentence naming the first `rimz teams flip`, and the flip syntax itself stays with the team's prompt. The channel rule follows only for a non-leader seat on a team that declares its leader: no user reads its pane, and user-bound questions go to the leader. The leader's own channel rule stays in its prompt. The [sandbox paragraph](../sandbox.md#launch-reminder) describes shared room tmp, its scratchpad, and its room lifetime; host-mode launches omit it. These paragraphs reach Claude, Qwen, Droid, and Codex; other providers' child user-prompt fallback stays no-delegation only. The model fragment is `on <model>` with `agents::model_display::display_model` for the model, and effort is left out; it rides inside the team identity sentence, and a launch with no team paragraph gets it as its own line opening with the card handle, role first and profile otherwise (`You are @planner, running on Opus 4.8.`, or `You are running on …` when the launch carries neither). A launch that knows no model has no fragment instead of guessing the provider default. When nothing applies, no tag is appended at all.

The wrapper then runs the agent in the pane, inheriting the pane's TTY. It launches through the user's shell-startup path when that shell and `/usr/bin/env` are available and falls back to direct exec otherwise.
...
A resumed pane runs `rimz agents exec <kind>` with a `Resume` action carrying the provider session id and the prior RimZ identity (name, profile, role, team, launch group, launch ordinal, channel). Its **posture** comes from `resume::resolve_posture`, the one pure seam every relaunch path shares: model, effort, and `auto-compact` argv, typed system-prompt source paths, permission mode, budget, and profile `args`. The values it replays are the planner's `ResumeLaunchPosture`, the same projection a resumed team cell uses; `ResumePosture` adds only the degradation reason. The native auto-compaction window replays through the current profile's rendered argv, not a separate environment variable or durable launch field. A session that launched as `@planner` comes back as a planner.

Once the provider process stage is ready, the resident exec wrapper appends `agent.attached` before it starts or execs the provider. That record binds the exact resumed session to the stable launch id exported to its process, the wrapper's pane, and the runtime owner of the agent process the row belongs to. When the wrapper execs the provider in place, it is that process, and the one record is complete. When it spawns the provider instead (a worktree, close-on-exit, or supervised launch), the pre-spawn record carries the wrapper's own owner, so identity exists before the provider can call RimZ, and a second `agent.attached` re-stamps the spawned provider's pid immediately after the spawn. The provider-owned stamp is what proves a later same-pane conversation belongs to the same instance ([model.md](../agents/instances.md#launch-identity-across-conversations)). A pre-launch-id row uses its provider session id for the new stable stamp, and a provider-store discovery gets an idle durable seed, so either can launch a child or be addressed before a lazily-registering provider emits its first hook.
