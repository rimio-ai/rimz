...
[`launch_reminders.rs`](../../../crates/rimz/src/harness/launch_reminders.rs) then renders those parts into one `<system_reminder>` tag, and the process compiler merges that single text through the adapter's native append-system-text channel. Paragraphs are blank-line separated in a fixed order: team launch context, the model and effort line, the sandbox view, then the subagent catalog or the child's no-delegation body. The [sandbox paragraph](../sandbox.md#launch-reminder) describes shared room tmp, its scratchpad and output directories, and its room lifetime; host-mode launches omit it. These paragraphs reach Claude, Qwen, Droid, and Codex; other providers' child user-prompt fallback stays no-delegation only. The model line opens with the card handle, role first and profile otherwise (`You are @planner, running on Opus 4.8 at high effort.`); it drops the handle behind a team paragraph that has already introduced the member (`You run on Opus 4.8 at high effort.`) and reads `You are running on …` when the launch carries neither role nor profile. The model text is `agents::model_display::display_model`, the effort is the configured string, and both are escaped like the team fields. Each missing field drops out of the sentence, and a launch that knows neither model nor effort omits the paragraph instead of guessing the provider default. When nothing applies, no tag is appended at all.
