...
1. **Resolve.** `plan::resolve_launch` reads the effective config (machine profiles and teams, merged with trust-filtered project config) and produces a `LayoutSpec` whose agent cells carry their profile-declared launch params and typed system-prompt file path. A launch-wide `--agent` override resolves as a profile-or-kind replacement base without changing cell profile or role identity. Portable fields from the original chain win over the replacement base; on a provider change, provider-specific model and raw args silently come only from the replacement base because the replacement is explicit.
