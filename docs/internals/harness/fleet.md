...
| 2 | Launch cwd, the user shell (`$SHELL`, else passwd), and the git transcript | env reminder enabled; the git transcript only when all three reads succeed |
...
The environment input comes from `launch_plan::compile` through `launch_env::read`, using the provider cwd rather than the compiler's current directory. The shell comes from `proc::user_shell()` at compile time; `None` omits the shell clause, and `explain` names the caller's shell. This does not promise which shell the provider's tools use. `[agents] env-reminder` defaults to true; a trusted project's value overrides the machine value, while an unavailable effective config leaves it on. Every exec, including children, resume, fork, restart, and rebirth, probes afresh. Each git command (`git status --short`, `git rev-parse --short HEAD`, `git log -1 --oneline`) has a five-second deadline, `GIT_OPTIONAL_LOCKS=0`, `LC_ALL=C`, and color disabled; compile and explain do not refresh the index. Missing git or cwd, a non-repository, unborn HEAD, nonzero exit, or timeout omits only the transcript with a debug diagnostic, not the environment line. The transcript uses `(clean)` for empty status and at most 40 status lines plus `… N more`; each line, the cwd, and the shell path are escaped independently to preserve the one-tag boundary.
