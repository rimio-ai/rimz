...
The default has the user-visible behavior of `rimz agents <profile> <prompt> -p --bg --timeout 30m`, plus a fleet digest: after every launched child in the fleet settles, the last settler's wrapper queues one status-only message, stamps `ended_at`, and closes its pane. `--wait` leaves the background run unchanged and passes the minted petname to the shared single-name wait path; `--wait=DURATION` adds a caller-side join deadline without changing the child's timeout. A join reads results from durable run records; when every row in one queued digest has printed inline, the joiner cancels that digest.
...
3. The in-pane wrapper notices the terminal record and terminates the provider. After its child-exit fallback guarantees a terminal run, [`subagent_report`](../../../crates/rimz/src/cli/agents_cmd/subagent_report.rs) reads the parent and every launched child's newest run from the audit projection. If any newest run is non-terminal, this wrapper queues nothing.
4. The last settler collects the terminal child rows that have neither `report_message_id` nor `joined_at`. When the parent still exists and that set is non-empty, it composes one registration-ordered, status-only digest and queues a parked `MessageSender::Harness { notice: SubagentReport }` record with gate `Done`. The `SUBAGENT_REPORT` envelope uses `From: @rimz`; immediate pane delivery is best-effort latency over the durable run records.
5. After queueing, the wrapper stamps that digest's message id onto every row it listed, then stamps its own child row's `ended_at` and closes its pane. With `--keep`, it instead transfers runtime ownership back to itself and remains alive to hold the pane.
...
The fleet includes every child launched before digest composition: a child launched while siblings run joins that fleet, while one launched after the digest starts belongs to the next. Two last settlers can race, each queueing a digest before either stamps its rows; both digests are truthful and neither is lost. A missing or ended parent suppresses the digest without changing the runs.

Inline joins and wrapper digests use a two-field handshake on the run record. The joiner stamps `joined_at` at the point it prints a terminal run, which excludes that row from a digest that has not yet been composed; the wrapper stores the queued digest in `report_message_id` on every row it lists. Both mutations take the run lock. After each join, the joiner loads every run carrying that message id and cancels the queued digest only when all of them have `joined_at`; an unjoined row keeps the notice intact, and a digest already sent cannot be recalled.
