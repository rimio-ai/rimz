...
A **launched subagent** is a full RimZ agent: its own pane, provider process, durable run record, petname, launch profile, transcript, cost, and address. `rimz subagents` creates it. The petname is always minted and remains its address; the sidebar card labels it by launch profile. Its separate transcript spend folds into the parent's live and lifetime figures in the sidebar, `agents show`, teams, and attribution, just as provider-native child spend already inside the parent transcript does. Attribution's all-in member and group figures include both mechanisms, while its subagent breakdown groups their spend by task as `{count} × {task}, … · {cost}`. This page owns the launched form.
