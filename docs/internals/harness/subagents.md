...
For an identified agent caller, a printed result counts as consumed only while it holds an open provider turn. The [wait print path](../../../crates/rimz/src/cli/agents_cmd/wait.rs) resolves the caller against `Store::snapshot_cached()` and asks [`AgentState::holds_open_turn`](../../../crates/rimz/src/agents/state.rs). The cached snapshot attaches provider rest certificates, so a turn settled without a `Stop` hook still reads as closed; native waits and proposed plans keep the turn open. An unresolved caller is treated as a human shell and counts as attended; a failed snapshot read logs a warning and preserves that joining behavior. Attendance is re-read at each print: a background wait that outlives its parent's turn prints but leaves its rows unjoined for the fleet digest to wake the parent. `stop` skips this check because dismissal is deliberate.
