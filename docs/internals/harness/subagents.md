...
Both settlement paths above are subagent-specific, and neither is the signal bus. A child's lifecycle transitions do also fire `agent.*` signals ([loops.md](./loops.md#the-signal-vocabulary)) carrying `session`, `handle`, and, when the observation records one, the parent's session id in `parent`, so a parent could arm `rimz wake --signal agent.ended --match session=<child>` instead of waiting. Prefer the digest and `wait`: they carry the child's text and its run identity, while a signal carries only the transition. The self-wake guard also refuses a subscription that does not name another agent, so `--match parent=<own session>` alone is rejected.
