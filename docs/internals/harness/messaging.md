...
`rimz agents compact @handle [INSTRUCTION]` resolves one agent with a bound pane and a native compaction command, renders the configured brief or the positional override, and calls `message::compact`. An explicit instruction is refused when the adapter does not accept trailing text; without one, those adapters get the bare command. The verb refuses adapters without native turn-start hooks (Kiro), since the strict no-repeat guard cannot work for them.
...
- **Reply.** Legs read `effective_status()` with the loop catalog's pending waits attached, and settle through `AgentState::turn_completion`: `Idle` or `Success` completes the leg only once a turn has opened (`turn_started_at` set). `Failed`, a delivery failure, a vanished card, or a skipped waiting input fails it while the other legs keep gathering. `Waiting` and `Paused` stay inside the reply. `Sleeping` stays inside it too and clears the turn anchor, so the turn its wake opens continues the reply instead of ending it. A changed `turn_started_at` while the card is still `Running` proves the reply turn ended and another began between polls.
