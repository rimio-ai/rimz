...
`message::compact::refuse_repeat` checks three states: `is_compacting(now)`, a non-terminal `MessageBody::Command` in the same card's live queue, and `AgentState::compacted_awaiting_prompt`. The marker is a durable timestamp, set by a command reaching `Sent` even without a context reading, or by a successful `CompactionEnded { auto: Some(false), failed: false }`. It survives rollup rotation and linked successor adoption, and clears on `TurnStarted` or `Registered`; automatic or unknown-trigger compaction closes do not set it. A later `Delivered` acknowledgement cannot restore a marker already cleared by its prompt hook. This replaces the idle helper's delivered-message history scan and includes manual compaction reported from the pane: a compaction never follows a compaction; the next user turn re-arms it.
