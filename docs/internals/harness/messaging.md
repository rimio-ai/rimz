...
Idle compaction sends a compact command to a resting team member, with no prompt behind it. [`harness/idle_compact.rs`](../../../crates/rimz/src/harness/idle_compact.rs) owns the shared producer/helper decision: `resolve_mode` chooses role `idle-compact` over `[harness] idle_compact`, `fire_point` resolves the idle threshold, and `should_compact` checks eligibility. The seat must have `AgentState.team`, not be a provider subagent, have a nonempty session id, and be effectively `Idle`, `Success`, or `Sleeping`, without an input request, budget park, or compaction marker. Occupied context must reach `IDLE_COMPACT_MIN_TOKENS` (50,000) and differ from `last_compact_command_tokens`; the adapter must expose a compact command. `cohort_live` reads `blackboard.md` through `scratch::board_stage` at the seat's worktree, not the pipeline projection: only `Done` refuses, so a missing board or worktree path counts live. PR state and teammate activity play no part.

The default `On` asks `ContextCapability::prompt_cache_ttl` using `AgentState.model` and the published account cache's `login_key()` lookup, without probing accounts. The adapter implementations own the [provider table](../../guide/configuration.md#idle-compaction): Claude requires `AgentAccount.metered == Some(true)`; Codex alias-expands the model and parses `gpt-<major>[.<minor>]` at version 5.6 or newer, independently of account. A missing or unsuccessful account-cache record supplies no account. Unknown TTLs and TTLs no greater than `PROMPT_CACHE_MARGIN` skip silently. `After(duration)` bypasses the TTL lookup; `Off` skips. Effective teams load lazily once an eligible seat exists, falling back to machine teams if effective loading fails. Absent Markdown role values stay `None`, so a machine opt-out reaches them.

`fire_point` subtracts the three-minute margin from the TTL. The idle clock uses `last_activity`, normally stamped by the final lifecycle event after the provider's final request, while the cache clock starts with that request. Final generation therefore starts the idle clock late. `cache_timing_reaches_the_request_before_expiry` tests the consumer decision exactly at and one second before the threshold, assuming a 1-second producer tick, 10-second helper spawn, 10-second keystroke-to-request delay, and 120-second final generation. Those 141 seconds fit inside the margin; they are runtime premises, not measured latency guarantees.

The elected producer requires a bound pane and spawns detached `rimz agents idle-compact`, keeping store writes out of the sidebar's import graph. The request carries `agents::compact_command` for the team seat, including the team brief where supported. The helper's `Ctx::resolution_snapshot_with_context` folds agent context, but not the activity sidecar that `sidebar::enrich` folds. Its `last_activity` can therefore be older than the producer's and its calculated idle span larger; the shared predicate does not make this snapshot a fresher activity check.

The helper re-resolves the workspace, session, and pane, applies the shared decision to its snapshot, and validates the command against the adapter and its configured instruction. Through `message::compact` it queues one automated `System` command with the `Done` gate, pins the pane, stamps `compacted_context_tokens` from its context reading, attempts boundary delivery, and appends an `IdleCompact` assist record. The optional `idle_after_secs` records the resolved threshold; `rimz stats` shows it when present and still accepts older records without it. A closed boundary leaves the command queued through the ordinary retry path.

Three layers make it once per idle stretch:

1. The producer's cache-class `(kind, agent_id)` record suppresses the same `last_activity` and throttles helper respawns for ten minutes. This pacing is producer-only: the helper must not veto its own spawn record.
