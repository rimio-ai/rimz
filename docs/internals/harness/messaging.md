...
| [`store/message.rs`](../../../crates/rimz/src/store/message.rs) | The durable record schema and vocabulary, card matching, FIFO/claim/batch selection, and the prompt alignment [confirmation](#confirmation-and-retry) runs against those records. |
...
4. **Write.** Send through the same pane-write path as `--steer`. If smart compaction fires, raw-type the rendered compact command alone and release the prompt batch back to `Queued` with no attempt penalty.
...
A `SUBAGENT_REPORT` digest points a parent at the response files of children that have settled. A parent that already joined those children inline has read the results, so the digest costs it a turn and shows it nothing new. The producer cancels the digest at that point: an attended inline `rimz subagents wait` (the caller still holds an open turn) and a `rimz subagents stop` stamp `joined_at` and then cancel a digest whose every listed row is joined; the composer also rechecks for fully joined rows after queueing ([subagents.md § The lifecycle, end to end](./subagents.md#the-lifecycle-end-to-end)). Those cancels are eager cleanup. Delivery does not depend on them, because any of them can fail after the run record is stamped and before the queue changes.

[`attempt_delivery`](../../../crates/rimz/src/message/deliver.rs) calls `cancel_joined_subagent_report` before it claims and repeats the call after a successful claim, before sending. When the record's sender is `Harness { notice: SubagentReport }` each call scans the durable run records whose `report_message_id` is this digest's id:

> **Each scan must suppress delivery when it finds linked run rows and every one is joined.**

Fully joined means at least one linked row and `joined_at` stamped on every one of them, through the same `harness::run::report::digest_fully_joined` scan the producer uses. If either scan finds a fully joined digest, the guard cancels it with reason `joined before delivery` and suppresses the pane write. Suppression also holds when producer cleanup has already removed the live message: `cancel_joined_subagent_report` returns true once the scan finds fully joined rows and the cancel call succeeds, even when `cancel_message` returns false because there is nothing left to cancel. A digest whose rows are partly joined delivers with its full text, and so does one with no linked rows at all, which is what keeps a genuinely unread result on its way to the parent. Only `SubagentReport` takes this path; any other harness notice gets ordinary delivery.
...
Large pastes do not require an unbounded mux command argument. tmux's roughly 16 KiB command frame cannot carry a full message body: RimZ loads a uniquely named buffer containing the opening marker, CR-normalized payload, and closing marker through stdin, then pastes it once with `paste-buffer -d -r` in the same invocation. Markers and body therefore take the same PTY delivery route even in copy mode. On tmux 3.7 and newer, `-S` disables the newly introduced control-byte sanitization; older supported releases already paste raw. Explicit markers work even when the application has not enabled bracketed-paste mode; raw pasting preserves the complete byte stream, and `-d` deletes the buffer after use. A failed batch best-effort deletes its named buffer before returning the original error. Zellij writes the same bracketed byte stream in ordered chunks of at most 8 KiB, stopping body writes at the first error and best-effort writing the closing marker before returning the original error; the per-pane lock spans every chunk and cleanup attempt. Cleanup does not retract any delivered fragment or guarantee closure if the backend remains unavailable. Raw command typing and public `pane send` remain unchanged.
...
Inside the paste, logical LF and CRLF line endings travel as CR. Agent composers normalize CR back to a newline but drop a bare LF; terminal emulators and tmux's own `paste-buffer` likewise send CR for pasted line endings. Multi-line prompts therefore land multi-line. A `Command` body takes the raw type path instead, because a composer treats pasted text as literal content and a pasted `/compact` would land as a prompt rather than run. When the adapter renderer appends arguments to its declared compact command, that complete declared command and its trailing space are written first, then its arguments as a second raw write one command-submit delay later. A composer can classify one long chunk as pasted text (Claude does so past 800 characters); keeping the slash literal lets it dispatch while the following paste-classified chunk becomes trailing command text. Bare commands, including multi-word plugin commands, remain one write. The raw path stays byte-faithful and has no paste close marker: composers can also reassemble fast typed chars into a synthetic paste, and Codex buffers chars less than 8 ms apart and suppresses Enter for 120 ms after the burst. RimZ therefore waits the same command-submit delay (1 s, `RIMZ_MESSAGE_COMMAND_SUBMIT_DELAY_MS`) between raw-typed command segments and before the Enter keystroke. The public `rimz pane send` also stays on the byte-faithful raw type path, since a bare shell would render the markers literally.
...
[`write_batch`](../../../crates/rimz/src/message/send.rs) records the batch as `Sent` after all of its text lands and **before** it presses Enter. A submitted message is therefore always preceded by its durable record and audit event. The failure ordering this buys: a crash between text and submit leaves a `Sent` record whose text sits unsubmitted in the composer, which the reconciler can reason about. The reverse ordering would let an agent start a turn RimZ has no record of.
...
`Type` is `AGENT_MESSAGE` for a send from an identified agent caller, `SUBAGENT_REPORT` for the status-only fleet digest sent after all of an agent's launched children settle, `WAKE` for timer, command, or clock deliveries, `SIGNAL` for every delivery fired by a `Trigger::Signal` row and direct `team.stage` stage-open deliveries from a flip or registration re-wake, and `USER_MESSAGE` for a human's `rimz message`; a human header always uses `From: @user`, while the fleet digest, wake, and signal all use `From: @rimz`. A harness notice this binary does not know, minted by a newer one, decodes as `HarnessNotice::Other` with its string intact, renders that name upper-cased as its `Type`, and takes ordinary harness delivery ([store.md § What is in it](../store.md#what-is-in-it) covers why the string survives). An agent handle gains `#channel` when the delivery crosses lanes. The recipient's lane comes from its registered channel, its live pane channel, or the addressed channel, so a just-launched same-lane teammate does not gain a spurious suffix before pane capture lands.
...
The helper re-resolves the workspace, session, and pane, verifies that the agent is still idle and the configured threshold is still due, and validates the command against the adapter and its current configured instruction. Through `message::compact`, it queues one automated system `MessageBody::Command` with `DeliveryGate::Done`, pins the pane, stamps `compacted_context_tokens` from the fresh context reading, and attempts `DeliveryPolicy::Boundary`. A closed boundary stays queued through the ordinary retry disposition rather than typing into a working, waiting, parked, or compacting pane.

Three layers make the action once-per-idle-stretch. The producer's cache-class `(kind, agent_id)` record suppresses the same `last_activity` and throttles helper respawns; the live queue prevents an outstanding command from being duplicated and `last_compact_command_tokens` suppresses the same occupied reading; and, for adapters with native turn-start hooks, the durable `compacted_awaiting_prompt` marker stops the helper until the next user turn opens. That turn may come from a queued prompt or text typed directly into the pane; it begins a new eligible stretch once its own activity and context thresholds are reached. Adapters without durable turn starts retain the context-baseline guard without consulting the marker.

## Manual compaction

`rimz agents compact @handle [INSTRUCTION]` uses the same command record without a following prompt. The CLI resolves one agent with a bound pane and a native compaction command, renders the configured brief or positional override, and calls `message::compact`, the shared entry for standalone operator and idle compaction. Explicit instructions are refused when the adapter does not accept trailing text; an omitted instruction keeps the configured convention of sending those adapters a bare command.

`message::compact::refuse_repeat` checks three states: `is_compacting(now)`, a non-terminal `MessageBody::Command` in the same card's live queue, and the capability-aware `AgentState::compaction_unprompted` predicate. The marker is a durable timestamp, set by a command reaching `Sent` even without a context reading, or by a successful `CompactionEnded { auto: Some(false), failed: false }`. It survives rollup rotation, linked successor adoption and re-registration, and clears only on `TurnStarted`; automatic or unknown-trigger compaction closes do not set it. A later `Delivered` acknowledgement cannot restore a marker already cleared by its prompt hook. Pane-native manual compaction sets the marker only when its adapter reports the manual trigger (Claude, Pi and Qwen today); a native Codex or OpenCode compaction with an unknown trigger cannot set it independently.

The operator verb refuses adapters without native turn-start hooks (Kiro today): their pulled lifecycle observations cannot durably re-arm the strict no-repeat guard. The reducer still records their sent-command marker, but smart and idle compaction ignore it through the same capability-aware predicate and retain their context-baseline behaviour.

Delivery rechecks the durable marker after claiming a command and before writing its pane. If a compaction happened while the command was parked, the obsolete request settles as `errored` with the no-repeat reason instead of delivering another compaction. Smart compaction also checks the marker, even when the context gauge has changed.

`message::compact::send_compact` queues a pane-pinned `DeliveryGate::Done` command and immediately attempts `DeliveryPolicy::Boundary`. A closed gate records a retry disposition so the ordinary lifecycle checkpoint or elder sweep owns later delivery; commands retain the at-most-once raw-typing path. When occupied context is known, `compacted_context_tokens` also stamps the smart/idle baseline guard. The operator record carries the human or calling-agent sender and `automated: false`, leaving a durable message and audit event but no `AssistRecord`; the idle helper still appends its `IdleCompact` assist.
