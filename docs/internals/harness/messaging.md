...
`fire_point` returns an `IdleWindow`: `On` fires from TTL minus the three-minute margin until, but not including, the TTL; `After(duration)` has no upper bound. The idle clock uses durable `turn_ended_at`, stamped by a non-ignored turn end, interruption, or successful compaction close. Registration on resume or restart and runtime heartbeats (including child-stop activity) carry it unchanged. A member found past its cache lifetime is left alone rather than compacted cold. Missing anchors never qualify, including old rows until their next completed request and local-observation adapters (Antigravity, Cursor, Kiro), even under `After(duration)`.

The cache clock starts with the provider's request, while its completion starts the idle clock, so final generation starts the idle clock late. `cache_timing_reaches_the_request_before_expiry` tests the consumer decision exactly at and one second before both the fire point and the TTL, assuming a 1-second producer tick, 10-second helper spawn, 10-second keystroke-to-request delay, and 120-second final generation. Those 141 seconds fit inside the margin; they are runtime premises, not measured latency guarantees.
...
1. The producer's cache-class `(kind, agent_id)` record suppresses the same `turn_ended_at` and throttles helper respawns for ten minutes. This pacing is producer-only: the helper must not veto its own spawn record.
