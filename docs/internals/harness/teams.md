...
6. **Compact the flipper.** On a hand-off, when the role's `flip_compact` threshold (falling back to `[harness] flip_compact`) is reached, the flipper's pane gets its compaction command. This is best-effort enrichment recorded in the assist log; it never fails the flip.
