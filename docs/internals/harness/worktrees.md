...
The shared `harness::scratch::scan` resolves the declared patterns against the checkout root for both the member launch reminder and `rimz teams show`. It strips leading `/` when rooting globs, matches files only, and sorts and deduplicates paths across patterns. An unreadable file has a zero line count; invalid globs and failed traversal set the scan's probe-failure flag. It also records modification times when metadata is available. The reminder presents relative paths and line counts as a launch-time snapshot; `teams show` scans again for absolute paths, line counts, and modification ages. Its human report displays paths relative to the worktree, while JSON retains absolute paths. The team agents maintain these files' freeform content. Within RimZ, only `harness::team_stage` writes the board: `rimz teams flip` creates a missing board, replaces its first `Stage:` line or inserts one after a leading H1 (otherwise at the top), and appends the required note to `## Progress` under a per-worktree lock with atomic publish. Existing `## Progress log` sections remain accepted. A member hand-off (leaving a stage its role owns for one it does not) first requires `git status --porcelain=v1 --untracked-files=all` over the worktree, excluding the root `blackboard.md`, to be empty and is refused otherwise, before any board write; other memory files count unless `scratch-files` excludes them. The first ledger entry says `opened <stage>`; registration re-wakes read the board without editing it.

The adjacent `harness::scratch::board_stage` reads the first `Stage:` line in `<worktree>/blackboard.md`, independently of `scratch-files`. Only a terminal ` (@owner)` suffix is split from the stage name, with the owner stored without `@`; other parenthesized text stays intact. A missing or unreadable board yields no stage. The declared `stages` array supplies pipeline order, with implicit `Done` rendered last, while board text supplies advisory progress; neither is derived from member status or used as lifecycle authority. Cohort PR/CI reporting separately reads the published sidebar projection, never probes the forge, and absence of projected facts is not verified absence of a PR.
