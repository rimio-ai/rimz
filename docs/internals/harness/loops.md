...
Flip compaction is labelled `flip compaction` in the assist event stream and contributes sent commands to the compact rollup. A queued command is not counted as sent merely because the flip completed. The effective harness or role threshold is recorded with each attempt, including errors and missing-pane skips; these enrichment failures do not fail the flip. Only a member leaving its own stage for one it does not own is eligible, including `Done`; below-threshold flips make no attempt.
