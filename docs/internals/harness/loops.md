...
A member the reaper stops (a pane closed with no lifecycle hook, [`store/writer/reap.rs`](../../../crates/rimz/src/store/writer/reap.rs)) passes through no hook, so it derives neither `agent.ended` nor `team.ended`. Its subscriptions are still retired, by the reconcile in [What persists](#what-persists) rather than by a signal: cleanup follows the durable end, and never broadens what an end emits.
