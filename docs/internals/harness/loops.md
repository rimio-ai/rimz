...
| [`schedule/signal.rs`](../../../crates/rimz/src/harness/schedule/signal.rs) | The signal vocabulary: `SignalName` grammar, `SignalSelector`, the reserved families, `Signal` and `SignalSource`, `WatchOutcome`, the lifecycle-to-signal mapping, in-process `fire_signal`, and `run_watcher`, the whole watched-command lifetime from the lock to the fire. |
