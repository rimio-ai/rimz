...
Stage-open delivery goes directly through message dispatch as `Type: STAGE` / `From: @rimz`, always at the done boundary, with a prose body and a `Note:` line for flips, no JSON; no implicit subscription is armed for the owner. Explicit `team.stage` subscribers still fire independently with the loop's signal body, including on re-wakes. Registration prose names a continuation since the last Progress line and carries no note. `Done` and a self-owned flip send no direct message; an owner who is not live is woken on registration.
