...
In both scheduled and manual fires, the check receives `RIMZ_LOOP_TASK=<task-name>`, `RIMZ_WORKTREE_PATH=<dir>` when `dir` is set, and the workspace pin (`RIMZ_WORKSPACE_ID` and `RIMZ_PROJECT_ROOT`) from `workspace::pin_env`, binding nested RimZ commands to the task's root. A prompted `rimz agents <cell> "<prompt>"` launched from that check is converted to a loop-owned supervised run: `-p` is implied, the command blocks for its result, its final message goes to the check's captured stdout, and its exit code reflects the run status. It uses the loop zone, inherits `loop.default-timeout` unless the launch sets its own timeout, and enables wrapper self-cleanup unless `--keep` is set. An unprompted launch is refused because there is no turn to complete; `--bg` is refused because the check must await the result. This conversion does not apply inside an agent pane carrying `RIMZ_AGENT_ID`, where agent ancestry owns child launches instead. Room birth clears the check marker so it cannot leak into later human shell launches.
