...
`rimz loop` fires agent work on a trigger: a fresh supervised turn, a prompt delivered to an agent that is already running, or a shell command that guards either one. The trigger is a clock, a signal selector, or a watched command; `rimz wait` is the agent-facing front end over the same rows.
...
| [`schedule/signal.rs`](../../../crates/rimz/src/harness/schedule/signal.rs) | The runtime signal vocabulary: `Signal`, `SignalSelector`, `WatchVerdict` and the `WatchOutcome` that carries it, the lifecycle-to-signal mapping, the `From<&Signal>` conversion into the durable payload, in-process `fire_signal`, `wait_output_path`, and `run_watcher`, the whole watched-command lifetime from the lock to the fire. |
...
| [`cli/wait/`](../../../crates/rimz/src/cli/wait) | Self-only trigger parsing, caller resolution, receipts, and pending list/cancel presentation. No scheduling policy. |
| [`schedule/arm.rs`](../../../crates/rimz/src/harness/schedule/arm.rs) | Shared delivery builder returning the compiled `LoadedTask` in `ArmOutcome::Armed`, caller-scoped signal defaults and guards, locked dedupe, watcher spawning, and session retirement. |
...
| [`schedule/catalog.rs`](../../../crates/rimz/src/harness/schedule/catalog.rs) | The task catalog: the three sources, visible and runnable precedence, source-aware mutation, scheduled consumption, `LoadedTask::enable` coordinating enablement and strike clearing, and private `clear_overlays` for replacement/removal cleanup. |
| [`schedule/fire.rs`](../../../crates/rimz/src/harness/schedule/fire.rs) | Shared elder/external firing: root ownership, arm-on-first-sight, due planning, `loop-fire.json`, and host selection for the detached `rimz loop run <name>` (ordinary child or transient user scope). |
...
| [`schedule/run_log.rs`](../../../crates/rimz/src/harness/schedule/run_log.rs) | `LoopRunRecord`, `LoopRunResult`, its `spawn_exit_code` mapping through `store::run::RunStatus`, the user-global JSONL history, cost rollups, and the daily-budget gate. |
| [`schedule/arming.rs`](../../../crates/rimz/src/harness/schedule/arming.rs), [`strikes.rs`](../../../crates/rimz/src/harness/schedule/strikes.rs) | Machine-local overlays: durable enablement, bounded pauses, effective arming and source defaults through `ArmState::resolve`, their effective-last-fire rule, and consecutive failure counts. |
| [`schedule/instances.rs`](../../../crates/rimz/src/harness/schedule/instances.rs), [`overlay_store.rs`](../../../crates/rimz/src/harness/schedule/overlay_store.rs) | RimZ-owned ephemeral task rows with locked insert/remove/rename operations, and the shared locked persistence the overlays use. Instance insertion keeps separate arming and strike cleanup calls to preserve their distinct error paths. |
...
`agent` and `wait` conflict; one of `agent`, `wait`, or `check` is required. `verify` requires `agent`, because verification needs a supervised run to re-prompt. `max-attempts` requires `verify` and must be at least 1.
...
The instance store exists so runtime churn never edits user config. The strict catalog loader migrates the legacy global instance file once by each row's resolved root, keeping an existing workspace name on conflict and removing the global file afterwards; lenient reads never migrate. Old wait rows in `loop.toml` remain `Config` and gc reaps them rather than migrating them. `TaskCatalog::load(Some(root))` reads that workspace's instances; `load(None)` reads machine tasks only. An agent scheduling its own `--in 30m` wait writes state, not your `loop.toml`, and the row retires itself after firing.

Project tasks are more constrained than machine tasks, because a committed task cannot make machine-local claims. Loading rejects `root` and `dir` (a project task runs at the project root), `wait` (it cannot pin a session on someone else's machine), and `deadline` (a poll-until timestamp is machine state), and requires `every`, `cron`, or `signal`, because a one-shot would have to delete itself from a trust-hashed file on fire.
...
The timer is only a clock host. It re-reads configuration every pass, leaves arming, trust, overlap locks, execution, and run history to the existing paths, and exits after one tick. Under systemd, the tick dispatches each runner through `systemd-run --user --scope`, moving it out of the timer service's cgroup so the runner and any multiplexer server it births outlive the tick. The tick waits up to five seconds to observe the child's changed cgroup before returning. External runners also get their own process group, including on launchd; elder and signal fires retain their inherited group so interactive launch probes cannot stall on background-terminal job control.

A `Spawn` fire births a room through the supervised-run path. A scheduled check-only fire ensures its root's room is open after the budget, run-lock, and deadline gates and before executing the check, including for a pure shell check. A fresh sidebar heartbeat skips repair; otherwise it uses the normal detached room entry, including durable ownership and the rimzd loop zone, with closed-agent resume disabled and no confirmation prompt. The runner remains a per-fire supervisor outside the panes; the room hosts any agents it launches. Both paths leave the room open, so later external ticks yield to its elder. `Deliver` still requires the pinned live session. Manual `rimz loop fire` stays in the foreground, with no transient scope or pre-birth for a check-only task. A room can be born between the heartbeat check and the state write, creating a one-tick race, but the shared fire stamp and per-task overlap lock remain the duplicate and concurrency defenses already used by hot elder ticks.
...
| `team.idle` | the live cohort is non-empty, every member is at rest (`Idle` or `Success`), and no live member has a queued message or pending one-shot wait; emitted only on the false-to-true edge, computed by overlaying `prior_status` on the member for the prior view |
...
`team.stage` is produced by `harness::team_stage`, not the cohort-edge derivation. Its string payload carries `team`, `instance` (`team#channel`), `from` when present (absent when the first flip opens a board), `to`, `owner` except for `Done`, `by` (role name, `user`, or `rimz`), optional `note`, `board` (absolute path), and `at` (RFC 3339). A flip writes the board and ledger before appending the signal with `SignalSource::Team` (serialized as `team`). Root-member registration re-wakes the current owner with `from == to` and `by = "rimz"`, covering resume, restart, single-member restart, and rebirth without editing the board or compacting. Neither producer is available through `rimz events emit`.

Stage-open delivery goes directly through message dispatch as `Type: STAGE` / `From: @rimz`, always at the done boundary, with a prose body and a `Note:` line for flips, no JSON; no implicit subscription is armed for the owner. Explicit `team.stage` subscribers still fire independently with the loop's signal body, including on re-wakes. Registration prose names a continuation since the last Progress line and carries no note. Both bodies end with the seat's channel rule (report to the stage file, the user through the leader, no pane text) when the team declares a leader and the owner is not it, the same condition under which the launch reminder carries that rule. `Done` and a self-owned flip send no direct message; an owner who is not live is woken on registration.

Two facts about durability are easy to get backwards. `rimz events emit`, the wait watcher, stage openings, and the hook's team derivation append a `signal.emit` event through the ordinary store commit ([store.md](../store.md#what-is-in-it)), so `rimz events follow` replays them; `agent.*` signals append nothing extra, because the `agent.lifecycle` record they were derived from is already the durable trace. And the durable record is the `SignalEventPayload`, which keeps only name, payload, and source: a watched command's exit status travels in the fire's argv, not in the log.
...
A watched command's signal carries a `WatchOutcome`: a `WatchVerdict` plus its evidence: the durable output tail, the agent-visible path of the complete output file, and its byte size and physical line count in `summary`. The verdict is the output-free half, and it is one enum with one renderer.
...
An armed instance-sourced one-shot delivery row projects its resting target (`idle` or `success`) to `sleeping` in the sidebar and `rimz agents`; `rimz teams` gains a `sleeping` rung below `working` and above `done`. Projection matches the workspace root and target kind/session and covers timers, PID waits, watched commands, and one-shot or deadline signal deliveries, not standing subscriptions or recurring clocks. Working, waiting, failed, paused, and live-child delegating states take precedence. The standard agent card counts pending one-shot waits on its delegation line regardless of status and, expanded by selection/density or the delegation toggle, lists each one after the subagent entries. Entries are kind-first: `timer 30m · in 12m` (or `timer · in 12m` without a stored delay, `timer 30m · due` past due), `pid 16776`, `shell cargo` with the command on a second line, and `signal pr.merged · 2h left` (deadline suffix omitted when absent). The sleeping description uses the same kind words after `wait `. Any live member's pending one-shot wait withholds `team.idle`, regardless of its displayed status. Reevaluation remains lifecycle-only: cancellation is self-only and the caller's following turn boundary reevaluates the cohort; row removal itself emits no immediate idle signal. If no lifecycle event follows a removal, evaluation waits for the next member lifecycle event. `agent.idle` remains the ordinary turn-boundary signal.

`rimz wait` accepts a positive delay shorter than 24 hours, a positive `--pid`, or a command after `--`, and resolves the live calling agent through `@me`. The typed `DeliveryTrigger::Pid` lowers in the shared delivery builder to the watched shell command `while kill -0 <pid> 2>/dev/null; do sleep 1; done`, reusing command check-ins, cancellation, and delivery without a separate scheduler trigger or GNU `tail`. It observes PID accessibility, not process identity or exit status, and rejects `--on`. A user shell cannot arm or cancel. Both this CLI and `loop add --wait` call `schedule::arm::arm_delivery`; neither command imports the other.

`DeliverySpec` carries typed trigger, name, prompt, target, and provenance. The private builder assembles every delivery row. `SelfWait` accepts only delay/PID/watch with no prompt or auxiliary gates and is the only provenance that writes `wait_meta`: `armed_at`, optional `delay`, and optional `pid`. Projection uses `WaitMeta.pid` to distinguish PID waits from shell watches and preserves the timer's armed delay. Legacy watches without PID metadata remain shell waits; no shell-string recognition or migration is involved. It mints a workspace-unique `wait-<petname>`; `Loop` accepts named clock and signal deliveries; `Team(instance)` accepts standing signals. Every wait target is an instance row.
...
Each `[[agents.teams.<name>.roles]]` entry owns a `signals` array of inline tables declaring `signal`, optional string-valued `match`, and optional `prompt`; the containing role is the receiver. `prepare_team` validates selectors and explicit other-agent matches for `agent.*`. Each role's complete ordered binding list enters the executable trust hash; empty lists preserve existing hashes. Moving existing bindings to roles changes their trust hash and requires a fresh grant. Fresh `launch_layout` refuses implicit root-checkout CI/PR scope before pane or worktree side effects, naming an explicit branch/path match or `-w <worktree>` as fixes. Launching from a linked worktree passes; this refusal is not added to resume.
...
The shared delivery builder ensures the private room tmp layout and creates `tmp/rimz-waits/<name>.output` and spawns detached `rimz wait watch <name>` with null stdin/stdout, the log as stderr, and `process_group(0)`. Spawn failure rolls back the row. `signal::run_watcher` acquires its workspace runtime lock before reloading and checking the catalog row, closing cancel-before-start races. The lock carries `{pid, started_at}`; a second watcher exits without running the command.

The command runs in the task's `dir`, the linked worktree it was armed from, or the project root when `dir` is absent, draining both streams into the room tmp output file and retaining a 4 KiB tail. The watcher uses `CheckInOnce`, with an explicit timeout defaulting to 30 minutes independently of `loop.default-timeout`. Ordinary `--check` and `--verify` retain kill-at-timeout behavior. At the deadline an observed exit wins; otherwise one `Running` outcome snapshots the tail and measures the output file so far and fires without killing the command, filtering on `--on`, consuming the row, or changing strikes. Draining continues until the final exit.

Watcher-originated fires spawn `rimz loop run <name> --signal-json …` and wait for completion while holding the watcher lock, so the interim delivery cannot make the terminal fire overlap. Append/fire failures are logged and the final outcome is still attempted. Other signal emitters remain detached. The check-in body includes `Stop it: rimz wait cancel <name>` and `Another check-in: rimz wait --in <timeout>`; that timer is independent of the running command.
...
`signal::wait_output_path` derives `<StatePaths.tmp_dir>/rimz-waits/<name>.output` for arming, watching, and lost-watcher evidence. `WatchOutcome::measured` records its byte size and physical line count in `summary`, and maps the host path through `sandbox::TmpView::current` (machine policy, since no recipient is known yet) to the agent-visible `output_path`: `/tmp/rimz-waits/<name>.output` under sandbox isolation, the host path otherwise. Measurement failure warns and records a zero summary. Room teardown removes the file; in long-lived rooms gc prunes it only with no catalog row, no running watcher, and a last write older than the 14-day retention. The durable run record keeps the tail for `rimz loop logs`.
...
Loop-owned runs, both `Spawn` turns and agents launched by a check, close their panes on every terminal status, not just success, unless explicitly kept. The in-pane wrapper defers self-cleanup while the run's waiter is live, preserving verification re-prompts and failure-tail capture; `store::run::run_waiter_is_live` probes the bound socket rather than trusting a possibly stale pathname. If that waiter dies, including when a check times out, the wrapper reclaims the pane once its record is terminal. Cleanup leaves durable evidence intact: the supervised `RunRecord` retains the failure tail, transcript path, and status; `LoopRunRecord` retains the fire's outcome and check evidence, exposed by `rimz loop logs`.

### Checks

`check = "<shell>"` runs through `sh -c` in the task's `dir`, the linked worktree it was armed from, or the project root when `dir` is absent, before any agent action. A hidden `loop check-exec` trampoline calls `setsid` before executing the shell, removing the caller's controlling terminal while keeping the check and its nested waiter in one killable process group. Manual interrupts are forwarded to that group, with a one-second grace before killing an unresponsive check; the fire records `canceled` and exits 130 without evaluating polarity or launching an agent. Timeout kills the group. Either stop waits at most 200 ms for output drains so an escaped pipe holder cannot prevent the history row. `on = "fail"`, the default, fires on a non-zero exit or a timeout; `on = "success"` fires on a zero exit. `timeout = "5m"` bounds it, defaulting to five minutes.

In both scheduled and manual fires, the check receives `RIMZ_LOOP_TASK=<task-name>`, `RIMZ_WORKTREE_PATH` set to its execution directory (`dir`, or the resolved project root when absent), and the workspace pin (`RIMZ_WORKSPACE_ID` and `RIMZ_PROJECT_ROOT`) from `workspace::pin_env`, binding nested RimZ commands to the task's root. A prompted `rimz agents <cell> "<prompt>"` launched from that check is converted to a loop-owned supervised run: `-p` is implied, the command blocks for its result, its final message goes to the check's captured stdout, and its exit code reflects the run status. Without an explicit worktree selection, the launched agent lands in the check's checkout (the arming worktree), unlike the task's configured `--agent` action, which keeps the canonical project root. It uses the loop zone, inherits `loop.default-timeout` unless the launch sets its own timeout, and enables wrapper self-cleanup unless `--keep` is set. An unprompted launch is refused because there is no turn to complete; `--bg` is refused because the check must await the result. This conversion does not apply inside an agent pane carrying `RIMZ_AGENT_ID`, where agent ancestry owns child launches instead. Room birth clears the check marker so it cannot leak into later human shell launches.
...
`wait` pins a task to one exact live kind/session at add time; `--wait @me` and bare `--wait` resolve the caller through the shared CLI resolver. The runner confirms the session is alive, including `ended_at`, before sending through the ordinary durable message path.

| Intent | Header | Dispatch |
| --- | --- | --- |
| Self timer or command wait (`wait_meta`) | `Type: WAIT` | `Steer` |
...
The sender is `@rimz` and delivery inherits the harness smart-compaction default. A boundary delivery reaches an idle agent immediately and parks for a working agent's next `done`; a self wait steers. A missing or ended session records `target gone` and removes the row. SIGNAL and WAIT are acknowledged as attributed harness messages, hidden in rendered transcripts, and kept in `--json` ([messaging.md](./messaging.md#the-message-header)).
...
The record carries the resolved task `root`, result, mode (`scheduled` or `manual`), duration, error chain, check evidence (exit code, timeout flag, capped output, and the output file's agent-visible path for a watch row), the watched command's `WatchVerdict`, the triggering signal's name and payload, the durable message id of a delivery, delivery target, supervised run id and transcript path, last message, cost, and fresh input and output tokens. Append caps the stored copies: 4 KiB of check output, 2 KiB of error text and last message, and a signal payload over 4 KiB collapses to a single `_truncated` field. `rimz loop show` reads it for a health verdict plus a separate agent-run rollup for check-gated work; `rimz loop logs` prints the stored forensics in full, including the output path above the check gutter.
...
| `flip_compact` | `rimz teams flip` | flipper provider and session, role, previous and target stages, occupied context when known, effective threshold, attempted compact-command message id (may not resolve if a store error prevented publication), delivery verdict, and error when present |
...
Flip compaction is labelled `flip compaction` in the assist event stream and contributes sent commands to the compact rollup. A queued command is not counted as sent merely because the flip completed. The effective `[harness] flip_compact` or role `flip-compact` threshold is recorded with each attempt, including errors and missing-pane skips; these enrichment failures do not fail the flip. Only a member leaving its own stage for one it does not own is eligible, including `Done`. The threshold is evaluated before pane availability; below-threshold flips, including those with unknown occupancy, make no attempt and write no assist record.
