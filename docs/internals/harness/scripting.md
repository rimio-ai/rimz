...
A run's permission posture is `auto` by default, `--ask`, or `--yolo`; those two flags are mutually exclusive. A virtual cell in the spec overrides the flag, so `rimz agents claude-plan ... -p` runs in plan mode even though `-p` exposes no `--plan` flag. Model, effort, and profile-declared `auto-compact` render through each adapter's `render_preset`; the native auto-compaction window is normalized to a decimal token count and has no CLI override. The resolved system-prompt file path stays typed through planning and is mapped directly to the adapter's argv or environment during provider-process compilation.

After profile and CLI arguments merge, launch reconciles adapter-declared model, effort, `auto-compact`, and matching system-prompt replacement flags against raw `args`, adopts an args-only model as identity, and stamps an adapter default only when no model was selected. Prompt-file existence and replacement support validate before a pane opens. An adapter with no native rendering for a typed parameter **refuses the launch and names the unsupported setting** rather than dropping the intent. Supervised `--max-turns` renders through a separate per-adapter turn-limit hook. The per-kind mappings live in the adapter docs indexed under [the agent layer](../README.md#the-agent-layer).
...
**`wait` never owns lifecycle transitions.** It polls records and maps a settled status to an exit code; a subagent join may stamp that it printed a result during the agent caller's open turn (or to a human shell) and cancel only the settled digest whose every listed child has been collected that way ([subagents.md § The lifecycle, end to end](./subagents.md#the-lifecycle-end-to-end)), but it never signals a process, cancels a run, or closes a pane. Three consequences follow, and each one surprises somebody. Not waiting at all changes nothing about a run's lifecycle, because [pane reclamation](#reclaiming-the-run-pane) belongs to the in-pane wrapper rather than to the joining caller. `--any` returns the first finisher and leaves every loser running to its own completion, which is why the [race recipe](../../guide/scripting.md#in-a-pipeline) stops the loser explicitly. And `wait --timeout` bounds *the caller's patience*, not the run: it exits `124` with the run still live, which is the same code a genuinely `TimedOut` run exits with. `--json` does not disambiguate them either, because the join stamps every unfinished target `timed_out` in its result map on the way out. Only the durable record still knows — a run the caller merely stopped waiting on is still `Running` — so a script that needs the difference re-reads the record with `agents show` rather than trusting the join's exit code.
...
**A subagent background run** stops its provider at the same terminal transition, stamps `ended_at`, and closes its own pane. On spawn the wrapper persists the provider PID and process-start token on the run record. Its timeout helper writes and wakes the terminal record, signals exactly that still-identical provider, and runs the ordinary pane-close backstop unless the run is kept. With `--keep`, the wrapper instead remains alive after completion and parent exit until `rimz subagents stop` or `rimz gc` reclaims it. The child wrapper still owns the fast parent watchdog for non-kept early parent loss. The producer's durable orphan scan runs at most once per minute: on its next pass it backstops a missed settled-fleet digest, while parentless children retain their ten-minute grace. Either repair is recorded as a diagnostic rather than routine cleanup.
