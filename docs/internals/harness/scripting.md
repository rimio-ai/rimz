...
`harness::owed::owed_wake` holds a clean end open for an armed one-shot wait, a non-terminal harness `Wait`, `Signal`, or `SubagentReport` message for the card, or a launched fleet member whose newest run is non-terminal or terminal with neither `joined_at` nor `report_message_id`. It reads the fleet term first, through `harness::fleet::FleetRuns` over the Audit projection, because the digest reporter stamps the child rows before it queues the digest: rows found unstamped are owed, and rows found stamped are followed by a queue read the digest has by then reached. The wait terms then reuse `TurnWaitView::load`, whose catalog → queue → rollup order matches a wake's own publish order. The hook evaluates this input only for a root Completed disposition, before taking the run's workspace lock; a read error logs and yields nothing owed. The signal classifier stays pure, and failures and cancellation are not held open.

The record stays `Running`: `parked_at` is an optional, defaulted timestamp, not a new status. The next `TurnStarted` clears it; every terminal write clears it too. `agents show` displays the park's age, and record JSON and NDJSON live status carry `parked_at`. Live status keeps reporting the card's raw rollup status, unchanged by this fold; the timestamp is the park evidence.

Every wrapped run checks for a stranded park at most once per `PARK_STRAND_POLL` (5 s), including blocking runs without self-cleanup. Two consecutive nothing-owed checks under the same timestamp call `run::settle_stranded_park`: under the workspace lock it compares `parked_at` and non-terminal status, writes `Failed` and the first failure tail together, then wakes the waiter after unlocking. The reason is `parked on a wake that never arrived: no armed wait, no wake in flight, no live subagents`; an existing failure tail wins. An owed wake, missing bound identity, or read error does not settle the park. A park is bounded by `--timeout` where one was given; otherwise its exits are the wake itself, this strand settle, the wrapper's digest repair below, and `rimz agents stop`.

The settle leaves *nothing owed*; a fleet that settled while its digest was lost is *owed and unresolvable*, because the parked parent is at rest waiting for exactly that digest. So on the same 5 s cadence, before the strand check, a parked run's wrapper runs the fleet digest reporter for its own session. It is the same idempotent repair the sidebar producer's `orphan_sweep` backstop runs, its stamp CAS tolerates a concurrent reporter, and a success turns the owed `Subagents` term into `WakeInFlight`, so the run completes on the digest's turn instead of failing. Every outcome and every error is logged and ignored: the repair never gates the settle that follows it. A park therefore has no age bound — an armed `rimz wait --in 8h` is a legitimate park of any age.
...
- Background [pane reclamation](#reclaiming-the-run-pane) belongs to the in-pane wrapper, not `wait`. A subagent without an attended join waits for digest receipt instead.
...
The in-pane wrapper reclaims its own pane when the request set `exit_on_run_completion`: every `--bg` run, every `rimz subagents` child, and every loop-owned run, unless `--keep`. `supervise_child` rereads the record every `RUN_MONITOR_POLL` (250 ms). Once the record is terminal and no waiter socket is live (`run_waiter_is_live`), non-subagent wrappers send the provider `SIGTERM`, escalate to `SIGKILL` after `CHILD_SIGNAL_GRACE` (300 ms), run worktree cleanup for a marked worktree, and close their own pane. Subagent wrappers take that same path only after the parent's receiving turn ends, with no child turn or pending message keeping the child alive ([subagent receipt checks](./subagents.md#the-lifecycle-end-to-end)). The live-waiter check leaves a blocking caller in charge of verification and cleanup; the wrapper takes over only when that caller is gone.
...
A `rimz subagents` child reports before provider exit and can take follow-ups; a parent's message can resume an ended child. `--keep` disables automatic completion cleanup and the parent watchdog and holds the pane past provider exit. For non-kept children the watchdog cancels the run when the parent's launch ends, and the producer's orphan scan backstops missed digests and orphans. [subagents.md § The lifecycle, end to end](./subagents.md#the-lifecycle-end-to-end) owns these rules.
