...
`sandbox::TmpView` owns the host-to-agent path mapping for output records and messages: paths under room tmp become `/tmp/<relative path>` under sandbox isolation and remain host paths otherwise. `TmpView::for_launch` maps for a recipient's recorded override (subagent reports use the parent's); `TmpView::current` reads current machine policy for emitters without a recipient, such as wait watchers, so a wait armed by an agent whose override differs from machine policy names the path as machine policy maps it. Room tmp is separate from host `/tmp`, not hidden from host processes: the host state path remains accessible inside and outside the sandbox.
