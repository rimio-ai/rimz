...
**Rich context (`AgentContext`).** Claude `exec`s a configured `statusLine` command on every render and pipes a JSON blob to its stdin — `session_name`, the context-window token accounting and `current_usage` breakdown, cost, rate-limit windows, PR info, version, effort, and more. `observe_context` normalizes that blob into `AgentContext` (every field `Option`, tolerantly parsed). Install therefore manages the statusline too: it sets `statusLine` to `rimz statusline feed --source claude`, and if the user already has one it **wraps** it — capturing the JSON, then passing it unchanged to the original command and forwarding that command's stdout and exit code, so rendering is visually unaffected. The original is stored verbatim under `_rimz_wrapped` and restored on uninstall. The wrap is a visible surface: the consent gate summarizes it and the install diff shows it in full. The feed path is ledger-free, lock-free, and its child's stdio is fully piped, never inherited. How `AgentContext` is stored and folded into the rollup is [agent.md → Rich context](./agent.md#rich-context-agentcontext)'s concern.
...
Codex has no statusline, so its rich `AgentContext` (the analogue of Claude's statusline feed) is read out of band from the official Codex app-server (`codex app-server`, JSON-RPC 2.0 over stdio) by [`agents/codex_app_server.rs`](../../crates/rimz/src/agents/codex_app_server.rs). The client speaks only **read-only, non-interfering** methods — `initialize`/`initialized`, then `account/rateLimits/read` (the 5h/7d windows) and `model/list` (the session model's display name + effort); the version rides the `initialize` `userAgent`. It never calls `thread/resume`, `turn/start`, or any write — those would rejoin and own the user's live thread.

The trigger is never inline: on the turn-boundary lifecycle events the Codex hook spawns `rimz codex refresh-context` **detached with null stdio**, so the hook returns before the app-server round-trip and adds no latency to the turn. That helper writes the same per-session `AgentContext` sidecar Claude's statusline produces; the sidebar's next wakeup folds it in. The sidebar producer also asks active root Codex sessions to refresh that sidecar at most once per 60 seconds — the provider dashboard prefers live session windows over the shared idle cache, and a long-running turn may otherwise wait until it ends — and a logged-in metered account with no root session uses `rimz codex refresh-rate-limits` on the same cadence for the account-scoped cache. A short freshness throttle skips a refresh when the sidecar is only seconds old, so two close boundaries don't each spawn a server. `RIMZ_CODEX_BIN` overrides the binary (tests point it at a stub). Everything is best-effort: a missing `codex`, an API-key account with no rate-limit windows, or any protocol hiccup degrades to an omitted field, never a failed hook.

Connection preference (warmest first), all best-effort with a cold-spawn floor so enrichment never depends on any one of them:

1. **The per-session broker** ([`agents/codex_broker.rs`](../../crates/rimz/src/agents/codex_broker.rs), run as `rimz codex app-server serve` in the `rimzd` daemon tab) holds one long-lived, already-handshaked `codex app-server` and serves it over a unix socket, so each datapoint skips the cold-spawn handshake. It runs whenever `codex` is on PATH — a local enrichment capability, not the remote-control feature.
2. **The per-user remote-control daemon** (`codex remote-control start`, auto-launched when `[remote_control] codex` is on), re-used via `codex app-server proxy --sock <path>`. Overridable by `RIMZ_CODEX_APP_SERVER_SOCK`.
3. **A fresh cold-spawned `codex app-server`** — the always-present fallback, so headless / no-mux still enriches.

The one datapoint the app-server does **not** expose read-only is token / context-window usage: as of the pinned Codex, usage rides only the live `thread/tokenUsage/updated` notification behind a subscribing `thread/resume`. So Codex's context gauge stays sourced from the rollout transcript tail (see [agent.md](./agent.md#enrichment-is-display-only)), and `AgentContext.tokens` is left `None`.
