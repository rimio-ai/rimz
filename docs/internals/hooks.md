...
**Subagents.** Claude routes Task-tool children through `SubagentStart`/`SubagentStop`, keyed by the child `agent_id` with the payload `session_id` captured as `parent_agent_id`, so the child gets its own row nested under its parent (see [agent.md](./agent.md#instance-identity-and-age) and [sidebar.md](./sidebar.md)). Identity is keyed, never guessed: the shared [`resolve_subagent_identity`](../../crates/rimz/src/agents/mod.rs) requires a child id distinct from the parent. A subagent event missing one is **quarantined** — it yields no observation (logged at `error!` under `rimz::agent::lifecycle`) rather than folding onto, and renaming, the parent's row. The child's type label is identity, not activity: the reducer carries it forward, so a `SubagentStop` that omits `agent_type` leaves the finished child labeled rather than degrading it to a `subagent <id>` placeholder.
