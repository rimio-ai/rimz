...
> See [DESIGN.md](../../../DESIGN.md) for the commitments this doc operationalizes. The per-provider native mappings — which native event means what for each agent — live in the adapter docs ([adapter/claude.md](./adapter/claude.md), [adapter/codex.md](./adapter/codex.md), [adapter/pi.md](./adapter/pi.md)); the account, balance, spend, and pricing model is [provider.md](./provider.md).
...
A coding agent reports to Rimz through hooks. Every agent — Claude, Codex, Pi, and every future one — speaks through one trait, [`AgentAdapter`](../../../crates/rimz/src/agents/mod.rs), registered in [`registry::ADAPTERS`](../../../crates/rimz/src/agents/registry.rs). The trait is the single place a native protocol diverges and the single place it is normalized; nothing downstream of it is agent-specific. The per-provider mappings it produces are the adapter docs ([adapter/claude.md](./adapter/claude.md), [adapter/codex.md](./adapter/codex.md), [adapter/pi.md](./adapter/pi.md)); the raw upstream protocols they read are the [external references](../../externals/agent-adapter/claude-reference.md).
...
**The installed config shape.** Claude and Codex have no wildcard event key, so install writes one block per wired event into their config files; Pi instead owns one whole extension file (see [adapter/pi.md](./adapter/pi.md)). Inside the config-merge shape it stays minimal:
...
A provider whose hook wire Rimz authors has a third option: stamp the gauge **onto the hook payload itself**. Pi's extension does this on every envelope, so its gauge needs neither a tail nor a transport ([adapter/pi.md](./adapter/pi.md#context-and-transcript)).
