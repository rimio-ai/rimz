...
**The durable transcript log** records the conversation surface Rimz renders: user prompts, each root turn's final assistant message, inter-agent deliveries as received prompts, native question asks, and effective answers. Ask entries carry structured `questions` with option labels while `text` holds the assistant lead-in; answer entries carry structured `answers` with chosen labels and notes while `text` holds the flattened answer for legacy and orphan rendering. Hook and resolver paths append compact JSONL entries under `transcript/<bucket-start>.jsonl`; `[transcript] file_days` chooses the bucket width for file-size control, and buckets are not pruned. `rimz transcript` reads this Rimz-owned log, so ended agents, past channels, asks, and answers remain visible after native agent files rotate or leave the live snapshot.
