...
| `agent.end` | `TurnEnded`, or `TurnInterrupted` for `cancelled` | done versus error; cancelled closes at rest |
| thread state enters `awaiting-approval` | `AwaitingInput { Permission }` | no ask detail or external answer handle |

Amp can run several threads concurrently in one CLI. The plugin forwards an event only when its thread matches `amp.activeThread.current`; execute and runner modes report with a null active thread and therefore forward normally. Switching the focused thread fired `session.start` for that thread on builds with the TUI thread sidebar; Amp removed that sidebar on 2026-08-27, and whether a focus change on later builds still fires the event is unverified ([amp-reference.md → Undocumented behaviour](../../externals/agent-adapter/amp-reference.md#undocumented-behaviour)). RimZ stamps that registration as `Fresh` pane occupancy, including when returning to an existing conversation, so the same-pane supersession rules retire the previously focused row and revive the newly focused one.
...
The permission path observes Amp's own UI without joining its decision chain. `awaiting-approval` carries neither tool detail nor a resolver, so the row reads waiting and the user answers in Amp. The plugin deliberately does not register `tool.call`, whose cancellation and multi-plugin result composition are not documented.

`agent.end` carries a three-way `status`: `done` folds to a clean `TurnEnded`, `error` (or a missing status) to `TurnEnded { errored: true }` and a `failed` row, and `cancelled` to `TurnInterrupted`, which closes the turn at rest with no result, the [turn-interruption](./model.md#displayed-status) rung.
...
**Neutral output.** Hook stdout stays empty. Permission prompts remain in Amp's own UI, and the sidebar's `?` row routes you there.

**Install.** Install, preview, and uninstall own the whole plugin file.

**Agent process.** Amp runs plugins in a child runtime process (`amp run …/plugin-runtime.ts`), so the plugin stamps `process.ppid`, the Amp CLI, as `RIMZ_AGENT_PID`. A plugin reload replaces the runtime process without changing the recorded owner. Both the July 2026 baseline binary and the September 2026 build launch that runtime.

## Launch and resume

`rimz agents amp` launches the stock interactive CLI. Supervised prompts use `amp -x <prompt> --plugin-ready-timeout 30 --no-archive-after-execute`: the ready wait keeps execute mode from starting before plugin handlers load, and the archive opt-out keeps the thread resumable, because Amp refuses `amp threads continue` on an archived thread and execute mode archives by default. `amp threads continue <T-id>` resumes a thread, while fork and manual compaction have no native command.

Profiles map `model` to Amp's `--mode` dial. Amp removed `--effort` (build 0.0.1789344113 rejects it as an unknown option), so a profile `effort` fails at launch; the dial's per-mode effort is chosen in Amp's Mode Dial settings instead.

Profiles pass a configured `--mode` verbatim: Amp accepts `low`, `medium`, `high`, `ultra`, or a plugin mode by key or label, and the deprecated `smart`, `deep`, and `rush` still name existing threads ([amp-reference.md → Models, modes, and effort](../../externals/agent-adapter/amp-reference.md#models-modes-and-effort)).
...
Live cost and historical spend are partial estimates: model pricing does not reconcile Amp credits, workspace billing, or subscription entitlements, and no included-balance or OAuth usage window is available.

## Known gaps

Run `rimz coverage` for the current wired/partial/unsupported matrix. Every item below is a signal Amp does not yet expose in a stable machine-readable form, or a RimZ abstraction that would need a new seam to carry one that it does. They are ordered roughly by user-visible value.

- **The private thread cache may be gone.** See [Context and transcript](#context-and-transcript). If a successful turn on a current build confirms Amp no longer writes it, live and history marks need to drop, and `amp threads usage` (human text only) or supervised stream JSON `usage` are the remaining token sources.
...
- **Permission coverage reads wired but is conditional** on another Amp policy surface opening `awaiting-approval`. RimZ maps that native state end to end and therefore declares the concern wired, while the coverage abstraction cannot also express that the default Amp posture creates no prompt and that legacy/custom approval dialogs still need live verification.
- **Permission presets render only provider argv.** Amp needs an installed policy handler for `ask` and prompt-level guidance for `plan`, so representing those postures faithfully would require an adapter-owned prompt transform or conditional managed-plugin policy rather than a fake CLI flag.
- **Supervised threads stay unarchived.** Keeping them resumable lists every supervised run in `amp threads list`; `amp threads archive` clears them by hand.
- **Rich context stays unsupported.** A context-window divisor, authoritative credits/workspace billing, built-in subagent identity, permission details and resolution, automatic-compaction brackets, and remote-control readiness all wait on stable machine-readable Amp signals. Amp does publish a paginated in-process transcript API; RimZ currently consumes only the bounded `agent.end.messages` slice needed for final answers, and a future rich-context transport can page `PluginThread.messages({ full: true })` without putting provider payloads below the adapter boundary.
