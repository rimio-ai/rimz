...
The per-kind surfaces each provider exposes are in the adapter docs ([claude.md](./claude.md#account-and-balance), [codex.md](./codex.md#account-and-balance), [amp.md](./amp.md#account-and-balance), [copilot.md](./copilot.md#account-and-balance), [pi.md](./pi.md#account-and-balance), [opencode.md](./opencode.md#account-and-balance), [cursor.md](./cursor.md#account-and-balance), [kiro.md](./kiro.md#transcript-lifecycle-and-context)); the raw auth and account-usage surfaces those read are in the per-provider upstream references ([claude-reference.md](../../externals/agent-adapter/claude-reference.md#auth-surface), [codex-reference.md](../../externals/agent-adapter/codex-reference.md#auth-file), [amp-reference.md](../../externals/agent-adapter/amp-reference.md#authentication-account-and-spend), [copilot-reference.md](../../externals/agent-adapter/copilot-reference.md#authentication-and-account-surface), [pi-reference.md](../../externals/agent-adapter/pi-reference.md#auth-file), [opencode-reference.md](../../externals/agent-adapter/opencode-reference.md#auth-file), [cursor-reference.md](../../externals/agent-adapter/cursor-reference.md#authentication-and-account-surface), [kiro-reference.md](../../externals/agent-adapter/kiro-reference.md#verified-stock-session-store)).
...
`[accounts.budget] <kind> = "100/day"` turns on one provider-login cap across every room on the machine. Eligibility requires the adapter's durable `account_spend` capability; identity, a plan label, quota enrichment, and a live-session price do not establish a provider-day decision input. Config parsing, room start, and `rimz budget --account` reject unknown or ineligible kinds before any cap or ledger can arm. The spending walk computes a local-calendar-day `SpendWindow` per eligible provider independently of the configured headline, publishes it in machine-shared `provider-spending.json`, and version-gates the cache so an upgrade re-aggregates the current cursor without reparsing unchanged transcripts. The producer accepts the cache's normal short TTL staleness as rate-limit latency.
...
The producer keeps every metered account's balance current between turns through one hidden helper, `rimz agents refresh-usage --kind <kind>`, spawned for each metered, logged-in panel whose OAuth-attempt marker is due. The marker uses its mtime as the `OAUTH_USAGE_TTL` clock and stores the provider's optional credential-file stamp; a changed known stamp bypasses the outer throttle on the next producer pass, while equal known stamps, two unknown stamps, and legacy empty markers for an unknown source keep the normal cadence. The helper single-flights the actual provider read with a dedicated `credits.json` `oauth_read_at_ms` stamp, so realtime/app-server writes to the display freshness stamp never suppress OAuth. `RIMZ_OAUTH_USAGE_OFFLINE=1` disables account-usage fetches for that process tree, and a provider with no local OAuth credentials records a quiet no-credentials attempt. A settled auth failure, including missing credentials or a provider-rejected token, records a quiet attempt retried on `OAUTH_USAGE_SETTLED_TTL` (1 h) and retried immediately once the provider's auth file changes or the local account key differs.
