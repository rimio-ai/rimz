...
The producer samples growth in the longest duration window into the user-shared `auto_redeem_rate.codex.json` cache. A three-day half-life EWMA predicts how long a fresh window takes to fill, and the full sorted credit-expiry list produces backward chain deadlines spaced by that refill time. `ScheduledRedeem` starts the chain even at low current usage so later credits can each capture a refill; a natural reset less than `auto_redeem_min_gain` away defers that deadline when the first credit still survives it by at least 24 hours. Missing or negligible burn rate collapses scheduling back to the 30-minute rescue deadline.
...
Two version stamps keep the cache honest across schema changes. A shape or parsed-value change to the entry split, store-time dedup, per-file origin metadata, pricing fallback, or logical source stamp bumps `SPENDING_CACHE_VERSION` (currently 20, which locally prices Grok completions that omit native cost and cold-rebuilds every v19 cursor once), so finalized sessions re-parse cleanly. A semantic change to the published aggregate or guaranteed pricing that does not require a source re-parse bumps `PROVIDER_SPENDING_VERSION` (currently 11, which replaced per-kind session bursts with one machine-global burst), so `provider-spending.json` recomputes once from the current entry cache. Writers also refuse a schema downgrade after a cheap leading-version probe, so an older long-lived build cannot blank a newer build's published aggregate or force persistent cursor cold-walks: a version bump costs one recompute, then the higher-version cache holds.
