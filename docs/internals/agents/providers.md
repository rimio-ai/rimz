...
Per-agent and per-team lifetime figures use [`spending::slot_effort`](../../../crates/rimz/src/agents/spending/effort.rs), the sanctioned transcript read for one durable agent seat. It selects each continuation's rows, runs one cross-session `SidechainDedup` so resumed transcripts cannot replay prior work into the total, and prices every retained entry through the shared book. Attribution, `rimz agents show`, `rimz teams show`, and the sidebar's finished-cohort receipt all consume that fold, so their dollars and token split name the same work.
