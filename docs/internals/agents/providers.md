...
Every credential-file path stays read-only. RimZ does not refresh tokens, write provider auth files, or import browser cookies.

A provider home is an account. Claude and Codex can each declare named accounts (`[accounts.claude.<name>]`), a standalone home selected by `CLAUDE_CONFIG_DIR` or `CODEX_HOME`; `default` is the provider's own native home and every other kind only has `default`. A room runs each kind under one account fixed at birth, so every probe, claim, and cache entry below is keyed by `LoginKey` (`claude@work`, `codex@default`) and runs under that login's environment ([`agents/login.rs`](../../../crates/rimz/src/agents/login.rs)). A room reads and writes only its own logins' entries; rooms on another account of the same kind keep theirs. Panels stay one per kind, and a non-default login labels its block `Claude · work`. Switching a live room between accounts is not supported: `rimz reset --account` starts a new room.
...
- **Account.** `metered`, the account scope, and the `plan` label come from the kind's account (a live session's, or the probed idle one). A `plan` tier formats into a brand label (`max` becomes `Claude Max`, `pro` becomes `ChatGPT Pro`), and a missing account infers `metered` from whether windows were reported or a keyed session owns the panel. The keyed fallback keeps cached authoritative windows attached while a newly registered session has not reported its first live window.
...
- **Which sessions speak for the panel.** Live windows are partitioned by [birth account key](./model.md#the-rollup) before fusion. The panel adopts the key of the newest-registered keyed session (by `registered_at`, then session id), then admits that session, every session carrying the same key, and every keyless session, and excludes sessions carrying a different key, so a session that registered under a different login contributes no usage to the account the panel shows. `plan` and `version` still come from the freshest context regardless of key. A panel whose sessions are all keyless stays keyless and excludes nothing, which is also what a fleet registered before this stamp existed looks like: those sessions keep speaking for whichever account the panel adopts until they restart and re-register.
...
**Where the caches live.** The account, rate-limit, and credits caches are user-scoped and persistent under `$XDG_STATE_HOME/rimz/shared/`, single-flighted across rooms by locks under `$XDG_RUNTIME_DIR/rimz/shared/`. All three are keyed by login (`accounts.json` and `credits.json` under `logins`, `rate_limits.json` under `entries`), and a publication replaces only the probing room's keys. The elected producer publishes `accounts.json`, `rate_limits.json`, and `credits.json`; consumers read them and never fork. A due account batch runs independent account-then-version chains through four scoped workers and joins them before one deterministic atomic `accounts.json` publication. A caller that cannot open the coordination lock may probe locally without publishing, and a caller that times out behind a live producer serves the current cache for that frame and lets the next tick observe the winner, so fresh rooms do not duplicate the cold subprocess wave.

The rate-limit cache schema is version 6, keyed by login; v5 and older kind-keyed caches cold-drop under the version gate, and the kind-keyed `accounts.json` and `credits.json` shapes cold-drop by failing to parse. Scoped quota values remain intact for status decisions after their reset; only display projection clears expired scoped usage to unknown.
...
Each [`RateLimitWindow`](../../../crates/rimz/src/agents/context.rs) carries its provenance: an `observed_at` capture stamp, and a `source` that is either `BestEffort` (Claude's statusline) or `Authoritative` (a direct account-usage query: Codex's app-server, a provider OAuth refresh, or Antigravity's verified private local service). Usage only climbs within a live window, so best-effort refills need confirmation; authoritative corrections instead depend on when they were observed. Two stages:
...
**Across source and time.** [`fuse_window`](../../../crates/rimz/src/sidebar/refresh/rate_limits.rs) folds that live candidate into the persisted truth ([Persistence across idle sessions](#persistence-across-idle-sessions)). The trust boundary is decided first, and direction only afterwards:

| Evidence | Verdict |
| --- | --- |
| an `Authoritative` reading stamped no earlier than the prior | adopt: it queried the official API, and it anchors the window in either direction |
| an `Authoritative` reading that cannot prove it is no earlier than the prior | hold the prior: an out-of-order or unstamped query cannot undo newer truth |
| a `BestEffort` reading stamped after an `Authoritative` prior | eligible to overlay, subject to the climb and refill rules below |
| a `BestEffort` reading that cannot prove it is newer than an `Authoritative` prior | hold the anchor |
| an admitted `BestEffort` climb or steady reading | adopt: usage only climbs within a live window |
| a later `resets_at` on a `BestEffort` drop | adopt: a new window epoch |
...
The confirm path exists for the mid-window free reset a provider sometimes grants, refilling progress with the reset timer unchanged, so one lagging or garbled statusline sample cannot dip a live budget while a genuine refill still surfaces. An out-of-order authoritative reading cannot undo newer truth; best-effort readings must be newer than an authoritative prior to enter the climb or refill rules.

The anchor rule leaves one disagreement standing: a live session that keeps reporting a fresh reading contradicting the API for the same account alternates with each direct query. RimZ cannot adjudicate between two provider surfaces, and the authoritative value is never more than one read cadence away.
...
Fused ground truth is persisted, including authoritative lifted rows and the open unknown episode's marker; reset-to-full and unknown windows stay read-time projections, never written. An entry with an exact credential account key also retains the matching authoritative windows separately for bound managed-launch controls. Scope-only live readings update fused display truth without acquiring that credential identity. The cache tracks login and drops the room's login entry once that kind's panel is gone; entries of other accounts are never touched by this room's publication.

A live panel reading meets its cached entry under the same admission rule the panel uses for sessions: the entry's bound key and the panel's key must be equal, or one of them must be absent. A keyed mismatch clears the live windows for that frame, so the cached account's truth paints on rather than being fused with another account's usage. Only an authoritative account read binds an entry's key, and it fuses onto a prior entry only when scope and key both match, so an account switch writes a fresh entry instead of inheriting the previous account's windows. The producer's own publication never binds a key: it republishes fused display truth under the prior entry's key, retaining that entry's bound copy and snapshotting one from its existing truth when a keyed entry has none yet.
...
The helper first folds a provider realtime account reading when the adapter exposes one, renews the matching claim, then runs the direct account-usage query on its own cadence where one exists. The lease covers each segment independently: realtime plus one rate-cache publication before renewal, then direct provider work plus publication after. A missing, replaced, or lock-contended claim cannot renew, and direct work stops; completion keeps the nonce check that rejects a superseded writer. Both paths publish the same `AccountUsageSnapshot`, and every window is routed through per-window timestamp-aware fusion while plan, paid credits, and reset credits follow one cache conversion. Direct-query windows are authoritative and merge after the realtime fold, so a fresh credential read can replace a stale warm realtime process. A current authoritative read replaces the fused window immediately in either direction; only a later best-effort reading may overlay it, with same-epoch refills still subject to confirmation. Identity-bearing completion invalidates the prior account's windows before publishing replacements; a known new-owner failure drops prior truth without publishing unverified values, while ownerless and same-owner failures retain it. Detached authoritative writers wait on the bounded rate-cache advisory lock and publish their read-modify-write result before returning, while the high-frequency producer keeps its non-blocking read-only fallback under contention.

When producer window fusion persists a new reset epoch, newly parks a refill for a window key, or first shows an unknown kind display, it clears `oauth_read_at_ms`, settled state, and any stale claim, allowing usage scheduling in the same refresh pass to re-probe before either the five-minute or one-hour settled cadence elapses. An already-pending refill does not force another read; a successful authoritative response clears that marker, so a newer contradictory low session reading can park again and request another query.
...
The walk discovers every registered spend store fleet-wide and across every declared account home (every Claude project directory, every Codex, Pi, and Copilot session file, every OpenCode database), so each provider counts on the same footing regardless of which project it ran in. Each adapter declares its store topology once: complete transcript lookup walks the declaration on demand, while indexed warm discovery prunes retired history and repairs its frontier on the complete reconciliation cadence. Transcript-only providers such as Kiro keep session lookup without entering fleet aggregation, and plugins enter only when their manifest declares both transcript globs and a spend probe.

Each window carries the token split (its `↘` input folds in `cache_write`, so the `◇` total is folded input plus output, and `cache_read` rides apart), named tool-call totals, and a `sessions` count of the distinct threads that ran in the window. The same parser path feeds [`spending::session_cost_usd`](../../../crates/rimz/src/agents/spending/mod.rs), the turn-end live-card floor for adapters whose provider transport does not push a dollar total: it selects one session's rows through `session_entries`, which applies the walk's `SidechainDedup` policy, then returns session coverage whether each retained entry carried provider USD or used the `PriceBook`. Droid uses that boundary for its cumulative exact-table value and keeps `spending_sources()` empty, so its settings snapshot cannot reach historical aggregation.
...
Two version stamps keep the cache honest across schema changes. A shape or parsed-value change to the entry split, store-time dedup, per-file origin metadata, pricing fallback, or logical source stamp bumps `SPENDING_CACHE_VERSION` (currently 24, which keeps Claude responses whose nested iteration `model` is null and cold-reprices every v23 cursor once), so finalized sessions re-parse cleanly. A semantic change to the published aggregate or guaranteed pricing that does not require a source re-parse bumps `PROVIDER_SPENDING_VERSION` (currently 13, which publishes per-account local-day windows for account daily caps), so `provider-spending.json` recomputes once from the current entry cache. Writers also refuse a schema downgrade after a cheap leading-version probe, so an older long-lived build cannot blank a newer build's published aggregate or force persistent cursor cold-walks: a version bump costs one recompute, then the higher-version cache holds.
...
Claude adds one provider-local layer when an administrator deploys `modelPricing` in the system `managed-settings.json` or an alphabetically merged `managed-settings.d/*.json` fragment. RimZ reads the same platform locations as Claude Code (`/etc/claude-code` on Linux/WSL, `/Library/Application Support/ClaudeCode` on macOS, and `C:\Program Files\ClaudeCode` on Windows). An exact override supplies the four USD-per-million-token rates Claude requires; its cache-write rate covers both cache-write durations, and fast-mode or US-data-residency surcharges are not added. The optional multiplier scales every resulting Claude rate, including an override. List rows not named by an override keep their capacity and tier metadata while their rates scale; an override retains the list row's published context capacity. Invalid files, fields, and rows are ignored as pricing enrichment, never a startup precondition.
...
GPT-6 Astra is a long-context canary: the projection fixtures cover both upstream shapes, standard input/output and cache rates, and the whole-request price switch strictly above 272,000 input tokens. Pricing capacity metadata does not change Codex's fresh-session context fallback.
...
| --- | --- | --- |
| **Claude** | [`claude auth status`](../../externals/agent-adapter/claude-reference.md#auth-surface) → plan + metered | statusline 5h/7d windows, or the idle OAuth usage probe ([adapter_claude.md](./adapter_claude.md#account-and-balance)) |
| **Codex** | app-server `planType` or `~/.codex/auth.json` | app-server `primary`/`secondary` windows + `credits.balance`, or the OAuth usage probe; reset credits come from the Codex-only OAuth reset endpoint ([adapter_codex.md](./adapter_codex.md#account-and-balance)) |
| **Antigravity** | statusline or running `agy` private local service → plan + metered | paired same-endpoint status and quota read → hashed kind-wide owner + authoritative 5h/weekly windows; credits and dollars remain unknown ([adapter_antigravity.md](./adapter_antigravity.md#account-and-balance)) |
| **Copilot** | `$COPILOT_HOME/config.json` host-safe non-secret login identity → metered | bounded internal account query → plan + named monthly `AIC`/`cht` and genuine `prm` scopes; completions, extra credits, dollars, and spend remain unsupported ([adapter_copilot.md](./adapter_copilot.md#account-and-balance)) |
| **Kimi** | typed `~/.kimi-code/credentials/kimi-code.json` shape → managed OAuth, kind-wide | fixed-host managed OAuth usage → weekly and detail windows + optional USD Booster ([adapter_kimi.md](./adapter_kimi.md#account-and-balance)) |
| **Pi** | `~/.pi/agent/auth.json` (oauth → metered, api_key → unmetered) | extension response headers + the OAuth usage probe, cached under `pi` ([adapter_pi.md](./adapter_pi.md#account-and-balance)) |
| **OpenCode** | `~/.local/share/opencode/auth.json` (oauth → metered, api_key → unmetered) | the OAuth usage probe over the active backing-provider token, cached under `opencode` ([adapter_opencode.md](./adapter_opencode.md#account-and-balance)) |
| **Qwen** | effective JSONC model/provider + credential source → Alibaba scoped, or recognized direct API unmetered | experimental fixed-host Alibaba API-key usage → scoped 5h/7d/30d display only; local transcript dollars stay separate ([adapter_qwen.md](./adapter_qwen.md#account-and-balance)) |
| **Cursor** | resolved CLI `status --format json` + `about --format json` → email, raw tier, version | no quota, paid-usage, or durable spend surface ([adapter_cursor.md](./adapter_cursor.md#account-and-balance)) |
| **Grok** | `${GROK_HOME:-~/.grok}/auth.json` non-secret metadata → session metered or API key unmetered | no quota-window query; active-branch `turn_completed` supplies native or locally priced durable USD spend ([adapter_grok.md](./adapter_grok.md#account-and-balance)) |
| **Amp** | thread and account surface ([adapter_amp.md](./adapter_amp.md#account-and-balance)) | no included-window surface |
