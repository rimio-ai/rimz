...
The per-kind surfaces each provider exposes are in the adapter docs ([claude.md](./claude.md#account-and-balance), [codex.md](./codex.md#account-and-balance), [antigravity.md](./antigravity.md#account-and-quota), [amp.md](./amp.md#account-and-balance), [copilot.md](./copilot.md#account-and-balance), [pi.md](./pi.md#account-and-balance), [opencode.md](./opencode.md#account-and-balance), [cursor.md](./cursor.md#account-and-balance), [kiro.md](./kiro.md#transcript-lifecycle-and-context)); the raw auth and account-usage surfaces those read are in the per-provider upstream references ([claude-reference.md](../../externals/agent-adapter/claude-reference.md#auth-surface), [codex-reference.md](../../externals/agent-adapter/codex-reference.md#auth-file), [antigravity-reference.md](../../externals/agent-adapter/antigravity-reference.md#account-and-authentication), [amp-reference.md](../../externals/agent-adapter/amp-reference.md#authentication-account-and-spend), [copilot-reference.md](../../externals/agent-adapter/copilot-reference.md#authentication-and-account-surface), [pi-reference.md](../../externals/agent-adapter/pi-reference.md#auth-file), [opencode-reference.md](../../externals/agent-adapter/opencode-reference.md#auth-file), [cursor-reference.md](../../externals/agent-adapter/cursor-reference.md#authentication-and-account-surface), [kiro-reference.md](../../externals/agent-adapter/kiro-reference.md#verified-stock-session-store)).
...
- **Account identity** — [`AgentAccount`](../../../crates/rimz/src/agents/context.rs): the non-secret `account_id` a provider exposes, the raw `plan` tier it reports (`max`, `team`, `pro`), a `metered` flag, and — for a multi-provider client (Pi) — the raw `sub_provider` credential id (`anthropic`, `openai`) naming the subscription the account runs on.
- **Included balance** — [`AgentRateLimits`](../../../crates/rimz/src/agents/context.rs): an ordered list of [`RateLimitWindow`](../../../crates/rimz/src/agents/context.rs)s (short→long), each a `used_percentage`, a typed `resets_at` instant a renderer formats as a countdown, and a `duration_mins` that names the window. This is the subscription mana bar: it spends first and refills automatically. Reported durations drive the bar label (`5h`/`7d`) and the reset-to-max roll-forward, so server-side changes in window count or length render gracefully (a transient Codex bug once widened its window to ~30 days; it painted a labeled `30d` bar rather than misrendering). When an authoritative full reading omits a previously reported duration while retaining another window, RimZ marks the omitted window as lifted and renders it as an unlimited full bar with `∞`; reporting that duration again or changing accounts clears the marker. An adapter can also declare `implicit_unlimited_window_mins` for a conceptual window the provider omits on some plans; Codex declares its 5-hour duration so a cold cache paired with a weekly-only reading still renders the 5-hour `∞` bar.
...
| **Antigravity** | statusline or running `agy` private local service → plan + metered | private local service model buckets conservatively normalized to authoritative 5h/weekly windows; credits and dollars remain unknown — [antigravity.md](./antigravity.md#account-and-quota) |
| **Copilot** | `$COPILOT_HOME/config.json` non-secret login identity | plan, quota, and spend remain deferred — [copilot.md](./copilot.md#account-and-balance) |
...
Every provider meters its account through two channels feeding one per-kind fusion: a **realtime** source where the provider exposes one — Claude's statusline, Codex's app-server, Antigravity's running local service, Pi's extension response headers — and a **direct account-usage query** where the provider exposes one. Claude, Codex, Pi, and OpenCode use provider OAuth tokens for the direct query; Antigravity reuses only the verified loopback service of an already-running `agy` and reads no credential. The producer runs the query for every metered, logged-in provider on its own cadence, even while a live session is active; OpenCode meters through that channel alone. The fusion is keyed by kind, so each account paints its own bars.

The realtime leg is the per-kind difference: Codex reads its app-server first because it is the provider's local read-only account surface, Claude uses statusline windows when a session is alive, Antigravity reads the verified running service, Pi reads extension response headers, and Pi/OpenCode select their active backing-provider OAuth token from `auth.json` and delegate to the Claude or Codex usage fetcher. The producer drives the account-usage channel on a shared `OAUTH_USAGE_TTL` through one hidden helper ([Refresh cadences](#refresh-cadences)), and its authoritative readings fold into the same cache as realtime readings.
...
Every registered adapter also exposes a display-only version probe by default: run `<kind> --version`, capture stdout, and treat any failure as no version. Rich context transports still win when present — Claude's and Antigravity's statuslines and Codex's app-server context can report fresher versions for live sessions — while the CLI probe fills idle or older account-cache entries. Antigravity overrides the default with no probe because invoking `agy --version` is unsafe for idle account enrichment. A provider's absent version bypasses its long success TTL only after its short retry TTL, so a binary that cannot report `--version` does not re-fork on every producer frame or disturb another provider's clock.
...
- **Brand style** — `[theme.providers.<kind>]` overrides the built-in defaults. Art resolves from the embedded emblem catalog, using a curated kind entry when present and the shared fallback otherwise; color and product name come from the descriptor, while an unknown kind gets neutral grey. See [theme.md](../../guide/theme.md#provider-styling).
...
Paid usage has a sibling `$XDG_STATE_HOME/rimz/shared/credits.json` cache with the same account-scoped shape and runtime-lock discipline. Provider-reported values are persisted when a local account-usage surface returns them; Codex plan and reset-credit fields persist there with the paid-usage entry, preserving partial realtime/OAuth observations across idle sessions. A successful OAuth observation carries the exact non-secret owner associated with the credentials that produced it, which wins over a preflight owner read. Owner comparison is symmetric: `None → Some`, `Some → None`, and unequal identified owners all make the credits read due immediately, block prior plan/paid/reset carry, and drop that kind's cached windows; failures remain associated with the current preflight owner. API-key spend projections are not persisted because they are already derivable from the transcript spending walk plus config. A stale or absent credits entry leaves the `ex` row unknown (`∞` value over a dim empty track), while a configured display ceiling can still scale the row if usage is known.

### Refresh cadences

The producer keeps every metered account's balance current between turns through one hidden helper, `rimz agents refresh-usage --kind <kind>`, spawned for each metered, logged-in panel whose account-usage attempt marker is due. The marker uses its mtime as the `OAUTH_USAGE_TTL` clock and stores the provider's optional credential-file stamp; a changed known stamp bypasses the outer throttle on the next producer pass, while equal known stamps, two unknown stamps, and legacy empty markers for an unknown source keep the normal cadence. The helper single-flights the actual provider read with the compatibility-named `credits.json` `oauth_read_at_ms` stamp, so realtime/app-server writes to the display freshness stamp never suppress the query. `RIMZ_OAUTH_USAGE_OFFLINE=1` disables account-usage fetches for that process tree; OAuth-backed providers without local credentials record a quiet no-credentials attempt, while Antigravity treats a missing verified running service as unavailable and preserves its last-known cache. A settled auth failure, including missing credentials or a provider-rejected token, records a quiet attempt retried on `OAUTH_USAGE_SETTLED_TTL` (1 h) and retried immediately once the provider's auth file changes or the local account key differs.

The helper first folds a provider realtime account reading when the adapter exposes one, then runs the OAuth query on its independent cadence where one exists. Direct account-usage windows are authoritative; they merge after the realtime fold, so a current query replaces a stale warm realtime process. The query's plan tier rides the credits cache and fills a provider panel only when no live session reports a plan. When window fusion persists a new reset epoch, it removes the producer marker and zeroes `oauth_read_at_ms`, forcing the next cache-refresher tick to re-probe before the five-minute cadence elapses. The per-kind account sections own the transport details — Codex's app-server plus OAuth reset-credit endpoint ([codex.md](./codex.md#account-and-balance)); Claude's statusline-stale merge rule ([claude.md](./claude.md#account-and-balance)); Antigravity's verified running local service ([antigravity.md](./antigravity.md#account-and-quota)); and Pi's and OpenCode's `auth.json` token delegation ([pi.md](./pi.md#account-and-balance), [opencode.md](./opencode.md#account-and-balance)).

Transient OAuth HTTP failures — transport, body read, and 5xx responses — retry up to three attempts in-process with short backoff before surfacing. Settled auth is provider-specific: Claude treats 401 as rejected auth and keeps 403 reportable, while Codex treats both 401 and 403 as rejected auth. A surfaced failure reports off-box under one shared `oauth_usage` operation with a `provider` tag, grouping every provider into a single issue while the error detail carries the request host. Every credential-file path remains read-only: RimZ does not refresh tokens, write provider credentials, import browser cookies, or implement managed multi-account switching.

## Live cost bases

Every session cost carries one basis. `ProviderReported` values render as ordinary dollars and participate in live budgets. `LocallyPriced` values use provider-owned counters plus RimZ's pinned price book, render with `≈`, and also participate in live budgets; Cursor's response/stop accumulator is this case. `DisplayEstimate` values render with `≈` but stay out of live budget and aggregate-spend decisions; Antigravity and Droid use this case. The approximate mark therefore describes provenance, while the basis independently decides enforcement.
...
Each window carries the token split — its `↘` input folds in `cache_write`, so the `◇` total is folded input plus output, and `cache_read` rides apart — plus a `sessions` count of the distinct threads that ran in the window. The same parser path feeds [`spending::session_cost_usd`](../../../crates/rimz/src/agents/spending/mod.rs), the turn-end live-card floor for adapters whose provider transport does not push a dollar total. The spending cache retains its internal `cost_estimated` flag and maps it to `DisplayEstimate` at the live-card boundary. Droid uses that boundary for its exact-table projection and keeps fleet `transcript_files()` empty, so no display estimate reaches historical aggregation.
