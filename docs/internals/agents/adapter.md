...
| `InstallationCapability` | `managed_integration` and the install, preview, uninstall, detection, trust, and statusline-wrap surface it drives, plus `launch_dir_trust_gap` for providers that gate a launch directory ([Codex](./adapter_codex.md#directory-trust-preflight)) | hook installation is unavailable for this agent, and no launch directory is gated |
...
An adapter declares one `managed_integration`, and that single Interface drives install, preview, uninstall, detection, partial-artifact cleanup, upgrades, wiring inputs, wrapped statuslines, and installed-hook trust. Adapters whose config is a JSON merge or a whole file RimZ authors return a shared [`ManagedSource`](../../../crates/rimz/src/agents/managed_source.rs) (Claude, Droid, Qwen, Grok, Amp, Pi, OpenCode). Adapters needing a TOML rewrite or a multi-file transaction implement the same Interface in their own `install.rs` (Codex, Cursor, Copilot, Kimi, Antigravity). Copilot, Cursor, and Antigravity report their per-file rows in order through the shared [`install_report.rs`](../../../crates/rimz/src/agents/adapters/install_report.rs). Kiro is the one adapter whose Interface only uninstalls: its install and preview arms return the unavailable error, so `rimz hooks uninstall` still cleans up a legacy install while `rimz coverage` shows `install ✗`.
