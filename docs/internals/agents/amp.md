...
`agent.end` carries a three-way `status`: `done` folds to a clean `TurnEnded`, while both `error` and `cancelled` currently fold to `TurnEnded { errored: true }` and settle the row to `failed`. See [Known gaps](#known-gaps) for why `cancelled` has not yet moved to the interruption rung.
...
The plugin carries the current built-in mode, or a custom agent's explicit model, plus reasoning effort from `thread.agent()`. Amp's public plugin transcript messages carry no token usage, cost, timestamps, or stable local path, so the plugin remains the sole lifecycle and final-answer authority.
