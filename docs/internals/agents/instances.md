...
Several root sessions can claim one pane: a persistent in-process fork, a `/clear` conversation, or a provider that switches conversation ids in place. Ephemeral side conversations (`/side`, `/btw`) are quarantined at ingestion and never reach the agent fold; the fold remembers only the side id, and each side hook resolves its host at ingestion from the merged rollup (log-rotation carryover included): among the live roots on the hook's pane and agent process that no sibling supersedes, the one `compare_same_pane_owner` picks below. That hook advances the host's activity and estimated active time. A side turn opens the host's span when it is closed, and a side turn end closes it only while the host's own turn is not running. [`compare_same_pane_owner`](../../../crates/rimz/src/agents/state.rs) picks the one that owns the card:
