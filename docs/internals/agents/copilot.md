...
The cleaned `copilot_plan` survives a plan-only Business response. IDE `completions`, authoritative billing dollars, extra credits, API-key balance, official billing reports, and account-budget-eligible `AccountSpend` remain unsupported.
...
Wired now: the turn lifecycle (`sessionStart`/`userPromptSubmitted`/`agentStop`), transcript-correlated child start/stop with start model and completed exact totals, permission and batched/singular `ask_user` asks with immediate post-tool completion, mutating-tool activity and the acting-phase edge, the `preCompact` bracket, the non-recoverable `errorOccurred` marker, `sessionEnd` durable end stamping, two-file hook/statusline install and reversible uninstall, resolved-model statusline occupancy and rich session context with OTel fallback, estimated live cost, finalized shutdown history with local token pricing, interactive and prompt-seeded launch with permission-mode and model/effort presets, `--resume` restore, provider-native transcript/history/final output, the host-safe login probe, and named monthly plan quotas. Deferred: child tool/permission detail, authoritative dollar/extra-credit history, official billing reports, and remote control.
...
Subagent coverage remains partial because the child standard hooks expose only prompt and stop boundaries. Parent transcript records supply the child model at start and its exact total after completion, but child tool activity and permissions remain unavailable.
