...
The account probe reads the non-secret login identity in `$COPILOT_HOME/config.json` (`lastLoggedInUser`, falling back to the first valid `loggedInUsers` entry). The file is JSONC, so the read-only probe accepts comments and trailing commas through the shared comment-tolerant reader. It leaves `copilotTokens` unmodeled, reports a github.com login as `account_id`, and qualifies an enterprise login as `login@host`, keeping identities on different GitHub hosts distinct. A documented environment token also establishes a metered account when the config identity is absent or unreadable, without inventing an account ID.

The best-effort usage probe resolves tokens in `COPILOT_GITHUB_TOKEN`, `GH_TOKEN`, `GITHUB_TOKEN` order and hosts in `COPILOT_GH_HOST`, `GH_HOST`, then `github.com` order. It sends a bounded read to the normalized public or enterprise `/copilot_internal/user` endpoint and maps a successful response's `copilot_plan` plus Premium and Chat entitlements into independent `prm` and `cht` bars. These provider-owned readings are authoritative for window fusion, while endpoint availability remains enrichment: missing credentials and expected authentication or unsupported-endpoint responses settle quietly. The cache account key is a SHA-256 fingerprint of the normalized host and selected token, so switching either invalidates prior quota without storing the credential.

Premium and Chat are named quotas rather than inferred monthly windows. Their reported reset stays visible, but the absent duration suppresses rolling refill, not-started detection, pace, surplus, and window-priming claims. Unlimited lanes remain visible; zero-entitlement Business placeholders are omitted while the returned Business plan still labels the account.
...
Context-window fill and cost need evidence beyond the current metadata-only `chat` span. Statusline wrapping stays deferred until its input and command-chaining behavior are documented or captured. The `default_context_window` stays `None` because the window depends on the live model and the `--context <tier>` selector (`default` or `long_context`, new in 1.0.70), so a denominator requires a provider-reported resolved tier rather than an invented table. Plan and quota come only from the bounded environment-token probe; RimZ does not read keychains, plaintext `copilotTokens`, browser cookies, billing budgets, or extra-credit balances, and `AccountSpend` stays unsupported because no dollar ledger is available.
