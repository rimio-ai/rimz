...
- An **agent kind** is a wired integration (`claude`, `codex`, `amp`, `copilot`, `kimi`, `pi`, `opencode`, `antigravity`, `cursor`, `droid`, `kiro`, `qwen`, `grok`), described by an [`AgentDescriptor`](../../../crates/rimz/src/agents/descriptor.rs). Compile-time-complete integration and lifecycle coverage records own support claims; `Capabilities` carries only operational policy such as lazy registration, transcript-tail reads, realtime-usage precedence, and native ask UI.
...
- **`decode_hook`** parses one native event and returns its channel (or `Unknown`, dropped), typed root/event/context routing, optional [`AgentLifecycleObservation`](../../../crates/rimz/src/agents/observation.rs), question/answer and transcript values, identity-bearing sidecar evidence, and neutral stdout together. An absent lifecycle observation means "no transition here", so high-frequency events stay silent.
...
- **Adapters never touch the store.** The adapter is a pure mapper. [`rimz hooks feed`](../../../crates/rimz/src/cli/hooks.rs) owns transport enrichment and submits normalized lifecycle intent; Store owns transition derivation, suppression, subagent adoption and reconciliation, and ordered durable append.
