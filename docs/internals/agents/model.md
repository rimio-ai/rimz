...
| launch | `launch_id`, `profile`, `login`, `mode`, `isolation`, `role`, `team`, `channel`, `launch_group`, `launch_ordinal`, `launch_depth`, `launched_by` | Set from launch parameters, then carried. A same-instance successor conversation inherits them ([instances.md](./instances.md#launch-identity-across-conversations)). |
...
- `model` is stored canonicalized: a trailing capability tag is stripped (`claude-opus-4-8[1m]` becomes `claude-opus-4-8`). The tag rides only a fresh-launch payload, so without canonicalization the carried label would flip the first time a tagless event arrived. The event log keeps the raw payload. Root lifecycle hooks seed missing `model` and `effort` from `RIMZ_AGENT_MODEL`/`RIMZ_AGENT_EFFORT`, else the adapter's `configured_identity`, only while the session's rollup holds no value for that field; a provider-observed value is never replaced by that seed.
...
A transition off `waiting` sets `waiting_cleared`, and Store appends it even for a proof-of-work tool it would otherwise skip, so a non-mutating approved tool still clears the row on replay. A compaction open or close also clears a waiting row, because a compaction runs only after the native prompt releases the pane ([the compaction bracket](#the-compaction-bracket)). A keyed ask also clears where its own call resolved with no hook to report it: when a keyed tool event reaches a row waiting on a *different* key, the hook path asks the adapter whether the provider's transcript already holds that open call's result, and on a yes records a derived answer edge for the open key ahead of the incoming event — the ingress-side twin of the `SubagentAskAnswered` derivation above, and an adapter that cannot read its transcript abstains ([adapter_claude.md](./adapter_claude.md#asks-and-answers)). A provider interruption marker newer than `last_activity` displays a waiting row as `idle` and releases its ask, keyed or not, which is how RimZ learns that Esc cancelled a native prompt when no hook reports it: the row leaves `rimz asks` and stops reserving pane input, while the durable question record still closes on the next prompt.
