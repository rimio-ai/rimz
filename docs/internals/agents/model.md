...
**The rollup reaps its own ghosts.** A session that never captured a pid and never fired a session-end event would pin a stale row forever, and relaunch-in-place or shared-pid sessions stack duplicates. At snapshot time the derived rollup (never the event log) drops two classes, both safe under one-pane-one-row: a *pidless* session past a few-hours TTL, and an *older* same-kind session a strictly-newer one supersedes by taking over its exact pane, or, for two paneless remnants, on the same `(worktree_path, worktree_branch)`. The write path also reconstructs Codex's missing `SessionEnd`: a debounced root-session sweep scans the audit rollup for a recorded owner process known dead and appends an `Ended` tombstone, so the session leaves runtime and audit projections instead of only being hidden by read-time expel. An agent holding its own distinct pane is always kept. The rules are **root-only**: a subagent has no pane and no pid of its own, so it is never reaped alone, and children render only through their parent. This is workspace-local and complements the cross-workspace `rimz gc`.
