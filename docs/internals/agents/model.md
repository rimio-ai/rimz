...
A logical member can span several session records. Provider compaction continuations and `/clear` conversations mint fresh session ids while the same contributor keeps its seat, so attribution folds by provider kind plus the first available stable slot: team and role, launch group and ordinal, explicit name, pane id, then session id. Pane-backed children join the seat that launched them to build the origin-tagged subagent breakdown and deduplicate child continuations; an orphan whose parent has left the audit rollup is omitted. Parent seat records supply `cost_usd` and every non-launched member figure, while launched-child records supply the member's separate `launched_cost_usd` and the `rimz_launched` rows. Provider-native rows use `provider_native`, and their spend remains inside the parent's `cost_usd`.
