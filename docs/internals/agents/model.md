...
A `TurnEnded` resolves the turn to `success`, or `failed` on its error bit, never back to `idle`. One exception keeps it `running`: a clean end that also carries `parked_on_background` means the main thread *parked on still-in-flight background work* rather than finishing, so the row stays `running` in the `parked` phase and paints a distinct `⋯ bg` marker rather than a false `✓` (the provider-specific detection is in [claude.md](./claude.md#hooks-and-lifecycle)). Claude wakes a parked parent by injecting the finished background task's notification as a `UserPromptSubmit`: folded onto a parked running row, that `TurnStarted` resumes the same logical turn and carries `turn_started_at` forward, so child verdicts stay visible through the delegation wave. Once the turn reaches a clean end, the next prompt stamps fresh and clears past-turn verdicts.
