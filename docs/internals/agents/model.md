...
A **daemon-routed** hook (Codex's, fired from the shared app-server) inherits its daemon's environment rather than the pane's, so an ambient pane pin can name the daemon's launch pane and never identifies the agent session. `rimz hooks feed` drops that pane pin for daemon-owned events and separately recovers the workspace pin from the sibling agent process ([`resolve_participant_with_pin_recovery`](../../../crates/rimz/src/workspace.rs)): the daemon spawns hooks with the session cwd, so the in-pane agent process sharing that cwd carries the room identity in its environment. Each candidate is verified like the env pin and adopted only when every candidate names one root; a split scan or unsupported host degrades to the static ladder. The workspace-resolution order remains `--root`, env pin, recovered sibling pin, static ladder; the unstamped lifecycle observation then uses focused-pane recovery to bind the live pane. A same-kind provisional launch stamp remains eligible so the recovered pane id completes placeholder adoption; non-provisional stamps retain the first-event-only sole-occupied fallback for a fresh Codex conversation replacing a resting fresh conversation after `/clear`.
