...
> See [DESIGN.md](../../../DESIGN.md) for the commitments this doc operationalizes. The per-provider native mappings (which native event means what for each agent) live in the adapter docs ([claude.md](./claude.md), [codex.md](./codex.md), [amp.md](./amp.md), [copilot.md](./copilot.md), [kimi.md](./kimi.md), [pi.md](./pi.md), [opencode.md](./opencode.md), [antigravity.md](./antigravity.md), [cursor.md](./cursor.md), [droid.md](./droid.md), [kiro.md](./kiro.md), [qwen.md](./qwen.md), [grok.md](./grok.md)); the account, balance, spend, and pricing model is [providers.md](./providers.md).
...
- An **agent kind** is a wired integration (`claude`, `codex`, `amp`, `copilot`, `kimi`, `pi`, `opencode`, `antigravity`, `cursor`, `droid`, `kiro`, `qwen`, `grok`), described by an [`AgentSpec`](../../../crates/rimz/src/agents/definition.rs). Compile-time-complete integration and lifecycle coverage records own support claims; `Capabilities` carries only operational policy such as lazy registration, transcript-tail reads, realtime-usage precedence, and native ask UI.
...
A `TurnEnded` resolves the turn to `success`, or `failed` on its error bit, never back to `idle`; a provider-native `TurnInterrupted` closes the turn at `idle` without a result. One `TurnEnded` exception keeps it `running`: a clean end that also carries `parked_on_background` means the main thread *parked on still-in-flight background work* rather than finishing, so the row stays `running` in the `parked` phase and paints a distinct `⋯ bg` marker rather than a false `✓` (the provider-specific detection is in [claude.md](./claude.md#hooks-and-lifecycle)). A parked row quiet past the stall window settles to `success`; reawakened activity advances its heartbeat and makes the row `running` again. Claude wakes a parked parent by injecting the finished background task's notification as a `UserPromptSubmit`: folded onto a parked running row, that `TurnStarted` resumes the same logical turn and carries `turn_started_at` forward, so child verdicts stay visible through the delegation wave. Once the turn reaches a clean end, the next prompt stamps fresh and clears past-turn verdicts.
...
`waiting` arrives like every other status: a blocking hook (permission request, plan approval, user question) classifies as [awaiting-user](#two-hook-channels) and records an `awaiting_input` lifecycle signal, and `step` moves the row to `waiting`. The reducer stamps `waiting_since` from the signal, and the shared guard [`is_awaiting_input`](../../../crates/rimz/src/agents/state.rs) reserves pane input while that durable ask postdates activity; the same guard recognizes Codex's rollout-derived plan marker when `Stop` was missed. An activity heartbeat newer than either marker releases the prompt without waiting for a durable clear. A provider interruption marker newer than `last_activity` also releases a waiting row to `idle`, proving Esc cancelled the native prompt when no lifecycle hook reports the cancellation. A transition off `waiting` sets `waiting_cleared`, and the ingestion path appends those durably even for signals it would otherwise skip, so a non-mutating approved tool still clears the row on replay. `paused` is derived at projection ([Displayed status](#displayed-status)).

**Fail-soft, never silent.** `step` is total: an unexpected `(state, signal)` pair never panics and never freezes. It takes the signal's natural edge (the agent is authoritative about its own activity) and tags the result [`TransitionKind::Reconciled`](../../../crates/rimz/src/agents/lifecycle.rs) with the state it overrode and why. The reducer discards the tag; Store returns it in the lifecycle receipt and the ingestion path ([`cli/hooks.rs`](../../../crates/rimz/src/cli/hooks.rs)) logs it once per fresh event under `rimz::agent::lifecycle` to stderr (`warn!` on a reconciled edge, `debug!` on an ignored no-op, `error!` on a quarantined identity), keeping hook stdout for the decision channel. Drift between the model and reality leaves a structured breadcrumb instead of a wrong-but-quiet row. The headline case is in the edge table: a **tool observed on a resting row** proves the rollup is stale, so `step` moves it to `running` and logs the edge.

**Extending the signal vocabulary.** A new provider-observed [`LifecycleSignal`](../../../crates/rimz/src/agents/lifecycle.rs) variant requires both (a) a concrete native event on a shipping provider that no existing variant plus enrichment expresses, and (b) a distinct `(status, phase)` edge in `step`, landed with its edge test and the totality test extended. RimZ-synthesized side-channel markers like `Lost` are narrower: they need replay compatibility semantics and an explicit ignored edge. `CompactionEnded` is one worked example; `TurnInterrupted` is another, shared by Cursor's aborted stop and Pi's aborted settled outcome to land a canceled turn at idle without a false success or failure. Anything short of both is enrichment on an existing signal, and a `Verifying` phase still has no provider that emits one.
...
- **Subagents** own separate `agent_id`s, so a child observation never mutates its parent's phase. Providers with bracket-only identity fold `SubagentStarted`/`SubagentStopped` and keep child per-tool work on its heartbeat; Codex hooks carry distinct child identity on prompt, tool, permission, and compaction progress, so those signals fold onto the child row with rollout enrichment.
...
**Phase 1: pre-session presence.** A wired instance with no bound session yet renders as an idle agent row, so a just-launched agent reads as itself rather than a bare process. Claude reaches this at the login screen and in the short span before `SessionStart` stamps the pane; Codex and OpenCode reach it before their first real session exists; Kiro remains identity-less until its provider-owned local store yields a safe binding, while Antigravity binds on its first invocation hook or an exact local-session match. RimZ synthesizes an idle `○ <kind>` row until a lifecycle hook or local-session observation binds the real session ([`idle_agent_row`](../../../crates/rimz/src/store/snapshot/panes/lazy.rs)). Installed hooks activate hook capabilities, while the declared provider-store observation path activates session capabilities. An integration with neither active path stays a [process row](../sidebar/sidebar.md#process-rows). The spec's `registers_lazily` flag is for cwd session binding, not idle synthesis.

**Phase 2: session binding.** A lifecycle hook arrives carrying a session id, and RimZ joins it to the right instance. A standalone hook stamped the pane id, so the join is exact and free. Only a definition whose spec sets `registers_lazily` enters the unstamped recovery ladder: hook ingestion writes a recovered same-cwd pane stamp from the repaired live frame; a `codex resume <session-id>` pane binds exactly; then same-cwd sessions pair newest-first to the latest viable pane process-start before the session's first event. Residual ambiguity binds deterministically and appends a `binding.log.jsonl` breadcrumb. The ladder's guards and limits are [sidebar.md → Presence model](../sidebar/sidebar.md#presence-model).

**Same-pane ownership is an adapter policy.** `KeepPrimary` pins a pane to its earliest registered co-resident root session, which keeps Codex `/side` and `/btw` forks from repainting the primary card. `FollowLatest` hands the pane to the latest registered root conversation, using latest activity and session ID only as deterministic tie-breakers; Antigravity uses this policy for an in-place conversation-ID switch, and Cursor uses it for the hook-less `/clear` conversation switch. The policy applies only after kind, pane incarnation, process identity, cwd, and root-session guards establish one live instance. Occupied-pane recovery still prefers unique focus evidence and otherwise admits only one resting same-kind owner; running, waiting, ambiguous, already-known, wrong-cwd, and wrong-incarnation candidates abstain.

Provider-owned local stores normalize binding separately through `LocalSessionObservation`. Adapters validate and discover these typed observations; the elected room producer batches the exact admitted workspaces per kind, publishes the normalized observations, and every renderer binds only an exact session/input-matching publication against its current admitted panes. The unchanged batch adapter call may reuse a provider-owned thread-local typed snapshot keyed by resolved provider roots and the sorted, deduplicated, lexically normalized absolute inputs. File-type, length, and nanosecond-mtime stamps gate reuse of validated observations and absence; changed dependencies re-run the provider validator, topology changes rebuild its bounded catalog, and a 30-second monotonic backstop forces catalog and candidate revalidation. Stamps remain an optimization: unstable or wrong-kind inputs fail closed, while provider validators remain authoritative and snapshots retain no provider handles or private payload bodies. `fresh_binding_at` is adapter-authorized evidence for a fresh same-cwd join, while `first_event_at` remains the first real lifecycle record and supplies lifecycle time only. The observation's projection declares source authority: `IdentityOnly` proves session identity and activity bounds but defers status, phase, prompt, wait, ask, compaction, context, and lifecycle clocks to an exact durable hook row; it synthesizes idle only when adopting a provisional row or creating a session with no durable state. `Lifecycle` carries a provider-validated fold and overlays an exact durable row only when its provider activity is at least as current as durable `last_activity` at provider-compatible second precision; an accepted provider-native wait is pane-only and clears any durable routable ask. Exact pane/session binding consumes both sides before that freshness decision, so a stale fold cannot rebind the pane through a later same-cwd candidate. Exact resume identity binds first. A recordless observation requires a pane process start no later than `fresh_binding_at`, with session activity belonging to that process incarnation; without a process start, same-cwd binding requires a real first event. Authorized sessions pair newest-first, and ambiguous pane starts abstain.

**Phase 3: instance exit.** The in-pane agent process is the liveness truth, surfaced through the pane: the CLI client is the pane's foreground process or the single hosted descendant under the pane root, so when it exits the pane reverts to a shell and stops reading as an agent. The instance leaves with no exit hook, in both launch modes. A `SessionEnd` hook (Claude) stamps the durable session ended and removes its context sidecar at once; runtime views hide the row while audit views retain its provider identity for explicit resume within retention. Codex has no `SessionEnd`, so the [rollup reaper](#liveness-and-presence) stamps the same state after pane liveness proves the process gone.
...
**Session death converges to the durable log.** After a publishing commit, the debounced write-path reaper appends an `Ended` observation and stamps `ended_at` for every root session whose death is provable from the store plus the process table: a recorded owner is dead, a pidless or daemon-owned session is inactive past the ghost TTL, an older session was replaced by a different process in the same pane or by a newer paneless remnant in the same worktree, a newer fresh-lineage conversation supersedes it after `/clear` or `/new`, or a `FollowLatest` adapter reports a distinct newer ID on the exact same pane and identified live agent process. Rebirth materialization appends the same durable signal for every lost session that the accepted recovery plan does not seed, including every candidate on a declined recovery. Same-process and fresh-lineage switches keep an older running, waiting, or paused owner authoritative because a child can report a distinct conversation ID from that process mid-turn; a provably different replacement process still supersedes it. One stricter rule retires a raw-running or raw-waiting fresh root after an interrupted `/clear` or `/new`: a distinct newer fresh root must carry later activity on the exact same pane and identified agent-process incarnation, and the provider transcript must rest on an interruption marker newer than the predecessor's last activity. Paused owners stay protected, forks and daemon-owned roots fail the proof, known pane- or process-start mismatches fail it, and any later predecessor hook self-clears the stale marker by advancing `last_activity`. The plain and interrupted supersession rules bypass live-roster protection because the successor proves that the predecessor yielded the slot; dead and stale candidates retain roster protection for crash recovery. Runtime expel and the snapshot-time view reap apply the store-only liveness and shared supersession rules as latency shims during the debounce window; provider interruption evidence reaches correctness only through the durable end event. An agent holding its own distinct pane is kept, and subagents leave transitively with their parent. Already-ended rows are skipped and never supersede an active row, so repeated reaps append nothing and a retained stamp cannot retire its replacement. This workspace-local convergence complements the cross-workspace `rimz gc`.

Address resolution enforces the same physical-instance boundary during convergence: when a live pane is bound to one session, a different root stamped on that pane is a shadowed audit record and contributes no recipient to role, kind, name, broadcast, pane, prefix, or exact-session addressing. Exact addresses miss shadowed roots like ended sessions; durable history and message audit surfaces retain them.
...
A coding agent reports to RimZ through a composed [`AgentDefinition`](../../../crates/rimz/src/agents/definition.rs). [`registry::all_definitions`](../../../crates/rimz/src/agents/registry.rs) chains the compiled-in `BUILTINS` slice with validated machine-tier process plugins. Private provider modules interpret native protocols and process roles; generic hook, process, context, account, session, and runtime-control flows resolve a definition and consume provider-neutral results. The per-provider mappings are the adapter docs; the raw upstream protocols they read are the [external references](../../externals/agent-adapter/claude-reference.md), and the external process wire is [plugin.md](./plugin.md).
...
- **`correlate_subagent`** validates one child-to-parent relation from provider-owned durable evidence; **`spawned_subagents`** walks the same evidence from a completed parent turn and returns normalized child facts. Hook ingestion performs provider reads before the Store call; Store revalidates pane, lineage, and metadata guards against durable state and appends any adoption or reconciliation records in the primary lifecycle batch.
- **`observe_context`** normalizes a rich out-of-band payload into `ContextObservation`, pairing a typed session ID with identity-free [`AgentContext`](../../../crates/rimz/src/agents/context.rs); **`local_context_refresh`** derives explicit keep/set/clear and token-merge patches from local provider state on hook or producer tick triggers; **`context_refresh_spawn`** maps hook or tick triggers to a detached `rimz` helper when a provider's rich context transport needs one. Store applies each patch under the sidecar record lock and owns persistence only; provider merge policy remains in the adapter.
- **`managed_integration`** returns one Interface for install, preview, uninstall, completeness, partial artifacts, upgrades, wiring inputs, wrapped statuslines, and installed-hook trust. Shared whole-file and JSON-merge providers return `ManagedSource`; custom TOML and multi-file transactions remain provider-owned in `install.rs`. **`probe_account`** feeds display identity; **`probe_account_usage`** returns one identity-bearing normalized usage result; **`probe_realtime_account_usage`** returns the same snapshot shape without direct-query cadence; **`spending_sources`** / **`parse_spend`** feed historical spend in [providers.md](./providers.md), while **`transcript_files`** remains the live transcript/session fallback.

Two invariants hold the seam shut:

- **Store ownership stays generic.** [`rimz hooks feed`](../../../crates/rimz/src/cli/hooks.rs) owns runtime binding and submits normalized lifecycle intent; Store owns transition derivation, suppression, subagent adoption and reconciliation, and ordered durable append.
- **Generic flows do not import providers.** They consume definitions, neutral services, canonical hook facts, and command identity. Compiler privacy and `cargo xtask invariants` enforce the private `agents::adapters` boundary.
...
A **daemon-routed** hook (Codex's, fired from the shared app-server) inherits its daemon's environment rather than the pane's, so ambient workspace and pane pins can name the daemon's launch room and pane rather than the agent session. `rimz hooks feed` drops both pins for daemon-owned events and recovers the workspace pin from the sibling agent process ([`resolve_daemon_participant_with_pin_recovery`](../../../crates/rimz/src/workspace.rs)): the daemon spawns hooks with the session cwd, so the in-pane agent process sharing that cwd carries the room identity in its environment. Each candidate is verified like a pane-owned env pin and adopted only when every candidate names one root; a split scan or unsupported host degrades to the static ladder. Pane-owned participants resolve in the order `--root`, env pin, recovered sibling pin, static ladder; daemon-owned hooks use `--root`, recovered sibling pin, static ladder. The unstamped lifecycle observation then uses focused-pane recovery to bind the live pane. A same-kind provisional launch stamp remains eligible so the recovered pane id completes placeholder adoption; non-provisional stamps retain the first-event-only sole-occupied fallback for a fresh Codex conversation replacing a resting fresh conversation after `/clear`.

### From native event to internals

A lifecycle hook fires → the hook capability returns a canonical lifecycle fact → the CLI binds pane, owner, workspace, launch, role, team, and channel identity and records an `agent.lifecycle` event → [the rollup](#the-rollup) and [the state machine](#the-state-machine) own it from there.

A blocking hook fires → the hook capability returns an `AskKind`, structured ask facts, and `HookReply` → the CLI records the waiting observation when the spec declares a native ask UI, emits only the reply, and exits. The agent's UI owns the prompt; the sidebar's `?` row routes you to the pane, and you answer there.
...
**The installed config shape.** Claude, Codex, Droid, Qwen, and Grok have no wildcard event key, so install writes one block per wired event; Copilot, Pi, and OpenCode instead own one whole integration file (see [copilot.md](./copilot.md), [pi.md](./pi.md), [opencode.md](./opencode.md)). Inside that shape it stays minimal:
...
A session's context data has two origins. Both flow through the context capability on [`AgentDefinition`](../../../crates/rimz/src/agents/definition.rs) and normalize onto the same internal fields, so a new provider implements one or both and the rest of RimZ is unchanged. Enrichment is **never correctness**: a missing file, a torn line, or an absent agent each degrades to an omitted field, never a failed hook or a wrong decision.
...
Claude, Codex, Amp, Copilot, Kimi, Pi, OpenCode, Antigravity, Cursor, Droid, Kiro, Qwen, and Grok are compiled-in integrations because RimZ owns provider-specific launch, identity, hook, enrichment, or native config behavior for them. A built-in may keep hook installation unsupported while a validated provider-owned local store supplies pulled truth, as Kiro does. A third-party agent normally ships a [process plugin](../../reference/agent-plugins.md): one machine-tier manifest, native shim, and optional probes, with no RimZ source change. A new built-in remains appropriate when RimZ must own a native config migration or a protocol surface that the canonical wire cannot express; it lands as one private directory under [`crates/rimz/src/agents/adapters/`](../../../crates/rimz/src/agents/adapters/mod.rs), one composed `registry::BUILTINS` entry, conformance coverage, and its adapter doc. The step-by-step sequence — from protocol reference to landed adapter, with the deliverables checklist — is [agent-adapters.md](../../contributing/agent-adapters.md); this section owns the model-level recipe it orders.

The definition projects two conformance-checked matrices printed by `rimz coverage` (wired green, partial yellow, unsupported/absent dim, so absences are visible at a glance):
...
The hook mapping has four jobs: route each native event to a channel, map lifecycle events to canonical facts, render the agent's `HookReply`, and put tool-name vocabularies in the spec. The context read path adds two more; either alone is valid:
...
> **Neutral semantics diverge; verify per agent.** Empty stdout hands the prompt to Claude's, Codex's, and Droid's own UI, but for Pi — which has no native prompt to fall back to — empty stdout *is* the allow. Each adapter documents what its no-op means; never assume one agent's neutral behaviour for another.
