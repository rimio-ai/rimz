...
The commitments this operationalizes are in [DESIGN.md](../../../DESIGN.md). Which native event means what for each agent is that agent's own page ([claude.md](./claude.md), [codex.md](./codex.md), and eleven siblings); the account, balance, spend, and pricing model is [providers.md](./providers.md).
...
A compaction signal for a session the rollup has never seen folds to nothing. Codex compaction rotates thread ids before the replacement session is real, so the rotated id registers at its first turn or tool signal instead, and aborted compactions cannot create unreapable ghosts that steal pane primacy.
