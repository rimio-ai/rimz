...
The `afterAgentResponse` and `stop` hooks repeat per-turn token composition but expose no dollars. RimZ calculates the same API-equivalent local price from either event: a response needs no status because the event certifies a completed response, while a stop must report `completed`, `aborted`, or `error`. Explicit model IDs use the shared price book and its fast-variant multiplier. Cursor reports the literal `default` sentinel for Auto without disclosing the serving model, so Auto turns keep their tokens without a fabricated dollar value. Unknown models and incomplete pricing likewise stay absent rather than publishing a known zero.
