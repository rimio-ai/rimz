...
The agent-context sidecar stores the last priced generation and cumulative locally priced total under a per-session lock. Response-to-stop duplication is ignored by generation ID, later generations add exactly once, and statusline refreshes preserve the total. `stop` remains the sole lifecycle boundary and per-turn token projection. The `≈$` value drives the live card, cockpit add-back, and live agent/room budgets, then resets when that live session sidecar ends. Cursor still has no per-user historical usage ledger, provider-billing total, account spend, quota parser, or account-day cap.
