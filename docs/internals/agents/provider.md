...
| **Pi** | `~/.pi/agent/auth.json` (oauth → metered, api_key → unmetered) | extension response headers + `rimz pi refresh-usage` OAuth usage probe, cached under `pi` — [adapter/pi.md](./adapter/pi.md#account-and-balance) |

Claude, Codex, and Pi can refresh included windows while idle when `[accounts] oauth_usage` is enabled and their local OAuth credentials are present. Codex uses the app-server first because it is the provider's local read-only account surface; the direct OAuth endpoint is its fallback. Claude uses the direct OAuth endpoint because the statusline is live-session only. Pi selects its active backing-provider OAuth token from `auth.json` and delegates to the Claude or Codex OAuth usage fetcher, then caches the result under `pi`.
