...
**Auto-continue (opt-in).** When a `rate_limit` park reaches that lift instant the producer can resume the turn instead of letting it settle to `failed`: it types the configured nudge (`continue` by default) into the agent's live pane through the `steer` pane-send path, so the agent's next hook moves the row back to `running` on its own. The resume is armed while the park is fresh — within seconds of the park the producer records the spent window's reset deadline (`rate_limit_resume_arm`) in a durable per-agent record — and fires off that record once the deadline passes and the agent is still idle. Capturing the deadline up front keeps the resume alive across the ephemeral reading it was first seen through: a 5h/7d window outlasts the 3h context-sidecar TTL, and a post-reset reading (Codex's app-server refresh rolls the window forward) can no longer erase the pending nudge. It is enabled per machine with `[resume] auto_continue` ([configuration.md](../../reference/configuration.md#resume)), throttled per agent so one reset nudges once, and audited as a text-free `agent.resumed` event; an `overloaded` park carries no local reset window and is left to provider-side recovery. The producer-side arm/fire trigger and the `rimz agents auto-continue` helper that performs the send live in [`sidebar/enrich/auto_continue.rs`](../../../crates/rimz/src/sidebar/enrich/auto_continue.rs).
