...
- **Balance windows** — the per-duration set fused by `fresh_windows`/`fuse_window` ([Window fusion](#window-fusion)). A kind that declares no window surface but runs on a metered sibling subscription (Pi on OAuth) borrows the sibling kind's fused windows instead — same account, same bars, so the Pi block and the sibling's block read identically when both show.
