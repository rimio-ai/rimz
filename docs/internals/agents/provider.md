...
**Auto-continue (opt-in).** When a `rate_limit` park reaches that lift instant the producer can resume the turn instead of letting it settle to `failed`: it types the configured nudge (`continue` by default) into the agent's live pane through the `steer` pane-send path, so the agent's next hook moves the row back to `running` on its own. The resume is armed while the park is fresh — within seconds of the park the producer records the spent window's reset deadline (`resume_park`) in a durable per-agent record — and fires off that record once the deadline passes and the agent is still idle. Capturing the deadline up front keeps the resume alive across the ephemeral reading it was first seen through: a 5h/7d window outlasts the 3h context-sidecar TTL, and a post-reset reading (Codex's app-server refresh rolls the window forward) can no longer erase the pending nudge. A separate `[resume] auto_continue_overloaded` toggle resumes an `overloaded` park on a configurable backoff ramp whose last value repeats until the attempt cap is reached; exhaustion stops attempting and leaves the row paused. Rate-limit resumes are audited with reason `rate_limit_window_reset`, overload retries with `overloaded_backoff_retry`, both as text-free `agent.resumed` events. The producer-side arm/fire trigger and the `rimz agents auto-continue` helper that performs the send live in [`sidebar/enrich/auto_continue.rs`](../../../crates/rimz/src/sidebar/enrich/auto_continue.rs).
