...
Compaction can rotate Droid's session id while one native event both closes the old bracket and introduces the replacement. `AgentAdapter::decode_hook` carries one observation per event today, so the adapter records the close on the reported id and lets later activity establish the replacement row. Supporting a close-old/register-new pair requires a kind-agnostic multi-observation or explicit session-replacement abstraction.
