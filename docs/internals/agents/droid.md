...
- **Launch-time model and effort selection is deferred with it.** Interactive `--settings <path>` merges a process-only settings file that *can* carry `model`, `reasoningEffort`, and `autonomyLevel`. The shared exec wrapper can materialize content-addressed prompt artifacts, but `AgentDefinition` has no typed settings-file renderer or ownership contract for a provider config that mixes several launch fields. That broader typed side-file contract is the missing abstraction.
