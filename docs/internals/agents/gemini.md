...
Gemini stores one main JSONL session per project under `~/.gemini/tmp/*/chats/`. The current live hook's absolute `transcript_path` wins over the remembered sidecar path; discovery is the fallback, includes main `.jsonl` files and migrated legacy `.json` records while skipping nested subagent directories, shortlists by the filename's first eight session-id characters, and verifies the full `sessionId` in file metadata.

The active-state fold applies message-id replacement, `$set.messages` checkpoint replacement, and `$rewindTo` pruning before projecting conversation, context, and spend. History exposes visible `type: "user"` and `type: "gemini"` content with timestamps; it prefers `displayContent` over `content`, sanitizes user prompts, and omits thoughts, tool details, warnings, information, errors, and empty records.

Streaming keeps the shared byte cursor. A from-start read emits every active assistant message after a torn-write-safe full fold; later reads seed active state from the consumed prefix only when the suffix contains assistant or control records, suppress repeated-id updates and checkpoint replay, and emit newly introduced assistant ids only when they remain active after the unread page. Id-less assistant messages remain visible in full history but stay out of incremental pages because they cannot be deduplicated safely. A rewind arriving after an answer was already printed cannot retract stdout, but it does not replay that answer or emit a branch abandoned within one unread page. `AfterAgent.prompt_response` remains the supervised-run result when present, with the newest active transcript assistant as its fallback.

Token fields accept the documented names and observed aliases independently, including finite nonnegative numeric strings and truncated floats. Normalization treats current Gemini input as cache-inclusive, uses an explicit matching total to recognize the legacy cache-exclusive shape, adds tool tokens to fresh input and thoughts to billable output, and trusts an explicit total for context fill. Without a total, context derives from input, output, thoughts, and tool tokens. The newest active token-bearing Gemini message drives context, and `model` chooses the window: current Gemini families and unknown routes use 1,048,576 tokens; Gemma uses 256,000.

Local refreshes stat-gate a normally 64 KiB tail whose first record is complete; a larger newest record expands the read just enough to stay whole, valid final records need no newline, and torn fragments stay unconsumed. An unreadable transcript leaves context unknown. A readable tail without token-bearing messages publishes the shared fresh sentinel, preserving established context while a genuinely new session renders empty.
...
Session records carry token categories but no dollars. The spend fold prices normalized fresh input, cached input, and billable output through the shared price book. Rewinds and checkpoints can invalidate earlier messages, so each producer tick cold-folds the changed Gemini file and replaces its cached entry set rather than appending a suffix. The 300 ms transcript watcher and lifecycle hooks publish tail-derived context with no cost; `AfterAgent` reconciles partial realtime cost even when a watcher already stamped the newest file stat, while the producer tick remains the full-history backstop.
