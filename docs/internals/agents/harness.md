...
**End and lost traces.** Interactive agent panes keep the wrapper resident. A clean child exit is deliberate. An abrupt exit from a tab/pane close is deliberate when the mux session is still healthy, even if the room is mid-teardown or missing sidebar chrome. The wrapper records a durable `rimz.agent-ended` trace before slower cleanup, so that agent stays out of future recovery. When the wrapper exits abruptly and the mux session is unhealthy — reboot, mux crash, closing the last tab, in-`start` stuck-room recovery — it records `rimz.agent-lost`, skips worktree cleanup, and preserves recovery state so the next birth can regroup the agent into a `#<channel>` tab. The health probe uses the backend's stuck-session check rather than a bare session listing, so a wedged-but-listed Zellij server still counts as lost.
