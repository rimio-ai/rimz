...
`rimz loop fire <name>` drives the same runner path in the foreground for testing, prints the outcome and check output tail, and leaves one-shot entries and bind schedules in place. `--keep` leaves the transient supervised pane open for inspection; scheduled `loop run` always lets the run cleanup reclaim panes.
