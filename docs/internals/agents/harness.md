...
The **channel** is the workspace segment the room already groups by: an explicit named channel, else an in-place team as `<dir>/<team>`, else the worktree or directory name ([message.md § Channels](./message.md#channels)). It matches by explicit name, team channel, path basename, or full path, and defaults to the channel the command runs in; an inline `#<name>`, `--channel`, or `--worktree` overrides it. A bare directory workspace has no current channel for humans, so an address there reaches *every* channel rather than silently narrowing to one; named-channel panes carry `RIMZ_CHANNEL`, and team member panes carry `RIMZ_TEAM`. Branch names stay display metadata. Mux tab names stay display-only — they are mutable and live outside the ledger, so they never form an address.
...
The petname is the harness's stable per-instance name: the ledger mints an adjective-noun pair at registration, collision-checked against the room's live names and refusing reserved command words and kind-shaped names, so a petname can never shadow `@all` or `@claude-2`. A session recorded before petnames re-derives one deterministically from its session id, so old logs still render a stable name. [petname.rs](../../../crates/rimz/src/harness/petname.rs) owns the generator.
...
`rimz loop fire <name>` drives the same runner path in the foreground for testing, prints the outcome and failure evidence, and leaves one-shot entries and bind schedules in place. `--keep` leaves the transient supervised pane open for inspection; scheduled `loop run` always lets the run cleanup reclaim panes.
...
A task whose `spec` is a `<kind>-ping` virtual cell starts a provider's budget window at a time you choose. It defaults the launch prompt to `ping` and lowest effort unless configured otherwise, while the virtual cell supplies the adapter's effort flags, and Claude's ping pins Sonnet so a large-context account does not prime at the flagship rate. The window is account-scoped, shared by every session of a provider kind ([provider.md → Window fusion](./provider.md#window-fusion)), so one ping per provider primes the whole account. Ping turns count in spend totals, but the session spend-window detector treats them as loop-fired automation rather than human activity. Before spawning the turn, the runner reads the shared rate-limit cache and skips when the shortest window is already counting down. The read is best-effort: an unknown or cold cache falls through to the ping, since missing a window-start defeats the feature while an occasional extra token is cheap.
