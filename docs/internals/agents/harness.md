...
- A **member** is an agent, named by a **handle**: `@claude` the kind, `@planner` the role, `@swift-otter` the one running instance.
...
Open a channel with a planner and a reviewer, address them by role or kind, and drive them in either tense:
...
rimz agents list                        # the cards, grouped by channel
