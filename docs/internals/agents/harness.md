...
**End traces.** Interactive agent panes keep the wrapper resident, including resume panes and non-worktree live launches that close their pane on exit. When the child exits cleanly or the pane sends SIGHUP because its tab/pane closed, the wrapper records a durable `agent.ended` trace for the agent stamped on its pane; SIGTERM suppresses that trace so reboot, mux crash, and the in-`start` stuck-room recovery still resume. Resume birth groups those surviving agents by worktree into `#<channel>` tabs, matching a live worktree launch. A stuck-but-live recovery can race SIGHUP before the teardown's SIGTERM sweep reaches the wrapper, so a narrowly-timed pane close can suppress that agent from a later resume; the next clean launch restamps the row.
