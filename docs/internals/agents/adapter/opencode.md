...
OpenCode's whole database is one spending file, so the parser stamps each row with the table's `session_id`; `SpendWindow.sessions` counts those ids rather than collapsing the DB to one thread.
