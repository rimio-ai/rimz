...
**Partial end, idle, and live dollars.** OpenCode exposes no per-session end hook: `dispose` belongs to the embedded server, not a session, so `SessionEnd` is `Partial` via pane liveness and the rollup reaper. There is no idle `Notification` event; idle attention is reconstructed from turn boundaries, `permission.ask`, and the stall window. Live dollars are reconstructed on `session_idle`/`session_error`: Rimz reads the SQLite spend store, filters rows by `session_id`, sums the current session through the shared `parse_spend` path, and merges that sum into `AgentContext.cost`. The descriptor declares `live$` as `Partial` because the figure arrives on turn end rather than from a provider-pushed realtime transport. Plan approval, user-question, background parking, rich context, and remote control have no wired surface in OpenCode 1.15.13 and stay declared unsupported.
...
The cursor is SQLite `rowid`, stored in `SpendCursor.offset`, so incremental parses read `message` rows with `rowid > offset`. Each row's `data` JSON must carry `tokens`, `modelID`, and `providerID`. Positive `cost` is used verbatim; zero cost is priced from tokens through `PriceBook`, including cache-read and cache-write rates. Missing token splits drop the row unless `tokens.total` is present, in which case the total is attributed to output as a conservative fallback. Unknown priced models keep their tokens and session with zero dollars, are recorded for the existing pricing-refresh chase, and recover dollars on a later cold reparse.
