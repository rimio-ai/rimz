...
| workspace `observe.log.jsonl` | every node's observer writer thread ([observe.md](./observe.md)) | humans and support tools | append-only rendered-stream anomaly records with per-kind cooldown; size-capped rotation retains the current log plus one rotated generation; diagnostic only |
