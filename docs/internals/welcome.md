...
```
  ...
  Models
  ● Opus 4.8  $72,104 · ↘ 61.0m · ↗ 260.6m · ◌ 1.2b     88.2% ━━━━━━━━━─
  ● Fable 5    $4,180 · ↘  4.8m · ↗  12.9m · ◌ 210.0m    5.1% ━─────────
  ● Opus 4.7   $3,020 · ↘  1.8m · ↗   8.7m · ◌  95.0m    3.7% ──────────
  ● Haiku 4.5  $2,410 · ↘  4.7m · ↗   6.6m · ◌  88.0m    2.9% ──────────

  Agents
  ● Claude     ◎ 612 · ◇ 48B · $82,100                  87.8% ━━━━━━━━━─
  ● Codex      ◎  85 · ◇ 13B · $21,440                  12.2% ━─────────

  Sessions: 697            Spend: $103,540.00
  Active days: 27/28       Longest streak: 27 days
  Most active day: May 29  Current streak: 27 days
  Cost/session: $148.55    Daily avg: $3,834.81
...
Beneath it the panel adds the figures the heatmap implies. In the `rimzd` dashboard, the **All time / Week / Month / Year** row is a tab bar: **All time** is selected by default, and `Tab` / `Shift-Tab` cycles the active window. The active tab is a color-filled chip; the plaintext mockup brackets stand in for that fill. The active tab scopes **Models**, **Agents**, **insights**, and **Assists** below it; the heatmap stays full-history. Every window's `◇` token total folds in cache-read, so All time and Year both report the trailing-year figure the spend cache carries. Non-interactive `rimz stats` and `rimz stats --json` render All time, preserving the report shape for pipes and scripts. The **Models** breakdown ranks by selected-window spend and shares each model's slice of selected-window USD with dollars, input (`↘`), output (`↗`), and cache-read (`◌`), friendliest-name first (`claude-opus-4-8` reads as `Opus 4.8`), then lands share in a right-aligned column shared with Agents and reinforced by a thin `━`/`─` bar. The **Agents** breakdown ranks by selected-window session count and shares sessions by adapter kind — Claude, Codex, Pi, Open Code, and any future kind with recorded spend — leading with the glyphed session count (`◎`), then the cache-inclusive `◇` token total and dollars before the same share column and bar. Long tails in both breakdowns fold into `Other`. The token glyphs follow the sidebar glyph set, so `[theme] style = "modern"` and `[theme.glyphs] set = "nerd_font"` switch this panel to the Nerd Font token vocabulary while the CLI colors stay on the default palette. The **insights** report selected-window sessions, spend, cost per session, average spend per active day over that spend span, active-day ratio, heaviest single day, and longest and current active-day streaks; Week and Month append the percentage change from the immediately preceding equal span. All time keeps its recent `N/28` active-day cadence while its daily average follows its trailing-year spend. **Assists** shows non-zero category counts for auto-ping, auto-continue, pre-delivery auto-compaction, credit redemption, and crash/reboot recovery; the complete newest-first event timeline lives in `rimz stats --assists`. Tokens carry the per-model and per-agent breakdowns, so attribution is independent of pricing coverage — an unpriced model still attributes its tokens.

`rimz stats` prints the panel and returns to the shell by default, so it pipes and scrolls like any report. On a short terminal, breakdowns fold into `Other` and the wordmark yields before data; piped output stays complete. `--refresh` holds the panel open, clears and redraws it every 60 seconds, exits on Ctrl-C in a standalone terminal, and exits when its pane closes; the `rimzd` runtime view carries a supervised held live `rimz stats --refresh --hold` pane as the default middle-column content beside the runtime column's managed hosts and always-present loop panel. In that held pane, `r` reloads the binary, `rimz reload` triggers the same in-place reload remotely on Linux, and `Tab` / `Shift-Tab` switches the selected stats window without refetching spend data. The held process is eligible to host the persistent/discovery-namespace spending service and retains no separate walker; a normal one-shot answers from `provider-spending.json` or uses the bounded direct walker fallback without becoming the warm owner. Account-global runtime setup creates only the shared roots, leaving the reserved all-zero workspace tree absent. The supervisor watches the per-machine config and restarts the configured child command in place on save, so Ctrl-C leaves the dashboard and sidebar width intact while pane close still ends the process. `[daemon]` configures that middle column; pane-count changes take effect on room restart. `--dollars` scales the heatmap by spend instead of tokens — the spend view the lobby's `t` toggles to — `--assists` prints the complete newest-first assist timeline, and `--json` emits the per-day buckets, trailing windows, model and agent breakdowns, insights, and structured assist events for scripts. `--refresh` conflicts with `--json` and `--assists` because the live panel is a screen, not a report.
...
- **Read-only on the store.** Each room's agent count and attention come from its durable `snapshots/latest.json`, read in process, never a write and never a per-room producer — so the lobby stays cheap across dozens of rooms and live truth arrives only when you enter. The lobby stays out of the store-write import graph, the same boundary the sidebar holds.
- **The pace graph reads the published spend aggregate.** The spending producer's walk publishes UTC-day and per-model rollups into `$XDG_STATE_HOME/rimz/shared/provider-spending.json` with the trailing windows. The front door and `rimz stats` read that aggregate; a first-launch or old-version cache takes the shared runtime spending election and publishes the same aggregate for every later reader.
...
- It adds no daemon. It is a foreground client that reads the store and the alias store and execs an existing command; nothing of it survives the handoff.
- `rimz list` stays the scriptable, `--json` projection. The lobby is its interactive sibling, not its replacement.

## Open questions

- **Dormant-room hygiene.** Whether `x` on a dormant known room (no live session) should also archive its store record, or stay a session-only kill and leave `rimz workspace`/`rimz gc` to own store cleanup.
