...
| --- | --- | --- |
| `frame_rejected`, `frame_shrink_verified`, `pane_count_drop`, `pane_carry_forward`, `pane_carry_refuted`, `carry_forward_expired`, `duplicate_pane_id`, `focus_contested`, `foreign_session_pane`, `mixed_build_writers` | `sidebar::produce::panes` / `sidebar::frame` | held-empty frames, verified shrinks, large pane drops, liveness-guarded carried panes, a forced re-pull that refutes an initial omission, carry expiry, duplicate ids, multi-valued focus candidates, foreign-session leaks, and the build-overlap window |
| `gate_hold`, `gate_release`, `fetch_failure`, `health_alert`, `link_alert`, `producer_elected`, `producer_demoted`, `renderer_panic` | `sidebar_pane::app` | renderer-side holds and their release, degraded-refresh episodes, remote-link degraded/recovered episodes, producer handoff, and panics that would otherwise vanish with the pane |
| `renderer_signal_death` | `sidebar_pane::supervise` | an abnormal signal or non-panic exit from the render worker, with the captured stderr tail |
| `tick_budget_breach` | `sidebar::meter` | sustained over-budget producer ticks: worst in-process wall time, mux wait, fold bytes, spawns, the declared budgets, streak length, and recovery ([performance.md → The tick budget](./performance.md#the-tick-budget)) |
| `row_conflict`, `newborn_quarantined`, `group_migration` | `ledger::snapshot::view` via `sidebar::enrich` | duplicate agent-identity suppression, newborn known-command unknown-cwd quarantine, and rows moving between groups |
| `frame_anomaly` | `sidebar::observe` | rendered-stream detector verdicts (flaps, oscillations, resets, per-frame consistency violations, elder cross-checks), each carrying its detector key, evidence, frame stamp, and the writer's elder-or-consumer role |
...
Severity follows the event: an active `health_alert`, `link_alert`, or `tick_budget_breach` is `warn` and its recovery edge is `info`; `renderer_panic` and `renderer_signal_death` are `error`; the rest are `warn` for a live fault and `info` for a verified-benign transition. The mapping is pinned by test in [`record.rs`](../../../crates/rimz/src/diag/record.rs).
...
- Each signature carries scalars from the un-fused pulled snapshot (`pulled_rows`, the pulled frame stamp, and pulled dashboard aggregates), so every record shows whether the producer's published truth already held the anomaly or that instance's fusion and gating introduced it. That is the first question of any investigation, answered inside the record.
...
- **The own view is coherent.** A derived `active_pane_id` names one of the view's own `working_pane_ids` (`own_view_incoherent`).
...
- **PIDs are alive.** A row's pane pid is checked through `/proc` with the start-time pid-reuse guard (`dead_pid`). A pid must stay dead across `OBSERVE_DEADPID_CONFIRMATIONS` consecutive passes before it logs, so a just-exited process the next frame removes leaves no record, and each dead-pid episode reports once. Platforms without `/proc` skip the check.

### Roles and cost

Every renderer runs the inline detectors on its own stream, because each node fuses its own events and gates its own frames, so a flap exists only in the renderer that painted it. The elected elder's writer additionally runs the real-world cross-checks, so the room pays the `/proc` and cache reads once; the split is one policy function on the writer thread, and every record carries the writer's elder-or-consumer role. The cost envelope (one O(rows) signature pass per committed fold, a bounded channel, one throttled cross-check pass) is budgeted in [performance.md → The cost map](./performance.md#the-cost-map).
...
- The sink rate-limits per identity, the record's kind plus its salient evidence fields ([`identity_key`](../../../crates/rimz/src/diag/record.rs)), over a thirty-second window, flushing its suppressed counter onto the next record of that identity, so a steady per-tick repeat collapses to one periodic line carrying the tally.
- The observer adds a per-kind cooldown upstream (`OBSERVE_COOLDOWN`, also thirty seconds). Repeats inside it increment the `frame_anomaly` suppressed counter before the generic sink window applies, so one `frame_anomaly` line can stand for a long episode. A separate `dropped_msgs` counter on the record reports drafts the bounded channel shed under load.
...
2. **Locate the fault: published truth or local fold.** Every `frame_anomaly` carries the pulled snapshot's scalars beside the rendered ones (`pulled_rows` and the pulled frame stamp), so the record itself says whether the producer's published truth already held the anomaly or that instance's fusion and gating introduced it. The distinct `instance_id` count says the same thing from the other side.
