...
**July 3, 2026 — network cadence on `rimz-rimz-f89e49`, Zellij, 30 repo worktrees / 4 providers / attached remote SSH client.** This was a cadence and socket-shape capture, not a CPU or absolute-bandwidth capture. The producer held zero established remote TCP sockets at the idle `ss -tanp` snapshot; core egress was three bursty lanes: account-usage GETs every 60s per provider, forge PR-state probes every 5min per worktree, and a weekly pricing table refresh of roughly 372 KiB plus the unpriced-model chase. `credits.json` showed `opencode` and `pi` stuck at `"ok": false` on the retry cadence. All 30 tracked worktrees were `merged` during the window, so the five-minute forge sweep was re-confirming terminal state; the fix pins cached `Merged` verdicts and guards target selection with `merged_worktrees_are_pinned_not_reprobed`. The dominant raw bandwidth was the SSH render-stream, already attributed per pane by `rimz remote bandwidth`, and the performance map now names the weekly pricing cadence and terminal-pinned PR-state lane.
