...
- an agent row carries no activity timestamp; a `waiting`/`failed` row escalates in place instead — its `?`/`!` glyph reddens to bold red once it has gone unanswered past the 30-minute neglect window — and a working agent's animated head is its liveness cue. There is no per-row age and no global "updated" timestamp.
...
2. **Fleet header (the cockpit)** — a fixed-height summary: two lines for a populated room, one for an empty one, so the body below never shifts vertically as agents change state. Line 1 is the whole make-up on one line, every bucket shown so a zero reads `? 0` and the cockpit is a fixed dashboard scannable by position: the attention buckets — `? n` waiting, `! n` failed — lead, then the calm tail (`running` split into working `⢿` and thinking `✽` by plan mode, then `◌` idle and `✓` success), with the agent total pinned right. Line 2 is the fleet's resource totals — total time worked behind a clock glyph (`◷`) leftmost, then tokens and lines changed, with the fleet spend pinned bold to the right. A status glyph and its count are always space-separated (`? 2`, never `?2`). Counts come from `status_counts` (so they span agents hidden by a per-worktree cap); the totals sum the full agent list, so a capped agent's spend still lands in the sum. An empty (or process-only) room collapses to the single `N agents` count line.
3. **Worktree groups** — the body (below).
4. **Footer** — a dim hint for the interactive keys, centered and pinned to the bottom edge (just above the health alert): `↵ jump`, plus `␣ next ?!` and `? keys` when more than one row needs you; with a single waiting item it names the target (`↵ jump to claude`). No timestamp; freshness is the health alert's job.
...
Each agent is a small stacked card. Line 1 is *what it is* — identity plus capability — line 2 is the description, and the three aligned meter bars (context, then the 5-hour and 7-day budgets) underline them. Bare process rows (below) and the remote-control host carry no model or meters and stay a single line; a standalone script/bridge ask is an agent row, so it shows its identity and its question on two lines but no model or bars.

```
⢿ claude · Opus 4.8 (1M) · high · auto          $1.3   line 1 — glyph · name · model · effort · posture, $cost right (age shown only when not working)
  ledger refactor                          ●●●○○ 3/5    line 2 — description (session-name | task); todo pins right at wide widths
  ctx ━━━━━━━━━━──────────────────────────    200k     bar #1 — context, fills as used (color-split by where it went); value = window size
  5h↻ ━━━━━━━━━━━━━━━━━━────────────────────  3h20m     bar #2 — 5-hour budget, drains, value = reset countdown
  7d↻ ━━━━━━━━━──────────────────────────── 5d12h      bar #3 — 7-day budget, drains, green → yellow → red by remaining
  76.5k tok · ↓64.2k ↑12.3k                            selection / density — token totals: total · ↓ input · ↑ output
  ◷ 12m worked · +127 -43                              selection / density — time worked · lines added/removed
```

The resting card depends on the `[sidebar] density` setting (default `compact`): `compact` shows lines 1–3 (through the `ctx` bar), `bars` adds the 5h/7d bars, and `full` adds the token and work lines too. Selecting a row always reveals the full card, so the deepest data is one keystroke away in every density.

Line 1 — identity and capability on one line:

- **Animated cell** — the leading glyph is the row's one animated cell, driven by a wall-clock tick: a `running` agent spins a braille wheel (`⣾ ⣽ ⣻ ⢿ ⡿ ⣟ ⣯ ⣷`) while working, sparkles (`· ✢ ✳ ✶ ✻ ✽`) while thinking in plan mode, and a resolver mid-flight spins braille. The shape carries the status under `NO_COLOR`; color reinforces it. When an agent goes silent past the stall window it does not freeze — it escalates to the static `!` attention state. Every static state (`?`/`!`/`◌`/`✓`) holds still; attention cues do not jitter. A blocked `?`/`!` glyph reddens to bold red once its row has gone unanswered past the 30-minute neglect window, so a long-ignored ask escalates in place.
- **Name**, clipped with `…`.
- **Capability** — model (the statusline's display name shortened for the row, `Opus 4.8 (1M context)` → `Opus 4.8 (1M)`, else the coarser `Opus`/`GPT-5.5`), effort (`high`/`xhigh`/…), and non-default permission posture (`auto` dim, `yolo` warn-colored). These degrade by width: a wide row carries model · effort · posture, a medium row drops effort, a narrow row keeps just the name. The default posture is omitted.
- **Cost**, pinned right: the session's cost bold in money-green at a fixed one decimal (`$1.3`, so the column aligns across rows). The row carries no activity timestamp — a blocked `?`/`!` reddens past the 30-minute neglect window (see the leading cell) and a working agent's animated head already signals liveness, so a stale row escalates by color in place rather than by a right-aligned age. `last_activity` still orders the attention buckets behind the scenes (the most-overdue row sorts up); the calm buckets hold a stable spawn order, so it never reshuffles a working agent.

Line 2 — the description: the user's session name when they set one (Claude `--name`/`/rename`), else the agent's reported task, else an em dash. Display-only enrichment, redactable, never drives a decision (the no-transcript-correctness rule). On its own full-width line it has room the old shared line-1 slot lacked; at wide widths it also inlines todo progress (`●●●○○ 3/5`).

The three meter bars share one grammar so they read as one set: a 3-cell left label (`ctx`, and `5h↻`/`7d↻` carrying the reset mark), the heavy-rule bar (`━` filled over a `─` track — the weight split carries it under `NO_COLOR`), and a 5-cell right value. The **context** bar fills as the window is *used* and ramps green → amber → red; when the statusline reports the per-message token breakdown the fill splits into colored segments by where the window went (fresh input, cache writes, cache reads — the three sum to the used percentage), and its value names the context-window *size* it fills (`1M` / `200k`, whole units) — the used percent lives in the fill and color, not the value. The **5-hour** and **7-day** budget bars drain — the filled run is budget *remaining*, so a full bar means headroom and the bar shortens as the window is spent — and both ramp green → yellow → red by how much remains; their value is the reset countdown (the 5-hour floors to minutes, the weekly keeps days/hours). The bars are the rich per-session enrichment Claude's statusline and Codex's app-server feed; the budget bars render only for a plan that reports them (a Claude.ai subscription, or a Codex plan with rate limits).

Selection never reshapes the resting card: every line already on screen renders identically whether selected or not — selection only *appends* the deeper lines (and marks the row with a left accent bar `▎` in a reserved one-cell gutter), so the card never reflows on expand, in any density. The appended **token totals** (`76.5k tok · ↓64.2k ↑12.3k` — cumulative total, then `↓` input and `↑` output; falling back to the bare rollup total for an agent with no read-only split, such as Codex) and **work line** (`◷ 12m worked · +127 -43` — time worked and the agent's own lines added/removed, distinct from the worktree-total diff on the group header) read from the statusline cost and token records. There is no cumulative cached figure; the cache split is per-message, and the `ctx` bar's colored segments already show where the live window went.
...
Every enrichment follows the same grammar. The three meter bars — context, 5-hour budget, 7-day budget — share one bar row: a 3-cell left label (`ctx`, `5h↻`, `7d↻`), the heavy `━` filled run over a light `─` track, and a 5-cell right value, so all three line up on one left bar-edge and one right value-edge by construction. The context bar fills as the window is *used* and reads the context-window size it fills (`1M` / `200k`) as its value; when the statusline reports the per-message token breakdown the filled run splits into colored segments by where the window went (fresh input, cache writes, cache reads — the three sum to the used percentage), otherwise it is a single green → amber → red ramp. The budget bars drain (filled run = remaining), ramp green → yellow → red by remaining, and read a reset countdown as their value. Todo progress renders as a bounded dot shape (`●●●○○ 3/5`) pinned right on line 2, the cost pins bold to the right of line 1 in money-green at a fixed one decimal (`$1.3`), the token totals and work line are compact (`76.5k tok · ↓64.2k ↑12.3k`, `◷ 12m worked · +127 -43`), and worktree diff stats render on the group header (`+127 -43`). The cockpit's L2 totals echo this — time worked behind the same clock glyph leftmost, then tokens and lines changed, with the fleet spend pinned bold right. All are display-only and privacy-gated; a missing field means "agent did not report it", never zero, and none drives attention counts or routing.

Two axes control how tall a card is. **Width tiers** degrade line 1: a narrow row keeps just identity, a medium row adds the model, a wide row adds effort and inlines todo progress; the bars keep their label and value at every width. **Density** (`[sidebar] density`, per-machine, default `compact`) sets the resting card height: `compact` shows identity + description + the `ctx` bar, `bars` adds the 5h/7d bars, `full` adds the token and work lines. Selection is orthogonal and light: a reserved one-cell gutter carries the accent bar `▎` on the focused row, and selection only *appends* the deeper lines down to the full card — it never reshapes a line already on screen, so the card never reflows on expand. Because density rides the snapshot (the `rimz sidebar snapshot` CLI folds it from config; the reducer stays pure), the renderer reads it like any other view-model field. Color is the muted 256-color palette by default; `NO_COLOR` suppresses color only, and every bar's heavy/light shape still carries the meaning.

### View-model fields the rows use

The rows read `SidebarRow.{row_kind, name, status, permission_posture, pane, task, model, effort, context_pct, total_tokens, todo_done, todo_total, context, worktree_path, worktree_branch, last_activity, resolver, options}` plus the per-group `status_counts` and `diff_added`/`diff_removed`. `context` is the session's rich statusline enrichment (`AgentContext`: session name, model display name, cost, the context-window token breakdown, and the usage-limit windows) copied from the agent rollup; the renderer prefers it over the coarse scalars (`model`, `effort`, `context_pct`) and falls back to them when it is absent. `row_kind` is `agent` or `process`; a process row carries its command in `name` and leaves `status` unset (it never enters `status_counts`). A standalone script/bridge ask with no agent pane is an `agent` row too — it carries no model or meter, so its card is just the identity line and its question on line 2, with no bars. Agent rows always carry `context_pct`, defaulting to `0` until transcript usage is known, so every observed agent paints the same context bar from its first frame. Pane presence reaches the reducer through `PaneRef.{command, cwd}`, resolved to `worktree_path`/`worktree_branch` before grouping. Age, the attention-bucket order, the stall escalation, and the ask-recovery guard all come from `last_activity`, which the per-tool activity heartbeat advances between turn boundaries; the same heartbeat also carries the agent's latest permission slider, which `with_agent_activity` folds onto `permission_posture` as a last-sample-wins override so a mid-turn slider move repaints before the next lifecycle event. The calm-bucket and worktree-group order come from `pane_process_start`, which the heartbeat never touches. The fixed fleet header aggregates each bucket from the groups' `status_counts` (so it spans capped-away agents) and splits `running` into working and thinking by the visible rows' `permission_posture` (`plan` → thinking). Row density rides `SidebarSnapshot.sidebar` (`SidebarConfig`): the reducer leaves it default (`compact`) and the `rimz sidebar snapshot` CLI fills it from `MachineConfig`, like `project_root`.
