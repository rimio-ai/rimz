...
◕ claude  ledger refactor          12m   line 1 — status · name · session-name|task · age
  Opus 4.8 · high · yolo           $1.27  line 2 — model · effort · posture · …, cost pinned right
  ━━━━━━━━━━──────────────                context bar — full-width, color-split by where it went
  5hr ▰▰▰▰▰▰▰▱▱▱ ↻ 3h12m                   selection only — 5-hour usage window (stamina, drains)
  wk  ▰▰▰▱▱▱▱▱▱▱ ↻ 3d4h                    selection only — weekly window (mana, drains)
...
Line 2 — the capability line: model (the statusline's display name `Opus 4.8` when present, else the coarser `Opus`/`GPT-5.5`), effort (`high`/`xhigh`/…), and non-default permission posture (`auto` dim, `yolo` warn-colored); the default posture is omitted. At wider widths it also inlines todo progress (`●●●○○ 3/5`), and the session's cost pins to the right edge (`$1.27`). The context bar (next section) renders on its own thin line directly below, underlining the model name. With no capability data the capability line is dropped, but the agent still shows its identity line and bar. Selection never reshapes the ambient lines: line 1, line 2, and the bar are identical to their unselected selves — selection only *adds* a detail block below them (the two usage windows and the token split) and marks the row with a left accent bar `▎` in a reserved one-cell gutter. A calm agent with nothing extra to show keeps its exact shape when selected.

The selected detail block: the **5-hour** and **weekly** usage windows render as draining resource bars in the game-stamina idiom — the filled run is budget *remaining*, so a full bar means headroom and the bar shortens as the window is spent, with a `↻` reset countdown to its right. The 5-hour bar ramps green → amber → red as it drains; the weekly bar is a calm violet that reddens only when nearly spent. Below them the token split shows the current-context total, input, and output (`76.5k tok · ↑64.2k ↓12.3k`). All of this is the rich per-session statusline enrichment (Claude today); an agent that publishes none degrades to the bare token total. The windows render only for a plan that reports them (Claude.ai subscribers).
