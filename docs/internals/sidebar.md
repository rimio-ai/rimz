...
Sub-agent lists are planned for the card but not yet wired: a paneless sub-agent has no parent→child link in the rollup today (see [Presence model](#presence-model)), so it surfaces only as a feed notification for now.
