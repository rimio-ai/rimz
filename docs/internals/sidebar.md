...
**Degraded reads hold the last good pane list.** `list-panes` is a 200–680ms IPC round-trip that the multiplexer occasionally answers mid-tick: an empty body (covered by the zellij transient-empty retry), or a live pane missing its command — which would otherwise fold an anonymous `external`/`process` row that blinks out next tick. The snapshot treats a live pane with no command as degraded (the zellij backend first drops held/exited corpses, so a live pane always reports a command) and reuses the last good pane list from the snapshot cache, bounded by `PANE_HOLD_MAX` (~2 data ticks), while still serving the fresh rollup. The bound is the trade-off: a genuinely closed pane's row can persist up to the hold window before clearing, and a persistently degraded backend self-heals rather than freezing on a stale list.
