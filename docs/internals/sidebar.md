...
- a per-row, right-aligned age shows time since the agent's last activity — drawn only for the resting states (a working agent's animated head is its liveness cue, so it carries no age) and ramped dim → amber → red as a `waiting`/`failed` ask goes stale; there is no global "updated" timestamp.
...
2. **Fleet header (the cockpit)** — a fixed-height summary: three lines for a populated room, one for an empty one, so the body below never shifts vertically as agents change state. Line 1 leads with the attention buckets — `? n` waiting, `! n` failed — loud, with the agent total pinned right; with nothing waiting it reads a calm `✓ all clear` rather than an empty line. Line 2 is the calm tail: `running` split into working `⢿` and thinking `✽` by plan mode, then `◌` idle and `✓` success. Line 3 is the fleet's resource totals — lines changed (`+/-`) leftmost, then tokens and spend, with total time worked pinned right. A status glyph and its count are always space-separated (`? 2`, never `?2`). Counts come from `status_counts` (so they span agents hidden by a per-worktree cap); the totals sum the full agent list, so a capped agent's spend still lands in the sum. An empty (or process-only) room collapses to the single `N agents` count line.
...
- **Cost** and **age**, pinned right: the session's cost in money-green at a fixed one decimal (`$1.3`, so the column aligns across rows), then — for the resting states only — the activity age, ramped by staleness (dim under two minutes, amber, then red + bold past the 10-minute stall window) so a long-ignored `waiting`/`failed` row visibly reddens. A working agent shows no age: its animated head already signals liveness and its age is always "now"; a stalled `running` agent is projected to `!` upstream, so it lands here with its age back, in red. Age still orders the attention buckets behind the scenes (the most-overdue row sorts up); the calm buckets hold a stable spawn order, so age never reshuffles a working agent.
