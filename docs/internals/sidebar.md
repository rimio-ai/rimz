...
2. **Fleet header** — a fixed, always-present one-line summary: the agent total and a glyph+count for each non-empty status bucket (`6 agents  ⢿3 ✽1 ?2` — working, thinking, waiting, idle, success, failed). It splits the `running` total into working and thinking by plan mode, and counts even agents hidden by a per-worktree cap, so the aggregate is never lost. Because it is always one line — never omitted, never wrapped — the body below it never shifts vertically as agents appear, clear, or change state.
...
```
⢿ claude · Opus 4.8 (1M) · high · auto   $1.3  12m   line 1 — glyph · name · model · effort · posture,  $cost age right
  ledger refactor                                    line 2 — description (session-name | task), full width
  ctx ━━━━━━━━━━──────────────────────────  78.2%    bar #1 — context, fills as used, color-split by where it went
...
- **Cost** and **age**, pinned right: the session's cost in money-green at a fixed one decimal (`$1.3`, so the column aligns across rows), then the dim activity age. Age orders the attention buckets (the most-overdue `waiting`/`failed` row shows the largest age and rises) and grows toward the stall window; the calm buckets hold a stable spawn order, so age never reshuffles a working agent.
...
Every enrichment follows the same grammar. The three meter bars — context, 5-hour budget, 7-day budget — share one bar row: a 3-cell left label (`ctx`, `5h↻`, `7d↻`), the heavy `━` filled run over a light `─` track, and a 5-cell right value, so all three line up on one left bar-edge and one right value-edge by construction. The context bar fills as the window is *used* and reads the used percent (`78.2%`); when the statusline reports the per-message token breakdown the filled run splits into colored segments by where the window went (fresh input, cache writes, cache reads — the three sum to the used percentage), otherwise it is a single green → amber → red ramp. The budget bars drain (filled run = remaining), ramp green → yellow → red by remaining, and read a reset countdown as their value. Todo progress renders as a bounded dot shape (`●●●○○ 3/5`) on line 2, the cost pins to the right of line 1 in money-green at a fixed one decimal (`$1.3`), the token totals and work line are compact (`76.5k tok · ↑64.2k ↓12.3k`, `12m worked · +127 -43`), and worktree diff stats render on the group header (`+127 -43`). All are display-only and privacy-gated; a missing field means "agent did not report it", never zero, and none drives attention counts or routing.
