...
- **The value corner seals the bottom.** Below the last account block (or, with no block, its own hairline) a two-row ledger pins above the footer — this month's pile over the all-time `total` — fed by `SidebarSnapshot::value_tally` (the [`SpendTally`](../../crates/rimz/src/agents/spending.rs) the spending enrichment computes). Each row reads `label:  <tokens> tok · $<spend>`, the two sharing one token and one spend column so the labels stack and the `$` figures align on one right edge. Today's `$` stays in the cockpit, so the corner never repeats it — the windows escalate `today → month → total`. The figures **count up** on a value increase — `UiState::tally` (`render/odometer.rs`) holds one eased roll per figure, the producer-side fold seeds each roll's target on every refresh, and the renderer reads the eased value at `animation_phase`. The roll keeps the fast tick warm only while in flight (the serve loop ORs `TallyAnim::any_rolling` into its animation gate), so a settled corner costs nothing; a decrease (today's UTC-midnight reset) and the first paint snap, never roll. The figures are presentation only — the [`SpendTally`](../../crates/rimz/src/agents/spending.rs) is the truth, the roll is a transient overlay toward it.
