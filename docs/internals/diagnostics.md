...
| `lsp.log.jsonl` | machine `~/.rimz/logs/` | shared-server refusals, queue timeouts, kills, and crashes | [lsp.md](./lsp.md#visibility) |
...
The LSP log rotates at 4 MiB and carries timestamp, checkout root, server name, event, and structured details. `rimz doctor` reads its latest refusal or queue timeout for human inspection. The separate `~/.rimz/lsp-history.jsonl` is an admission input owned by `lsp/history.rs`, not diagnostics.
