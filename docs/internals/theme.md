...
`StatusSleeping` (`status.sleeping`) resolves to `☾` (U+263E), or Nerd Font U+F186 (`fa-moon_o`). The `sleeping` animation role defaults to `cool`, static, with no breathing: informational rest, not attention. It accepts the same `[theme.animations.sleeping]` overrides as other status heads.
