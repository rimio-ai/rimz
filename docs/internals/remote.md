...
`--no-reconnect` skips the master and panel. Prep, credential provisioning, and the tunnel each use a direct SSH connection; the foreground command exits when that one tunnel ends.
