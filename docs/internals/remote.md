...
SSH counts as established once its link probe receives the first ack, once its initial master is confirmed, or once the foreground attach lives past the gatetime (30 seconds by default). The gatetime is also independent evidence that the multiplexer attach ran rather than failing at launch. Both tmux and Zellij may report zero when their attached session is destroyed, just as Zellij does for an explicit detach. While a supervised remote mux client is attached, the remote RimZ wrapper polls bounded session liveness every two seconds. The watchdog arms after seeing the session live; when the session becomes missing or exited, it kills and reaps a stuck attach client and emits sentinel `68`, which the local supervisor reports as a clean room end. After a mux-client exit, the wrapper repeats the same liveness check to cover the race between the last poll and detach, overriding either a zero or nonzero client status only when the watchdog had observed the room live. Only snippets compiled for the reconnect supervisor export the marker that enables this translation; one-shot `--no-reconnect` attaches preserve the mux client's status. A failed liveness query is unknown and preserves the attach, so uncertain evidence never overrides an intentional detach. `ReconnectState` folds transport establishment and consecutive failures across sessions; `settle_zombie_kill` records an intentional kill without classifying its signal exit as fatal.
