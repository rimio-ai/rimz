...
| `RIMZ_REMOTE_SUPERVISED` | Set only when the local reconnect supervisor will interpret remote session-loss sentinels. |
...
supervise_remote
 ...
 wait_for_master ........... runs behind the connection panel
     ...
     │                       Verdict::OfferReattach → ask before recreating the room
...
| remote session-loss sentinel (`68`) | foreground attach lived past gatetime | `OfferReattach` — ask before recreating the room; acceptance reuses a live ControlMaster or recovers a dead one |
...
SSH counts as established once its link probe receives the first ack, once its initial master is confirmed, or once the foreground attach lives past the gatetime (30 seconds by default). The gatetime is also independent evidence that the multiplexer attach ran rather than failing at launch. Both tmux and Zellij may report zero when their attached session is destroyed, just as Zellij does for an explicit detach. After a successful supervised remote mux exit, the remote RimZ wrapper checks session liveness: a missing or exited session becomes sentinel `68`, while a live session preserves zero as an intentional detach. Only snippets compiled for the reconnect supervisor export the supervision marker that enables this translation; one-shot `--no-reconnect` attaches preserve the mux client's status. Because session destruction may be deliberate, a settled sentinel prompts before recreating the room; accepting reuses a live ControlMaster or enters normal link recovery when the master also ended, while declining, EOF, and non-interactive stdin return to the caller. If the liveness query fails, the wrapper preserves zero so it never overrides an intentional detach on uncertain evidence. `ReconnectState` folds transport establishment and consecutive failures across sessions; `settle_zombie_kill` records an intentional kill without classifying its signal exit as fatal.

**Terminal hygiene** wraps every session. `TtyGuard` snapshots local termios at connect, repairs a leftover raw tty at entry, and restores the snapshot after each SSH session. Before every replacement attach, RimZ puts the local tty in raw mode, writes a DSR status query (`ESC[5n`), and discards input through the terminal's status answer (`ESC[0n`). Terminals answer queries in order, so every DA1, DA2, or XTVERSION reply addressed to the dead SSH generation precedes that fence reply; DSR is used instead of DA1 because the dead client's own outstanding DA1 reply would be indistinguishable from a DA1 fence reply. A duplicate reply is what tmux otherwise passes through to the focused pane as keystrokes after the replacement client has already accepted the stale copy. RimZ reads one byte at a time and stops on the fence reply's final byte, leaving type-ahead after the hand-off byte-exact for the replacement. A terminal that ignores `ESC[5n` pays the full two-second cap on every replacement attach and loses typeahead entered during that interval before RimZ flushes the input; only a non-terminal stderr, where the query cannot be written, uses the 250 ms quiet-window drain and flush. If a terminal answers after the cap, its one status reply can reach the replacement client and tmux can pass it to the pane; the cap bounds RimZ's wait, not the terminal, and this is the accepted residual. Ctrl-C is discarded like any other byte while a fence is outstanding, so the fence still resolves in one round trip; it ends only the quiet-window fallback promptly. Keystrokes typed during an outage are discarded because no session owns them. Ordinary exit paths and the initial attach never drain, preserving type-ahead for the shell that regains the terminal and input typed while `rimz remote connect` starts; the room-recreation prompt deliberately flushes queued input before reading a fresh answer. Every session end writes the emulator reset string after releasing or holding its handoff screen, so mouse, focus, paste, and alternate-screen restoration never depends on the child's cleanup. While any full-screen guard owns the terminal, the tracing writer buffers warnings instead of writing through the frame; the panel can render their cleaned tail, and terminal restore replays the original bytes to stderr. The live `backend::tmux::remote_reconnect` suite proves the fence against a real query-producing tmux client.
...
- Restore the terminal on every exit path. Snapshot termios at connect, restore after each session, fence incoming terminal replies with a DSR round trip before each replacement attach, and emit the emulator reset after an unclean end.
