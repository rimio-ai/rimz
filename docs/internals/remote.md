...
- Restore the terminal on every exit path. Snapshot termios at connect, restore after each session, settle incoming terminal replies before each replacement attach, and emit the emulator reset after an unclean end.
