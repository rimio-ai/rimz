...
The supervisor keeps the initial attach attended and exports `RIMZ_REMOTE_RECONNECT=1` only in retry command snippets, including zombie replacements. A remote room start carrying that marker uses its unattended posture even though `ssh -t` provides a terminal: hook installation prints its non-interactive notice, project trust stays ungranted, first-run setup waits, and fleet recovery runs silently. Plain reattaches to a healthy live room skip hook detection entirely. Older remote RimZ versions ignore the environment marker, preserving command compatibility across version skew.
...
Hidden seams keep this behavior deterministic in tests: `RIMZ_REMOTE_DIAL_MS` changes the dial cadence and `0` disables both endpoint discovery and dialing; `RIMZ_REMOTE_BLACKOUT_MS` changes the probe blackout threshold. `RIMZ_REMOTE_GATETIME_MS`, `RIMZ_REMOTE_BACKOFF_MS`, `RIMZ_REMOTE_BACKOFF_CAP_MS`, `RIMZ_REMOTE_PROBE_MS`, and `RIMZ_REMOTE_PROBE_TIMEOUT_MS` tune the existing establishment, retry, and probe clocks.
