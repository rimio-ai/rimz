...
| remote session-loss sentinel (`68`) | any | `CleanExit` — report that the room ended and return to the caller |
...
SSH counts as established once its link probe receives the first ack, once its initial master is confirmed, or once the foreground attach lives past the gatetime (30 seconds by default). The gatetime is also independent evidence that the multiplexer attach ran rather than failing at launch. Both tmux and Zellij may report zero when their attached session is destroyed, just as Zellij does for an explicit detach. While a supervised remote mux client is attached, the remote RimZ wrapper polls an authoritative, bounded pane roster every two seconds. The watchdog arms after seeing a work pane and ends the attach when no tiled non-sidebar pane remains; if a sidebar-only session lingers, it removes that session before killing and reaping a stuck attach client. A missing or exited session follows the same path. The wrapper then emits sentinel `68`, which the local supervisor reports as a clean room end. After a normal mux-client exit, the wrapper repeats the same roster check to cover the race between the last poll and detach. Only snippets compiled for the reconnect supervisor export the marker that enables this translation; one-shot `--no-reconnect` attaches preserve the mux client's status. A failed roster or liveness query is unknown and preserves the attach, so uncertain evidence never overrides an intentional detach. `ReconnectState` folds transport establishment and consecutive failures across sessions; `settle_zombie_kill` records an intentional kill without classifying its signal exit as fatal.
