...
The daemon record at `$XDG_STATE_HOME/rimz/web-ttyd-share.json` carries `pid`, `port`, `interface`, optional `pixel_protocol`, and optional `index_key`; `$XDG_STATE_HOME/rimz/web-ttyd-share.lock` serializes allowlist and process transitions. A record is live only while its pid names ttyd and the listener accepts a connection. Listener or generated-index drift replaces the process before reuse. Both writable and broadcast records participate in the tmux pixel-client ancestry check.
...
`enabled` defaults to true, `interface` defaults to `127.0.0.1`, `port` defaults to 8200, and `share_port` defaults to 8201. Absent base URLs resolve to `http://127.0.0.1:<respective-port>`; a reverse proxy can set either public prefix, and RimZ appends `/?room=<session>`.
