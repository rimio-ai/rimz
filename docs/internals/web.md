...
The bootstrap waits for the embedded faces, then changes xterm's font option, clears its texture atlas, redraws, and refits the grid so the first fallback measurement cannot leave Nerd Font private-use glyphs as blocks. It keeps the cursor steady across ttyd reconnect resets, translates Shift+Enter into CSI-u before xterm collapses it to plain Enter, and intercepts Alt letter chords before macOS dead-key composition; unshifted Alt digits use the same direct Meta path while shifted digits retain xterm's symbol-aware mapping. tmux's existing extended-key binding emits the configured CSI-u or xterm sequence to the agent. An OSC 52 bridge carries tmux copy-mode yanks to the browser clipboard without answering clipboard reads, browser-native Shift-drag selections copy on selection, and the compatibility layer presents ttyd's disconnect and resize overlays as neutral dark-glass pills.
