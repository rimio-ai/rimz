...
Each cycle spawns a worker thread that loads through the elected spending service and sends back a single `RefreshEvent`; the foreground polls for keys every 100ms against a 60-second refresh deadline. A failed load becomes an event rather than an unwind, so the dashboard survives a bad read.
