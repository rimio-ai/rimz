...
The `Delegation` slot is one standing line with the lifetime child count and cost and the count of armed one-shot waits; it renders empty until either exists. `DelegationEntries` lists subagents, then pending waits, then an optional history tail. Two frame-rate rules follow from the slots: the serve loop holds the breath cadence while a visible expanded card shows the unlabeled compose affordance, and holds the fast frame grid while a visible card's PID or shell wait entry animates.
