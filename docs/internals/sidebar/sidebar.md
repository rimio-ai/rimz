...
**Child retention is turn-scoped once a child ends.** A live provider-native child or pane-backed launched child stays listed while it runs; `--keep` keeps the launched child's pane and live lifetime. After either kind finishes, it holds its `✓`/`!` verdict until the parent records its next prompt boundary. A fresh prompt, a `/clear`, and a manual `/compact` each advance that boundary and clear the prior turn's finished children; automatic mid-turn compaction resumes the same turn and keeps them. The ghost-session TTL is the backstop when the parent never records a boundary. A child whose parent row is absent or ended is an orphan and never renders.

Claude's description, cumulative token count, exact cost, and start time are paneless enrichment harvested from `subagentStatusLine`; [`with_subagent_context`](../../../crates/rimz/src/store/snapshot/view.rs) folds them onto the child before projection. A launched child instead projects its launch profile and session-sidecar cost. `AgentCard::delegated_cost_usd` sums every launched child before turn-scoped visibility filtering, so the parent figure does not fall when a new prompt retires old entries. Codex carries nickname, task path, role, model and effort, and current context tokens through child lifecycle observations, with durable registration as the elapsed-time fallback.
