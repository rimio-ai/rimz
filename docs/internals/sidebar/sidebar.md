...
| [`process.rs`](../../../crates/rimz/src/store/snapshot/process.rs) | command classification for panes no agent claims: transparent process/shell wrappers, launchers, the supervised exec wrapper, agent-kind sniffing |
...
**`crates/rimz/src/sidebar/` is the data plane.** Producer election, the published caches, the realtime overlay store, and fusion live there; the wakeup wire under them is `crates/rimz/src/wakeup/`. Both are documented in [state.md](./state.md#where-the-code-lives).
...
**No live pane, no row.** An agent with no live pane is data rather than presence, whether it is a subagent, a ghost a kill left in the rollup, or a relaunch the [reaper](../agents/instances.md#session-death) has not collapsed. It cannot resurrect a row or latch onto a stranger's pane, and there is no `offline` status.

A relaunch in place is the subtle case, because the old and new sessions claim the same pane. The [reaper](../../../crates/rimz/src/store/snapshot/view/reap.rs) collapses an older same-pane root when a newer root is a provably different process (a genuine relaunch), or when a rested owner yields to a proven same-process conversation replacement. A same-process thread fork (Codex `/side` or `/btw`) survives the reap, so pane ownership chooses the one card: an open turn outranks every rested root; adapter policy orders only co-resident open turns; and the latest `last_activity` wins among rested roots. Thus Codex keeps an open primary ahead of an open fork, but the fork takes the card when the primary rests and the fork is open or latest-touched. Every other same-pane root's activity and estimated active time fold display-only onto the bound card.
...
2. **Recovered stamp.** A daemon-routed [lazy-registering agent](../agents/instances.md#binding-a-session) starts with no pane id in its hook env, so hook ingestion recovers a durable stamp before appending the first lifecycle event. It reads the same repaired pane frame the producer publishes, filters to same-cwd same-kind panes no other live stamp owns, and writes one candidate directly. Client focus disambiguates plural candidates. Every attempt appends its probes, candidates, and outcome to `binding.log.jsonl`.
3. **Exact-cwd recovery.** An unstamped session binds the live same-kind pane whose cwd equals the session worktree exactly, refused when the session's last activity predates the pane's process start ([`pane_start_allows_bind`](../../../crates/rimz/src/store/snapshot/panes.rs)), so a stale session never latches onto a freshly started pane. This is the normal join for a daemon-routed lazy session, and the rebirth repair for a session whose stamp was cleared while its process kept running. A reborn pane whose command line resumes an exact session (`codex resume <id>`, or the resume syntax the Kiro, Antigravity, and Grok adapters parse) binds that session ahead of the heuristics.
...
- **Carried panes.** A pane a fresh read omits carries forward with a matching non-zombie process, or by identity for the renderer's own pane. A pidless sibling in a tab with confirmed liveness can also carry through tab coherence without its own process proof; proven-dead siblings do not. Carried panes are marked, publish a `pane_carry_forward` diagnostic, and expire after `PANE_CARRY_TTL` (30 seconds), except the renderer's own pane, whose carry never expires.
...
**The card's line set is chosen before any content fills it.** [`CardStage`](../../../crates/rimz/src/sidebar_pane/render/sections/agent_card/template.rs) is `Fresh { labeled }` or `Engaged`, derived only from durable lifecycle facts: status, submitted prompt, session history, context gauge, and RimZ-authored labels, never provider presentation strings. A template table maps stage, expansion, and density to an ordered list of the seven card slots (identity, description, awaiting dots, gauge, tokens, `Delegation`, `DelegationEntries`). `Delegation` belongs to engaged cards and renders empty until the session has spawned a child or armed a one-shot wait; it shares the child count/cost and wait count on one standing line. Expansion appends `DelegationEntries`: subagents, then pending waits and an optional history tail. Snapshots retain finished prior-turn children tagged `prior_turn`; `DelegationEntries` filters them out unless the delegation toggle is open. The muted tail reads `  +K more` for K hidden prior-turn children and `  − less` while open, and is absent when there is no history. K counts tagged entries, not the lifetime count minus visible entries. The compact resting template trims both with the other stats. Status enters the table on one arm only, the resting compact card, which trims by status; every other arm reads stage, expansion, and density alone. A unit test pins all four dimensions across every combination. Enrichment only fills the chosen slots, so a card cannot grow a line because a provider started reporting a field.

`[theme.display] card_density` picks which arm of that table applies. `auto` uses the standard shape and puts entries on cards expanded by selection or the delegation toggle, `expanded` puts entries on every engaged card, and `compact` trims resting cards by status, including `Delegation` and `DelegationEntries`. Selecting a named-team member feeds the same expanded shape to every visible teammate, while selection styling remains exclusive to the focused row. Under auto and expanded, expansion only appends the deeper lines, so the card never reflows. Compact is the scoped exception: a resting compact card is shorter, and expanding it reflows up to its stage's full shape. A fresh expanded card opens the authored description or compose affordance plus the empty context meter, and the serve loop holds the breath cadence while any visible expanded card carries the unlabeled compose affordance. A visible card whose template includes PID or shell wait entries holds the fast frame grid while its Working animation moves; collapsed or filtered-out wait entries do not keep that grid awake.

**Delegation expansion has two sources.** Selection (including visible named teammates) or expanded density shows current entries; the renderer-local `UiState.expanded_delegations` toggle also reveals prior-turn history, independently of selection. Each map entry stores the row id and the parent's `user_turn_started_at` when opened. A changed turn stamp makes it inert immediately, and pruning removes it when the stamp changes or the row disappears. Both the delegation summary and history tail carry `HitTarget::ToggleDelegation`; clicking toggles the map entry, anchors selection and pins manual scroll, but never focuses a pane. Closing the toggle restores the selection/density view rather than hiding entries that view still requires. The next user-authored prompt expires the toggle; automatic deliveries and other-agent messages leave it intact.
...
The expanded card lists both provider-native subagents and pane-backed agents launched by another agent. Neither renders as its own row while its parent launch has a rendered row: the rollup carries each child's root `parent_agent_id` and the snapshot nests every descendant in one flat list at projection time. A launched child's link names its parent's launch, a provider-native child's names the parent session, and projection resolves both with [`AgentState::parent_is`](../../../crates/rimz/src/agents/state.rs), so mixed links coalesce under one card before the counts and costs below are computed. Cross-provider launched children also carry the parent's provider kind so the pair resolves to the correct card. The list orders by creation time ascending (`registered_at`), stable across refreshes, while Codex's root-relative task path preserves nested lineage in the label. A launched child's pane remains an addressable agent pane, but projection suppresses the redundant process row.

**Child-entry retention is user-turn-scoped once a child ends.** A live provider-native child or pane-backed launched child stays listed while it runs; `--keep` keeps the launched child's pane and live lifetime. After either kind finishes, it holds its `✓`/`!` verdict until the parent records its next user-authored prompt boundary (`user_turn_started_at`). A prompt RimZ delivers on an agent's or the harness's behalf (`AGENT_MESSAGE`, `SUBAGENT_REPORT`, `WAIT`, `SIGNAL`) opens a provider turn (`turn_started_at`, still used for budgets, delivery, and status) but keeps the entries. Classification reads the durable submitted prompt: every segment must have a non-user message header to count as harness-delivered; `USER_MESSAGE`, leading bare composer text, or a missing prompt counts as the user's. Text appended after a pasted header's body is indistinguishable from that body and remains harness-delivered. Headerless `--no-from` text and the budget-continue prompt are therefore user prompts too. A launched child whose session is observed to end mid-turn (its deadline fired, `rimz subagents stop` reached it, or its provider died) resolves to `failed` and holds `!` ([model.md](../agents/model.md#the-state-machine)); a reaper-inferred end of a still-active child rests the row at `idle`, where it wears the calm `○` instead ([provenance](../agents/model.md#observed-ends-and-reaped-ends)). Either way its elapsed time freezes at the end stamp, because the clock runs to `now` only for a child still in `running`. A fresh user-authored prompt, a `/clear`, and a manual `/compact` each advance that boundary and clear the prior turn's finished entries; automatic mid-turn compaction resumes the same turn and keeps them. The ghost-session TTL is the backstop when no user-turn boundary is known: finished entries then expire after three hours of child inactivity, including when old carryover predates `user_turn_started_at`. A known user-turn stamp survives subsequent log rotations; automatic deliveries alone do not create one. A child that no rendered row of its parent launch claims is an orphan: a pane-backed launched child is promoted to its own top-level row, because its pane is live and would otherwise render nowhere, while a paneless provider-native child does not render and is logged instead ([subagents.md](../harness/subagents.md#who-counts-as-a-launched-child)).

Before user-turn-scoped entry filtering, projection de-duplicates all children retained by store GC and computes `sub_agent_count` and `sub_agent_cost_usd` across provider-native and launched origins. It computes the launched-only `delegated_cost_usd` alongside them; that field is the child spend added to the parent's own session cost, while the all-origin cost is only a breakdown because native child spend is already in the parent transcript.

Claude's description, cumulative token count, exact cost, and start time are paneless enrichment from the runtime `subagentStatusLine` feed; [`with_subagent_context`](../../../crates/rimz/src/store/snapshot/view.rs) folds them onto the child before projection. A launched child instead projects its launch profile and session-sidecar cost, and its token headline prefers the cumulative `session_usage` displayed total: input, cache writes, and output, excluding cache reads. A Codex-native child carries nickname, task path, role, model and effort, and current context tokens rather than cumulative usage through child lifecycle observations, with durable registration as the elapsed-time fallback.

**Child activity counts as the parent's.** At projection ([`fold_child_activity_onto_parents`](../../../crates/rimz/src/store/snapshot/view/aggregate/subagents.rs)) the freshest child `last_activity` advances the parent row's displayed clock, so the card's age stays honest and the stall check never false-fires while children work. A live child, one whose own status is still `running`, also makes a clean resting parent display running or delegating; a child holding a terminal verdict never does, so a retained ended child cannot keep its finished parent reading running. The fold is display-only and guarded: the rollup keeps the parent's own clock, a blocked or budget-parked parent stays put, and pause or failure evidence beats a still-ticking child. Cases are pinned in [`view/tests/subagents/`](../../../crates/rimz/src/store/snapshot/view/tests/subagents).
...
The mux command supplies the ordinary label through the shared `proc::command` parser. It sees past environment assignments and transparent wrappers (`sudo`, `doas`, `exec`, `command`, `nohup`, `nice`, `time`, `timeout`, `stdbuf`, `env`, and `sh`/`bash`/`zsh`/`dash`/`fish` with `-c`), then projects a `node`/`nodejs`/`npx`/`bun` script or supervised `rimz agents exec` kind while retaining that launcher as the identity-bearing root. Shell scripts can be quoted or flattened process argv: selection is lexical, taking the first simple command other than assignment-only or `cd`/`export`/`set`/`source`/`.` setup. It does not evaluate expansions, redirections, or control flow; unsupported shell keywords or punctuation, incomplete wrappers, and exhausted nesting retain the wrapper. Source spans let command details shorten only the resolved binary's absolute path without changing arguments or quoting. A shared-runtime basename such as tmux's bare `node` resolves to an agent label only when the producer proves a known CLI from one bounded root-to-single-child process-chain walk over full `/proc` command lines. An unreadable, branching, startless, depth-exhausted, or unclassified chain keeps the runtime label. The proof never rewrites the mux command. Comm-only muxes are probed even for idle-looking shells; their matched `foreground_cmdline` drives process-row activity and detail, never identity.
...
**Pane width is a mux operation dispatched off the render thread.** [`mux/width.rs`](../../../crates/rimz/src/mux/width.rs) owns the pure share, step, and stop-band math used by both convergence engines; [`mux/width_target.rs`](../../../crates/rimz/src/mux/width_target.rs) owns the persisted room-wide record, its resolve/pin operations, and the broadcast renderers and room options consume. One room-runtime share always exists: unpinned policy follows the live view and configured percentage/cap, while `a`/`d` and mouse drags pin the selected proportion of the view ([multiplexers.md](../multiplexers.md)). Every renderer resolves that share against its own geometry, rounds fractional columns up, and converges to the nearest backend-reachable width with one nudge in flight and a bounded number per stall when the backend stops answering; its own late-landing nudges are never mistaken for a drag. After a settled native resize, unchanged view width and sibling count identify a user drag and publish its exact measured share once; view changes re-render either policy or a pin at the new scale, structural sibling changes converge without adoption, and missing geometry never pins. Zellij fullscreen parks this controller until topology reports fullscreen cleared, so override geometry is never mistaken for a user resize.
...
- **The focus key reaches the sidebar from any pane.** `[sidebar] focus_key` (default `Alt+p`) toggles `rimz sidebar focus`: tmux binds it at session birth, and Zellij runs it from the presence plugin ([multiplexers.md](../multiplexers.md#room-keys)).
...
A producer frame that omits the sidebar's own pane is re-pulled through the mux's `PreferAuthoritative` seam before it feeds the sibling count, so a stale plugin roster cannot leave self-close parked in an unknown state.
