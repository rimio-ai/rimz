...
**Child-entry retention is turn-scoped once a child ends.** A live provider-native child or pane-backed launched child stays listed while it runs; `--keep` keeps the launched child's pane and live lifetime. After either kind finishes, it holds its `✓`/`!` verdict until the parent records its next prompt boundary. A fresh prompt, a `/clear`, and a manual `/compact` each advance that boundary and clear the prior turn's finished entries; automatic mid-turn compaction resumes the same turn and keeps them. The ghost-session TTL is the backstop when the parent never records a boundary. A child whose parent row is absent or ended is an orphan and never renders.
