...
The pet overlay is best-effort enrichment. When a sprite grid is available, the active provider block narrows and the pet column zips onto its right edge. `size = "medium"` keeps the original pet footprint; `size = "small"` fits the sprite to the active block height. With pets enabled, the provider side uses the taller spend layout: `T:` reads the active provider's trailing-24h spend, `Total:` marks the scope change, and `W:`/`M:` read the account-global fleet totals. The pet caption rides in the tab rail's spacer above the sprite. Under `NO_COLOR`, and on panes too narrow or short to afford the sprite without crowding the provider block, the sprite body drops out and the same spend table uses the full provider block width.
