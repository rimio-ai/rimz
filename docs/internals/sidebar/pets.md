...
Both bits stay off under Zellij because Zellij 0.45 does not accept the Unicode-placeholder placement the pixel tier uses. `rimz doctor` still probes Zellij's kitty graphics support separately (`probe_zellij_kitty`, 0.45 minimum), but the renderer does not consult that result. Its `zellij kitty graphics` row is informational: `cli/doctor/render.rs::push_capabilities` reports a supported probe as "supported by the host terminal; Zellij rooms still draw cell art (Zellij rejects the placement the pixel tier uses)", not a success verdict or a promise of pixels.
