...
| `pane-topology.json` | Zellij presence plugin via the host CLI | Zellij producer pull | Zellij's pane roster — live panes, tab names, foreground commands, geometry, attached-client count, terminal `viewed_panes`, and an optional session `focused_pane` derived from the full client map; pane entries carry no focus marks |
