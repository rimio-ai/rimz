...
| `codex-daemon-reap.json` | Room | The Codex daemon-mode reap inputs: live daemon PIDs and the app-server's loaded-thread list, stamped `produced_at_ms`. The producer re-probes on a 30-second TTL (`CODEX_DAEMON_REAP_TTL`) and every reader — the enrich fold and the consumer's cached reap — accepts a publication only within that TTL; an absent, unreadable, or stale record reaps nothing and reports Codex daemon health unknown, so a stopped producer degrades to keeping every session rather than reaping on frozen evidence. |
