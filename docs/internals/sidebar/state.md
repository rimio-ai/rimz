...
- **The elder's transcript watcher** ([`transcript_watch.rs`](../../../crates/rimz/src/sidebar_pane/app/transcript_watch.rs)) watches each live session whose adapter declares `transcript_tail_context`, including Codex and Gemini, and runs the stat-gated Watch refresh on write to cover mid-turn gaps between progress hooks. Only the elder watches; demotion drops the watch, Watch refreshes stay on the tail-only latency path, and a watcher that never starts costs nothing because the producer-tick refresh stays unconditional.
