...
| --- | --- | --- | --- |
| `PaneClosed` | `pane_id` | Delete every row bound to the pane (highest precedence) | shared host presence projector from Zellij topology or tmux control mode |
...
| `FocusChanged` | previous / current session pane ids encoded as unfocused / focused ids | A current pane updates `SidebarSnapshot::focused_pane` and appends it to `viewed_panes`; a transition to no current pane clears the named prior register; rows carry no mirrored focus bit | shared host presence projector from client-derived Zellij focus or tmux pane/view focus |
...
| `FocusStranded` | owning sidebar `pane_id` | Renderer focus-repair action only, dropped past the short `FOCUS_STRANDED_EVENT_TTL` so late delivery cannot yank focus: the matching renderer validates its baseline as a live visible work sibling or chooses the deterministic leftmost sibling; distinct client views leave focus alone because `focus-pane-id` is session-global | shared host presence projector from a Zellij settled generation or tmux window switch |
| `PaneOpened` | `pane_id`, optional `command` | Nudge a producer verification pull; never admits a card on its own | shared host presence projector |
| `PanesChanged` | none | Nudge a producer pull — topology moved, identity unknown | shared host presence projector fallback or incomplete-layout input |
