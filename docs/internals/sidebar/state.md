...
| `lanes/pipeline.json` | Room | Version 2: stage lists, board stage and owner, open-visit timestamp (`stage_started_at`), prior visit seconds (`stage_prior_secs`), entered-stage set (`visited`), and run start/stop timestamps (`started_at`, `done_at`) by group key. Published by `refresh/pipeline.rs` every refresher pass only when content changes, using `write_temp_then_rename_cache`; a version mismatch reads as empty. Projected onto `SidebarWorktreeGroup.pipeline`. |
