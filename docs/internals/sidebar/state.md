...
| `pr-state.json` | room | producer-only `gh`/`tea` pull-request state, number, and best-effort open-PR CI verdict by worktree path, plus per-repo probe stamps, path-to-repo metadata, and last-seen HEAD SHAs; GitHub supplies CI in the existing `statusCheckRollup` list field, while tea reads Gitea's combined commit status through `tea api`; a pending verdict keeps the repo on the hot refresh TTL; absent when no PR or unsupported forge |
