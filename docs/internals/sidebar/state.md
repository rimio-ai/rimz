...
| `pane-topology.json` | Zellij presence plugin via the host CLI | Zellij producer pull | a pre-producer Zellij roster hint — live panes, tab names, authoritative per-tab active panes, raw focus candidates, geometry |
...
`SidebarSnapshot::panes_observed_at_ms.or(panes_produced_at_ms)` is the supersession baseline. An event no newer than the pane observation is skipped — the pull already saw later pane truth. A plugin-resolved work tab trusts `active_panes` over raw `is_focused` churn and is not focus-contested; a fallback frame whose own view remains focus-contested abstains from superseding `FocusChanged` events that name panes in that contested view.
