...
| Module | What it owns |
| --- | --- |
| [`produce/`](../../../crates/rimz/src/sidebar/produce/mod.rs) | The producer read. `panes.rs` assembles and publishes the pane frame behind a single flight, `metrics.rs` samples per-pane `/proc` and owns the Zellij pid backfill walk, `git.rs` enumerates worktree roots. |
...
After cached git facts finalize branch labels, the same fold stamps `SidebarWorktreeGroup.label_qualifier` only for colliding checkout labels, using the shortest distinguishing trailing path suffix. Snapshot schema version 16 carries the launch identity, cross-provider ancestry fields used by pane-backed children, and the provider-native marker that lifts paneless child attention; older persisted projections are rejected by the existing version gate.
...
| `codex-daemon-reap.json` | Room | The Codex daemon-mode reap inputs: live daemon PIDs and the app-server's loaded-thread list, stamped `produced_at_ms`. The producer re-probes on a 30-second TTL (`CODEX_DAEMON_REAP_TTL`); every reader — the enrich fold and the consumer's cached reap — accepts a publication for up to 90 seconds (`CODEX_DAEMON_REAP_STALE`), allowing replacement probes to overlap the previous evidence. An absent, unreadable, or stale record reaps nothing and reports Codex daemon health unknown, so a stopped producer degrades to keeping every session rather than reaping on frozen evidence. |
| `workspace-spending.<hash>.json` | Room | The room cockpit spend tally, headline cutoff, and the live-card session keys excluded from walked headline USD. A consumer reads the publication addressed by its own scope hash and requires the record's `scope_hash` to match; when that misses (its cached roots lag the producer's), it serves the sole surviving publication whatever its hash, since the producer prunes every sidecar but the live one; two candidates or none leave the tally absent. |
| `cohort-spend.json` | Room | Lifetime dollars, token split, and retained active time for each finished multi-agent group, plus each durable seat's lifetime dollars and tokens keyed by agent-row id. The producer refreshes only collapsible groups on a 60-second TTL, folds the audit rollup through the shared cross-session effort aggregator, and projects the cached result onto `SidebarWorktreeGroup::cohort_effort`; renderers never parse transcripts. The fold admits only records born in their checkout's current lifetime ([attribution.md](../agents/attribution.md#selecting-records)), and cache version 3 carries that change so a receipt cached under version 2 is discarded rather than served. An unreadable checkout contributes no seat totals; the producer logs the omission and publishes the remaining figures. |
...
`rate_limits.json` is the single fused source of truth for every sidebar and `rimz providers` reader. Every writer, including out-of-band OAuth usage reads, fuses by stable window identity before publishing. The newest authoritative reading anchors a window: it supersedes any prior stamped no later than itself, climb or drop, and an unstamped prior yields to it, while an authoritative reading carrying no stamp of its own cannot prove it is current and holds. A best-effort reading overlays that anchor only when it is stamped strictly later, so equal stamps keep the authoritative value and an unstamped best-effort reading holds. Between best-effort readings the older rules stand: a climb is adopted at once, a reset-timer advance proves a new epoch and lowers the bar immediately, a drop at or below the reset floor is held until the low reading has stood for two minutes, and a mid-range drop reads as jitter and holds the most-drained prior. Consumers share these rules but never confirm a best-effort refill on their own. One disagreement survives this: while a live session keeps reporting a fresh reading that contradicts the provider's API for the same account, admitted readings can alternate the bar between the two on direct usage polls. The ordinary cadence is five minutes while the probe is answering, but a newly parked refill can request an earlier read even after the API rejected a previous suspected refill. That is upstream disagreement RimZ cannot adjudicate, and the authoritative value is never more than one poll away. An exact-account entry may additionally retain its authoritative windows for bound managed-launch controls; that copy is not read by the sidebar or `rimz providers`, so a scope-only live reading cannot acquire the cached credential identity.

Scoped readings are never refilled by display projection: an elapsed reset makes a named quota or model sub-cap unknown until the provider reports it again, even when the sub-cap carries its parent's duration. Unscoped temporal windows retain their reset-to-max roll-forward. A sub-cap's scope keeps its fusion identity separate from the same-duration parent. Its tick position and tone use its own remaining percentage across the full bar width, independent of the parent's fill: `min(filled_cells(remaining, width), width - 1)` with nearest-cell rounding. The tick can replace fill or empty track, including on exhausted or forced-exhausted real bars. An unknown sub-cap reading, unknown parent row, or lifted unlimited row draws no tick. The parent label and countdown are unchanged, and no row is added.
...
- **Forge transitions become signals.** Each PR-state publication is diffed against the one before it ([`refresh/pr/transitions.rs`](../../../crates/rimz/src/sidebar/refresh/pr/transitions.rs)): a link leaving the open state yields `pr.merged` or `pr.closed`, and a CI verdict settling yields `ci.passed` or `ci.failed`, for a linked PR or for a tracked branch without one. Each carries `path`, `branch`, and `repo`, plus the link's `number`, `url`, and `state` where known, and the observed `head` with a `checks_url` built from the target's `RemoteRepo` (the GitHub `/commit/<head>/checks` page, the Gitea commit page), never from the payload's `repo` cache key. A transition counts only when the same target stamp owns both readings and its repo probe succeeded, so a branch switch or a failed probe emits nothing. The producer stays read-only about it: it spawns a detached `rimz events emit --source forge`, which does the durable append and fires subscribed loop and wait tasks ([loops.md](../harness/loops.md#the-signal-vocabulary)).
- **The elder's cache refresher** ([`cache_refresh.rs`](../../../crates/rimz/src/sidebar_pane/app/cache_refresh.rs)) ticks on the data cadence, rechecks the election each pass, refreshes the heavy caches from the last published pane frame, fires due loop tasks for this room, including the watch-lost backstop for a `rimz wait` watcher that died, and wakes due scheduled messages. Demotion turns it into a sleeper and drops no account-global spending state, because the separately elected service owns the sole warm walker. A panic resets only the refresher's rollup cursor, and the next tick retries from cache truth.
...
| Unreadable store | The serve loop holds its last committed frame. | Sustained failure raises the sticky health alert; a producer recovers only on a completed produce, while a consumer recovers on its next successful published read. A renderer degraded past `GIVE_UP_AFTER_DEGRADED` exits for supervisor respawn ([sidebar.md → Degraded reads and give-up](./sidebar.md#degraded-reads-and-give-up)). |
