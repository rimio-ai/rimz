...
One **producer** — the eldest fresh heartbeat per workspace — owns every consistent-cadence external pull and publishes each lane as its own single-writer cache. Two producer threads split that work. The fetch worker publishes pane truth, provider-local session observations, and group roots each data tick, then projects the heavy caches' last published values into its snapshot. The elder-gated cache refresher owns git diff-stats, PR state, accounts, usage, credits, and auto-continue side effects; it requests spending from the separately elected user-scoped warm service, which can live in any existing RimZ process and survives workspace producer demotion. Every other node consumes the published caches in process and never pulls for freshness on its own. Realtime events never patch a published cache — pulled truth on disk is written only by producer pulls. A dead workspace producer or spending owner is a degradation like any other, handled by its corresponding election.
...
Producer **enrichment lanes** fold onto the admitted cards. The fetch worker handles process metrics and group roots with pane production; the cache refresher handles git, PR state, accounts, usage, credits, and auto-continue and asks the user-global service to refresh spending. Durable provider and workspace publications remain the figures every fast lane and consumer folds.
