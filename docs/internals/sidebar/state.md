...
| `local-sessions.json` | producer ([`sidebar::local_sessions`](../../../crates/rimz/src/sidebar/local_sessions.rs)) | every node's fold | provider-validated observations keyed by the exact session name and sorted admitted `(agent kind, absolute workspace)` inputs; consumers accept only an exact input match and otherwise bind no observations; semantic changes publish atomically and wake every renderer |
