...
- **Forge transitions become signals.** Each PR-state publication is diffed against the one before it ([`refresh/pr/transitions.rs`](../../../crates/rimz/src/sidebar/refresh/pr/transitions.rs)): a link leaving the open state yields `pr.merged` or `pr.closed`, and a CI verdict settling on success or failure yields `ci.finished`, for a linked PR or for a tracked branch without one. Each carries `path`, `branch`, and `repo`, plus the link's `number`, `url`, `head`, `state`, and `conclusion` where known. A transition counts only when the same target stamp owns both readings and its repo probe succeeded, so a branch switch or a failed probe emits nothing. The producer stays read-only about it: it spawns a detached `rimz events emit --source forge`, which does the durable append and fires subscribed loop and wake tasks ([loops.md](../harness/loops.md#the-signal-vocabulary)).
