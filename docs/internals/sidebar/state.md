...
| `pr-state.json` | Room | Producer-only `gh`/`tea` pull-request state, number, and best-effort CI verdict by worktree path, with each link stamped by its PR head branch plus per-repo probe stamps, path-to-repo metadata, merge SHAs, and last-seen HEAD SHAs. A branch switch invalidates the path's old link and resolves the new head branch; trunk checkouts stay outside the probe set. Open PRs use the head-check rollup; merged PRs use the forge-reported merge commit and fall back to the PR head verdict when merge checks are absent. A pending verdict keeps the repo on the hot TTL. Absent when there is no PR or the forge is unsupported. |
