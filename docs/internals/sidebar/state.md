...
| `pipeline.json` | Room | Versioned stage lists, board stage and owner, and run start/stop timestamps by group key. Published by `refresh/pipeline.rs` every refresher pass only when content changes, using `write_temp_then_rename_cache`; a version mismatch reads as empty. Projected onto `SidebarWorktreeGroup.pipeline`. |
...
| Team pipeline | Every refresher pass, no TTL; content-identical writes suppressed | Board stage, owner, and run clock |
