...
| `pane-topology.json` | Zellij presence plugin via the host CLI | Zellij producer pull | Zellij's pane roster — live panes, tab names, the plugin-resolved session focused pane, attached-client count, viewed panes, raw focus candidates, foreground commands, and geometry; each fresh manifest is authoritative for the active tiled focus mark, so the plugin reconciles its session register and re-emits a missed `FocusChanged` when they drift |
