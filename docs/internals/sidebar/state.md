...
| `agent-wiring.json` | producer ([`sidebar::agent_wiring`](../../../crates/rimz/src/sidebar/agent_wiring.rs)) | every node's fold | normalized admitted kinds and default launch models for the exact mux session; the producer memo keys one cold adapter probe by sorted resolved provider paths plus exact presence/mtime/length stamps, publishes and wakes only on semantic change, and retains the last stable projection across a raced edit; missing, malformed, and wrong-session reads fail closed, and consumers never inspect provider configs |
| `local-sessions.json` | producer ([`sidebar::local_sessions`](../../../crates/rimz/src/sidebar/local_sessions.rs)) | every node's fold | provider-validated observations keyed by the exact session name and sorted admitted `(agent kind, absolute workspace)` inputs; the producer checks each represented kind on every data tick, while provider-owned typed snapshots make unchanged checks metadata-only and force full validation every 30 seconds; same-session consumers bind the safe intersection of published and current inputs, so unchanged observations survive membership edits, newly added inputs wait for publication, and removed inputs disappear immediately; semantic changes publish atomically and wake every renderer |
...
| `FocusChanged` | previous / current session pane ids encoded as unfocused / focused ids | A current pane updates `SidebarSnapshot::focused_pane` and appends it to `viewed_panes`; a transition to no current pane clears the named prior register; rows carry no mirrored focus bit | Zellij plugin client-derived transitions, tmux watch (pane-active and non-stranded window switch) |
