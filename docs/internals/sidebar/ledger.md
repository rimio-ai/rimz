...
- A resolution takes the workspace lock, then CAS on `status = pending`, validating `workspace_id`, `request_id`, and nonce. The first valid writer wins (`writer/resolve.rs`).
