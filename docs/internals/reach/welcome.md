...
- **The pace graph reads the published spend aggregate.** The spending producer runs `compute_daily_spend` and `compute_model_breakdown` beside the provider-dashboard walk, publishing those rollups into `$XDG_STATE_HOME/rimz/shared/provider-spending.json` with the trailing windows. The front door and `rimz stats` read that aggregate; a first-launch or old-version cache takes the shared runtime spending election and publishes the same aggregate for every later reader.
