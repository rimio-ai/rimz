...
```
  ...
  Models
  ● Opus 4.8  $72,104 · ↘ 61.0m · ↗ 260.6m · ◌ 1.2b     87.8% ━━━━━━━━━─
  ● Fable 5    $4,180 · ↘  4.8m · ↗  12.9m · ◌ 210.0m    4.8% ──────────
  ● Haiku 4.5  $2,410 · ↘  4.7m · ↗   6.6m · ◌  88.0m    3.1% ──────────
  ● Opus 4.7   $3,020 · ↘  1.8m · ↗   8.7m · ◌  95.0m    2.9% ──────────
...
Beneath it the panel adds the figures the heatmap implies. In the `rimzd` dashboard, the **All time / Week / Month / Year** row is a tab bar: **All time** is selected by default, and `Tab` / `Shift-Tab` cycles the active window. The active tab is a color-filled chip; the plaintext mockup brackets stand in for that fill. The active tab scopes **Models**, **Agents**, and **insights** below it; the heatmap stays full-history. The All time and Year tabs show the same breakdown rows because the spend cache spans the trailing year; their headline figures differ because All time includes cache-read tokens in the row total. Non-interactive `rimz stats` and `rimz stats --json` render All time, preserving the report shape for pipes and scripts. The **Models** breakdown shares each model's slice of your selected-window `◇` token total with dollars, input (`↘`), output (`↗`), and cache-read (`◌`), friendliest-name first (`claude-opus-4-8` reads as `Opus 4.8`), then lands share in a right-aligned column shared with Agents and reinforced by a thin `━`/`─` bar. The **Agents** breakdown shares the selected-window token total by adapter kind — Claude, Codex, Pi, Open Code, and any future kind with recorded spend — leading with the glyphed session count (`◎`), then tokens and dollars before the same share column and bar. The token glyphs follow the sidebar glyph set, so `[theme] style = "modern"` and `[theme.glyphs] set = "nerd_font"` switch this panel to the Nerd Font token vocabulary while the CLI colors stay on the default palette. The **insights** close it: selected-window sessions, active-day ratio, heaviest single day, and longest and current active-day streaks. Tokens carry the per-model and per-agent breakdowns, so attribution is independent of pricing coverage — an unpriced model still attributes its tokens.
