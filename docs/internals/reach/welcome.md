...
The data is the account-global aggregate the provider dashboard producer publishes ([provider.md](../agents/provider.md)): `provider-spending.json` carries the fleet windows, the UTC-day buckets, and the model buckets. When that aggregate is missing or still on an old shape, the pace panel walks transcripts once in memory as a cold fallback, prints a note to start a sidebar for instant reads, and never writes shared spending files.
...
`rimz stats` prints the panel and returns to the shell, so it pipes and scrolls like any report rather than holding a TUI it has nothing to dispatch from. `--dollars` scales the heatmap by spend instead of tokens — the spend view the lobby's `t` toggles to — and `--json` emits the per-day buckets, the trailing windows, the model breakdown, and the insights for scripts. It is read-only: the fast path is one read of `provider-spending.json`; the cold fallback walks transcripts in memory and never writes back, so it adds no write path and never races the sidebar producer's owned cache.
...
- **The pace graph reads the published spend aggregate.** The producer runs `compute_daily_spend` and `compute_model_breakdown` beside the provider-dashboard walk, publishing those rollups into `provider-spending.json` with the trailing windows. The front door and `rimz stats` read that aggregate; only a first-launch or old-version cache falls back to an in-memory transcript walk.
