...
The data is the account-global aggregate the spending producer publishes ([provider.md](../agents/provider.md)): `provider-spending.json` carries the fleet windows, the UTC-day buckets, and the model buckets. When that aggregate is missing or still on an old shape, the pace panel takes the same shared spending election as the sidebar producer, refreshes the transcript cursor cache, and publishes the same aggregate for the next reader.
...
- **The pace graph reads the published spend aggregate.** The spending producer runs `compute_daily_spend` and `compute_model_breakdown` beside the provider-dashboard walk, publishing those rollups into `provider-spending.json` with the trailing windows. The front door and `rimz stats` read that aggregate; a first-launch or old-version cache takes the shared spending election and publishes the same aggregate for every later reader.
