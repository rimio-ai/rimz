...
**Phase 2 — session binding.** A lifecycle hook arrives carrying a session id, and Rimz joins it to the right instance. A standalone hook stamped the pane id, so the join is exact and free. A daemon-routed hook stamped no pane, so the unstamped session falls to a fallback ladder — exact-cwd today, with the multi-client-one-cwd case unsolved. The ladder and its limits are [sidebar.md → Presence model](./sidebar.md#presence-model).
