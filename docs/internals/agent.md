...
| `plan_mode`                         | read-only plan mode (→ thinking)  | set by `SessionStart`/`UserPromptSubmit` · `permission_mode == "plan"`; cleared unconditionally at a turn boundary (`Stop`/`SubagentStop`) and by leaving plan mode mid-turn (§ Leaving plan mode) | carry-forward |
...
`thinking` is `running` joined to the `plan_mode` flag, so it must clear the moment the agent stops planning — otherwise the sparkle outlives the plan. The trap is that `permission_mode` is the *session slider*, sticky across turns and present on every hook (`Stop` included), not a per-turn "planning right now" signal — so a plan-slider session reports `plan` on every event, and a naive carry-forward latches `plan_mode` true for the agent's whole life. The hard case under that is plan **approval**: the agent presents a plan via `ExitPlanMode`, the human approves, and the agent runs the plan firing only per-tool hooks, which carry no lifecycle observation (the event log is turn-grained). Three clears close the gap:

- **A finished turn is not planning.** A `Stop` (or Codex `SubagentStop`) clears `plan_mode` **unconditionally** — it reports the end of the planning *phase*, not the slider position the payload still carries — so thinking never latches across turns. This also covers the background-tasks `Stop` that stays `running`.
- **An approved plan exits plan mode mid-turn.** The "left plan mode" signal lives in the feed channel, not the lifecycle log, so the snapshot applies it at projection time (`clear_exited_plan_modes`), not in the reducer. For an `ExitPlanMode` ask bound to the agent and newer than its last lifecycle event (`last_seen`, which the activity heartbeat never advances), two paths clear `plan_mode`: an **allow-resolved** ask (approved through a Rimz resolver), or — the default, since Rimz does not answer for you — an ask the agent has **moved past**, i.e. the per-tool heartbeat advanced `last_activity` beyond the ask once the human approved in the agent's own UI and it ran its next tool. A **denial** leaves the agent planning, so the sparkle stays even if it then runs read-only tools; a **prior turn's** approval is older than a fresh plan-mode prompt, so a new planning phase still sparkles.
- **A manual exit clears mid-turn.** Shift-tabbing the slider out of `plan` (to `acceptEdits`/`auto`) without going through `ExitPlanMode` raises no approval and no lifecycle event — only per-tool hooks, which carry the new non-plan slider. The activity heartbeat ferries that per-tool reading to projection time, where `with_agent_activity` applies it as a **clear-only** override: an agent whose freshest heartbeat reports `plan_mode = false`, newer than its last lifecycle event (`last_seen`), drops the sparkle. A heartbeat still reading `plan` is ignored, so the override never re-arms thinking or fights the approval clear. A no-tool turn fires no per-tool hook, so a racy mid-turn capture there still clears only at `Stop`.
