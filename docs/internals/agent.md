...
Some agents publish far richer per-session data out of band than their hooks carry. Claude `exec`s a configured `statusLine` command on every render and pipes a JSON blob to its stdin — exact context-window token breakdown, cost, rate-limit windows with reset instants, PR info, vim mode, version, output style, effort, and `exceeds_200k`. `observe_context` normalizes any such transport into the agent-agnostic `AgentContext`; Claude parses the statusline JSON, and a later agent (Codex, over JSON-RPC) feeds the same record through the same path with no change to storage or fold-in.
