...
Connection preference (warmest first), all best-effort with a cold-spawn floor so enrichment never depends on any of them:

1. **The per-session broker** (`agents/codex_broker.rs`, run as `rimz codex app-server serve` — a pane in the `rimzd` daemon tab). It holds one long-lived, already-handshaked `codex app-server` and serves it over a unix socket at `RuntimePaths::codex_app_server_socket_path` (`$XDG_RUNTIME_DIR/rimz/<workspace_id>/sock/codex-app-server.sock`), so each datapoint skips the cold-spawn handshake: the broker answers `initialize` from its cached result and forwards the read-only methods to the warm child, routing responses back by id. This is a **local** enrichment capability — not the account-linking remote-control feature — so it runs whenever `codex` is on PATH, ungated. The broker socket path is derived from the workspace id alone, so broker (bind) and client (connect) agree without an env var.
2. **The per-user remote-control daemon** `codex remote-control start` brings up (auto-launched detached when `[remote_control] codex` is on; see [remote control auto-launch](../reference/configuration.md#remote-control-auto-launch)), re-used via `codex app-server proxy --sock <path>`. The socket is `$CODEX_HOME/app-server-control/app-server-control.sock` (`~/.codex/...`), overridable by `RIMZ_CODEX_APP_SERVER_SOCK` (empty drops it from the order).
3. **A fresh cold-spawned `codex app-server`** — the always-present fallback, so headless / no-mux still enriches.

The JSON-RPC contract is identical over every transport.
