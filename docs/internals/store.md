...
```text
workspace.json                                  project root, root class, session name, room-owned rimz_bin
...
- `workspace.json` records the project root, root class, session name, and optional room-owned `rimz_bin` for maintenance commands and session-local helpers. A record predating `root_class` decodes as `repo`, and a record predating `rimz_bin` decodes with no owner binary. Generic re-records preserve `rimz_bin`; only `rimz start`, cwd-based `rimz attach`, and `rimz reload` update it, while named attach by session preserves the recorded owner, so routine CLI commands from another worktree do not change a live room's helper identity. Launch reads the record before overwriting: when the derived session name diverges from a still-live recorded session, launch rebirths the workspace under the new name rather than stranding the session ([`workspace_record.rs`](../../crates/rimz/src/store/workspace_record.rs)).
...
- `rimz workspace rotate-events` archives the active log once it crosses a byte threshold and prunes archives older than the retention window. Lifecycle hooks trigger the same rotation path automatically at the default 64MiB threshold, debounced through `locks/auto-rotate.stamp`; manual rotation uses the same threshold unless `--max-bytes` overrides it, and archive pruning defaults to 14d. Rotation first merges every non-tombstoned agent in the rotating log's audit rollup into `agents.carryover.json`, including sessions whose runtime owner has exited, then prunes dead records older than the retention window and reseeds the rollup base. Rotation is therefore a storage mechanism: it preserves resumable identity within retention and never changes what the audit rollup remembers ([`event_log/rotation.rs`](../../crates/rimz/src/store/event_log/rotation.rs)).
- The write path converges store-provable session death between rotations: after publishing commits, a debounced `locks/dead-reap.stamp` gate scans the audit rollup and appends `agent.lifecycle` `Ended` tombstones for root sessions with a dead recorded owner, pidless or daemon-owned sessions past the ghost TTL, relaunch-superseded sessions, and `/clear`-superseded conversations. A key present in `live-roster.json` stays protected until rebirth planning consumes the recovery roster. Runtime expel and snapshot view reaping use the same rules as latency shims; the live daemon loaded-thread probe remains snapshot-side.
...
- **Expel** is read-time filtering. Default runtime views (`rimz sidebar snapshot`, the default `rimz doctor` agent summary) keep only agent rollups whose `runtime_owner` is still the live process that wrote them. Ownerless legacy records abstain and remain visible; known-dead owners and Linux PID-start mismatches are suppressed until the write-path session reap tombstones them.
- **Audit** is durable history. `rimz doctor --audit` reads the full non-tombstoned rollup.
