...
tmp/rimz-wakes/<name>.output                   one watched wake's combined output, removed at room teardown
...
This doc explains the log, the caches derived from it, and the workspace record. For the rest: messages, the transcript, and channels are [messaging.md](./harness/messaging.md), run records are [scripting.md](./harness/scripting.md#the-record), wake logs are [loops.md](./harness/loops.md#watched-commands), the diagnostics files are [diagnostics.md](./diagnostics.md), and the roster and crash files are [Session death](#session-death) below.
...
| Audit appends | `messages/history.jsonl`, `transcript/*.jsonl`, `tmp/rimz-wakes/<name>.output` | `O_APPEND`, no per-record fsync. History and transcript append under the workspace lock: a queue transaction commits `messages.jsonl` before it appends history and the event frames, and history retention runs last, so a history append or retention failure warns and never undoes the queue transition ([messaging.md § Storage and audit](./harness/messaging.md#storage-and-audit)). A wake log takes no lock of its own: `rimz wake` creates it at arm time and the one watcher process holding that wake's flock is the only writer after that ([loops.md § Watched commands](./harness/loops.md#watched-commands)). | Trailing records can be lost. The cost is history completeness, never queue correctness; for wake output, the run record keeps the last 4 KiB while the delivered message carries only the file path, size, and line count. The output file is removed at room teardown. |
