...
- The write path also reaps dead-owner agent ghosts between rotations: after publishing commits, a debounced `locks/dead-reap.stamp` gate scans the audit rollup and appends `agent.lifecycle` `Ended` tombstones for root sessions whose recorded owner process is known dead. This turns read-time expel into durable history before rotation retention would otherwise prune the carryover.
...
- **Expel** is read-time filtering. Default runtime views (`rimz sidebar snapshot`, the default `rimz doctor` agent summary) keep only agent rollups whose `runtime_owner` is still the live process that wrote them. Ownerless legacy records abstain and remain visible; known-dead owners and Linux PID-start mismatches are suppressed until the write-path dead-owner reap tombstones them.
