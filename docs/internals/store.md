...
- Lifecycle ingestion catches the durable rollup up once under the workspace lock, derives the transition and proof-of-work append policy from that state, revalidates observed and provider-derived child lineage, and appends the primary record plus any adoption or metadata reconciliation as one ordered batch. Adapter transcript and filesystem reads finish before this transaction. The receipt carries transition diagnostics, `waiting_cleared`, append suppression, derived outcomes, event identity, and rotation due so CLI code does not re-read state to decide policy.
...
| --- | --- | --- | --- |
| Event log | `events.log.jsonl` | one CRC-framed `write()` per record or ordered lifecycle batch; the off-lock tail issues a group `fdatasync` (~1/s), and rotation syncs before the rename | intact through the last group sync. The trailing window can be lost; the frame CRC turns any torn suffix into deterministic corruption that repair truncates |
