...
| Audit appends | `messages/history.jsonl`, `transcript/*.jsonl` | `O_APPEND` under the workspace lock, no per-record fsync. A queue transaction commits `messages.jsonl` before it appends history and the event frames, and history retention runs last: a history append or retention failure warns and never undoes the queue transition ([messaging.md § Storage and audit](./harness/messaging.md#storage-and-audit)). | Trailing records can be lost. The cost is history completeness, never queue correctness. |
