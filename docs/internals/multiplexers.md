...
The renderer-local controller and reconcile-time repair use the same reachability contract. The backend step seeds a symmetric half-step stop band before the first nudge, and observed feedback refines that estimate. A crossing that lands nearer to the target, or equally near, parks where it landed; one that lands farther earns exactly one reverse step and then parks unconditionally, so neither engine oscillates. No progress and bounded step budgets remain terminal outcomes.

The one-second settled-resize pass adopts only with positive evidence: backend geometry must exist, the pane observation must be newer than the resize, and neither the full view width nor sibling count may have changed. Missing or stale evidence converges and re-arms classification; a view or sibling change converges without adopting. Only the remaining case is a mouse drag: RimZ converts the measured width to a share, snaps it to the backend rung, pins it room-wide, and broadcasts it once. A dragged Zellij pane already within half a native step (at most 2.5% of its view) keeps its measured width even when the snapped share resolves a few columns away. Lattice-resident panes reach the snapped target exactly; a room displaying widths such as 25/25/23/25 across tabs is therefore at the platform floor, not carrying divergent targets.
