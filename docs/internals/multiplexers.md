...
- **Zellij** derives a keypress target step from the floor of 5% of the view, so a wider press never targets beyond the next reachable lattice width, and issues one relative step per resize-feedback event or one-second backstop. It separately uses the ceiling as the stop-band width, guaranteeing that band contains a reachable width when native steps alternate between the floor and ceiling as the pane moves. It rejects a narrower intent whose next reachable width would cross the floor. Missing topology rejects either direction because a fabricated view would pin the wrong room-wide share.
