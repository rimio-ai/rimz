...
No progress and bounded step budgets park the renderer rather than spinning. If the measured width remains off-spec, a five-second backstop re-arms convergence; a transient failed resize therefore cannot strand the pane until another `SIGWINCH`. Reconcile remains stateless and relies on its later passes instead.
