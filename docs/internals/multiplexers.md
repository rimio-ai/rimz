...
Zellij does not expose a tab-width query, so RimZ infers the viewport from the rightmost tiled pane extent. The sidebar is the first pane materialized from a layout; until a sibling contributes a second extent, that inference is self-referential and the viewport stays unknown. Renderer and reconcile probes retry rather than resolving a room target against the sidebar's own width. Layout-only birth paths may still use their non-persisted configured seed, then live convergence takes over once the completed tab proves its geometry.
