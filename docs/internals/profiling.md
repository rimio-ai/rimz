...
| What goes out on the network? | `strace -f -e trace=%network -p <pid>` for the producer's own calls, `ss -tanp` for established peers, and the runtime cache stamps for cadence. `rimz pane bandwidth` attributes the SSH render stream per pane. |
