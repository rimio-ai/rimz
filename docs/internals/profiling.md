...
**July 18, 2026 — delegated account-usage retry loop.** A live Zellij producer repeatedly spawned Pi usage helpers even though the cache recorded a settled no-credentials result: account preflight requested the selected OAuth sub-provider, while the usage error returned the kind-wide default, so every next claim treated the scope mismatch as a login change and bypassed the long settled-auth TTL. Fix: settled no-credentials normalization fills unresolved identity fields from the exact claimed preflight, guarded by a completion-to-next-claim test. The same profile found hidden consumers allocation-bound in full-snapshot fusion clones when rapid store movement outruns exact-source projection adoption; making the pulled baseline lazy remains a separate renderer-state change because overlay expiry, focus confirmation, observation diagnostics, read receipts, and presentation order all depend on the unmodified baseline.

**July 18, 2026 — allocation and warm-materialization follow-up.** The delegated-usage capture's remaining renderer and spending-service profiles were actionable without changing their truth contracts. Overlay-free pulls now transfer directly into presentation while compact focus and observer projections preserve confirmation and diagnostics; a full raw baseline is retained only for an active overlay, focus fence, or gate hold. The spending discovery index reuses an ordered active-path materialization until explicit frontier or cache-reconciliation generations change, and each renderer thread reuses merged read receipts until the atomically replaced receipt generation or directory stamp changes. Focus/fusion tests preserve expiry, confirmation, gate, and event behavior; discovery tests pin warm reuse plus changed-frontier and retired-file invalidation; the read-mark test pins one merge scan across unchanged folds and immediate visibility after atomic peer publication.
