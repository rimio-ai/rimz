...
The common-cohort run installed exact base `22fdbc23` (ELF build `09ff626b9920`) and completed feature `f1414a95` (ELF build `891a28a80cc3`), reloaded the fleet, and verified every measured heartbeat build before each window. The base stabilized with 19 worker instances; the same 19 instances were tracked after the feature reload, although one additional pane and worker appeared before the feature window, so the feature carried a slightly larger input set and the topology comparison is conservative rather than exact. Over matched 20-second scheduler-truth windows, those 19 workers fell from 6.5971 to 1.7718 cores (73.1%), or 0.3472 to 0.0933 core per tracked worker. The elected producer fell from 1.0863 to 0.2605 core; across the other 18 workers, the ordinary-consumer median fell from 0.2867 core (range 0.2665–0.6614) to 0.0795 (0.0775–0.1530). The full final-build fleet reached 20 workers only after the common-cohort snapshot; that count and its publication traffic are unpaired supplemental evidence, so no 20-worker scheduler total is estimated. Parallel 12-second `strace -f -c` windows fell from 700,975 to 106,888 syscalls for one common consumer (84.8%) and from 2,011,170 to 1,100,112 for the producer (45.3%). Three-second consumer file traces fell from 26,148 to 3,624 path operations below active `~/.codex/sessions` (86.1%) and from 48 to 6 below the archive. Matching `perf stat` windows corroborated the scheduler result: consumer task-clock fell from 2,494.64 to 585.54 ms and producer task-clock from 12,610.54 to 3,838.65 ms over ten seconds. The observation is lower CPU and filesystem work despite the extra feature-side pane; the code-level cause is separately pinned by the multi-workspace Codex test, which admits two requested roots and excludes an unrequested root from one bounded candidate pass.
...
Allocator retention remains material and does not justify an inline allocator workaround. The memory snapshots are descriptive rather than paired because process age and topology differed: the 19 selected instances totaled 2,425,588 then 2,197,536 KiB RSS and 2,134,544 then 1,898,180 KiB private memory. Read-only `mallinfo2()` found producer free arena space of 201,839,984 then 209,013,936 bytes and mature-consumer free arena space of 100,245,424 then 100,934,752 bytes; their RSS snapshots were about 300/307 MiB and 117/119 MiB. These figures support no per-process memory-improvement claim. The repeat-work fixes materially reduce CPU and filesystem churn without removing allocator-retained arenas, so allocator policy remains separate follow-up work.
