...
**Wait without a pane.** [`rimz wait`](./docs/reference/cli/wait.md) is the alarm an agent sets for itself instead of holding its turn open on `sleep`: it ends the turn, and when the delay elapses or the command finishes RimZ delivers a message back into the same conversation naming what fired and on what.
...
# Wait for CI to fail on the branch this agent is working on
rimz loop add ci-red --signal ci.failed --wait

# Wait on an event anything can emit: a git hook, a deploy script, another agent
