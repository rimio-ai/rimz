...
The scripts are single-file Python 3 (stdlib only — no third-party deps). They write their own heartbeats and shell out to public Rimz commands: feed commands for bridge resolvers, pane commands for resolver-owned TTY inspection, and `message --steer` for human-authored recovery text.
...
Demonstrates the state-aware recovery loop for transient provider stalls: poll `rimz sidebar snapshot --json`, detect rows whose context reports `paused_overloaded`, skip rows with pending asks, capture the pane and match a bounded recoverable-error pattern, then send `continue` with `rimz message --steer`.
...
Use `--mode replay` to send `pane send --key up --key enter` instead of `message --steer ... continue` for agents whose previous prompt should be replayed. The script has `--dry-run-snapshot <file>` for fixture smoke tests.
...
- **Pane inspection belongs to resolvers.** Core sends only human-authored text through `message`; resolvers own capture-before-send when a policy depends on pane contents.
