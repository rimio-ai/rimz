...
- Every system-initiated intervention leaves a durable record: auto-continue resumes, Codex credit redemptions, and scheduled pings each append trigger, evidence, and outcome to an account-global assist log. `rimz stats --assists` renders the history and `rimz stats --json` publishes the same rollup. → [loop internals](./docs/internals/harness/loops.md#the-assist-log)
