...
- Cursor hook detection survives the CLI re-serializing `cli-config.json`, so recurring installed-hook prompts stop and uninstall can still restore a wrapped user statusline from RimZ's durable sidecar. → [Cursor adapter](./docs/internals/agents/adapter_cursor.md#hooks-and-lifecycle)
- Remote browser tunnels inject ttyd's Basic credential at their local loopback relay, so Safari can attach WebSockets without a password prompt while the public URL stays stable across reconnects. → [web](./docs/guide/web.md#open-a-remote-room)
- Residual mouse-wheel motion during SSH and browser attach handoffs no longer reaches an agent composer as Up or Down: recovery panels keep mouse reporting armed through the transition, and the ttyd client suppresses xterm.js's alternate-screen wheel-to-arrow fallback while no application owns the mouse. → [remote](./docs/guide/remote.md) · [web](./docs/guide/web.md#browser-appearance-and-input)
- Cursor binary discovery verifies the generic `agent` executable before claiming it, launches only a verified path or the provider-unique `cursor-agent` alias, and leaves ambiguous process basenames unattributed until a native hook binds the pane. → [Cursor adapter](./docs/internals/agents/adapter_cursor.md#launch-and-resume)
...
- Browser cursors stay steady and stop jumping to random positions while terminal apps redraw. → [web](./docs/guide/web.md#browser-appearance-and-input)
