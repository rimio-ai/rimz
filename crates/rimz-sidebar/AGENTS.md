...
- **One write, two reads.** The crate's only write is its own liveness heartbeat (`rimz::sidebar::write_heartbeat`, a runtime-file write); state arrives through the in-process consumer read (`rimz::sidebar::snapshot`) or, on the elder, the in-process produce (`rimz::sidebar::produce` — its cache writes belong to that library module, never this crate's code), and the no-ledger-writer-import rule is a CI grep.
- **Mux touches stay read-only.** Pane presence arrives through the snapshot pipeline — `rimz::sidebar` owns every `list-panes`, running on the elder's fetch worker. The crate's own mux calls are two read-only probes, neither of which updates the rendered snapshot: the resize-time sibling count for self-close (geometry only), and on tmux the elder's control-mode presence stream ([`app/tmux_watch.rs`](./src/app/tmux_watch.rs)) — topology nudges that only request a refetch, with the poll as truth. Width is the launch path's concern: every pane is born at the launch's width verdict (30% capped at `sidebar.max_cols`, resolved once at start), so the renderer never resizes its own pane.
