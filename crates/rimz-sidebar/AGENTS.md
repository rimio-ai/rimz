...
- **Panes are opaque.** Pane presence arrives in the snapshot; never call `pane capture` / `pane send`. The renderer touches the mux in exactly two read-only ways, neither of which updates the rendered snapshot: the resize-time sibling count for self-close (geometry only), and on tmux the elder's control-mode presence stream ([`app/tmux_watch.rs`](./src/app/tmux_watch.rs)) — topology nudges that only request a refetch, with the poll as truth. Pane width is the launch path's concern: every pane is born at the launch's width verdict (30% capped at `sidebar.max_cols`, resolved once at start), so the renderer never resizes its own pane.
