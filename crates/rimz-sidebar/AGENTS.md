...
Golden the composed frames with `insta` snapshots (the `render_*` tests) and keep `compute_next_state` covered by pure unit tests — health debounce, the regression gate, the self-close latch, the selection contest. Run through `cargo xtask test`.
