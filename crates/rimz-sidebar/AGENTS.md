...
- **Panes are opaque.** Pane presence arrives in the snapshot; never call `pane capture` / `pane send`. The renderer's mux calls are geometry-only: the resize-time sibling count for self-close, and the one creation-time `resize_sidebar_pane` toward the `sidebar.max_cols` cap (latched per process, never on a reload re-exec). Neither updates the rendered snapshot.
