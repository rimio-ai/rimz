...
/// Forward budget headroom in `kind`'s longest account-scoped window.
pub(crate) fn longest_window_surplus(
    runtime: &RuntimePaths,
    kind: &str,
    now: Timestamp,
) -> Option<WindowSurplus> {
    let cache = read_rate_limits_cache(&runtime.shared_rate_limits_path());
    longest_window_surplus_in(&cache, kind, now)
}
...
fn longest_window_surplus_in(
    cache: &RateLimitsCache,
    kind: &str,
    now: Timestamp,
) -> Option<WindowSurplus> {
    let window = cache
