...
Topic detail lives in the internals leaves the root map describes — [adapter.md](../../../../docs/internals/agents/adapter.md) (this layer: the registry, the capability traits, the hook path, install, context sources, and declared coverage), [model.md](../../../../docs/internals/agents/model.md) (the agent model the adapters feed: the rollup, the state machine, and the displayed-status ladder), the per-kind internal mappings ([adapter_claude.md](../../../../docs/internals/agents/adapter_claude.md) and its Codex, Amp, Copilot, Kimi, Pi, OpenCode, Antigravity, Cursor, Droid, Kiro, Qwen, and Grok siblings), [providers.md](../../../../docs/internals/agents/providers.md) (accounts and balances), [spending.md](../../../../docs/internals/agents/spending.md) (spend and pricing), and the per-agent upstream references in [agent-adapter/](../../../../docs/externals/agent-adapter/claude-reference.md). The sequenced integration playbook for a new built-in is [agent-adapters.md](../../../../docs/contributing/agent-adapters.md).
...
- [`petname.rs`](./petname.rs) owns the handle grammar: the adjective-noun minter with its deterministic fallback for pre-petname sessions, `valid_agent_name`, the reserved command words, and the pseudo-handles message envelopes give non-agent senders. It sits here because a handle is invalid when it reads as a catalog kind, which makes validity a question for [`registry::known_kinds`](./registry.rs), and because `store` mints an agent's handle (at launch and in the snapshot fold) and validates every candidate name through `agents::petname`.
- Shared decisions keep one home, and adapters supply the inputs: [`hook_types.rs`](./hook_types.rs)'s `SessionSource::session_start_signal` owns what a `SessionStart` source means (`compact` closes a compaction, every other source registers), and `LocalContextRefreshCtx::changed_transcript` owns the unchanged-source gate. An adapter parses the native source and resolves the logical stat that stands for its local source; neither rule is restated per kind.
...
- A provider home is an account. Every host-side read of one resolves from the login env map its caller passes, through the adapter's `config_home`; [`login.rs`](./login.rs) owns the ambient snapshot and the room's login, and a CI grep keeps the provider home keys out of process-env reads outside tests.
...
`AgentDefinition::probe_account_usage` returns one identity-bearing provider-neutral result. Provider transports parse native wire shapes and provider account modules normalize plan/windows/credits. Sidebar refresh owns account cadence, durable claims, and cache publication; `agents::spending` owns fleet-spending cadence, refresh, and publication — the engine is the only runtime writer of `provider-spending.json` and `workspace-spending.<hash>.json`, through one version-stamping writer per file — while sidebar supplies workspace inputs and consumes its durable results.
