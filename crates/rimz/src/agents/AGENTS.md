...
- Each built-in agent kind is a sibling directory ([`claude/`](./claude/mod.rs), [`codex/`](./codex/mod.rs), [`amp/`](./amp/mod.rs), [`copilot/`](./copilot/mod.rs), [`gemini/`](./gemini/mod.rs), [`kimi/`](./kimi/mod.rs), [`pi/`](./pi/mod.rs), [`opencode/`](./opencode/mod.rs), [`cursor/`](./cursor/mod.rs), [`droid/`](./droid/mod.rs), [`kiro/`](./kiro/mod.rs), [`qwen/`](./qwen/mod.rs)) owning its integration and the typed payloads and enrichment surfaces it supports; [`plugin/`](./plugin/mod.rs) is one kind-agnostic adapter for every validated machine-tier process plugin.
