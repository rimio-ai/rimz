...
pub fn check_from_root(
    ...
    if super::super::registry::BUILTINS
        .iter()
        .any(|adapter| adapter.spec().kind == kind)
    {
        return Err(format!(
    ...
    let coverage = definition.coverage.iter().fold(
    ...
    let lifecycle = definition.lifecycle_hooks.iter().fold(
        PluginCheckSummary::default(),
        |mut summary, (_, coverage)| {
            match coverage {
            ...
            let status = if !probe.present {
                ...
                match probe.name {
                    ...
                    "account" => adapter
                        ...
                        .as_ref()
                        .map(|argv| {
                            probes::check_account(adapter.spec.kind, adapter.plugin_dir, argv)
                        })
                        .map(probe_status)
                        .unwrap_or_else(|| {
                            ProbeCheckStatus::Failed("probe declaration disappeared".into())
                    ...
                    "version" => adapter
                        ...
                        .as_ref()
                        .map(|argv| {
                            probes::check_version(adapter.spec.kind, adapter.plugin_dir, argv)
                        })
                        .map(probe_status)
                        .unwrap_or_else(|| {
                            ProbeCheckStatus::Failed("probe declaration disappeared".into())
...
fn replay(adapter: &PluginAdapter, path: &Path) -> Result<ReplayCheckReport, String> {
    ...
    for (index, line) in text.lines().enumerate() {
        ...
        if matches!(envelope.event, CanonicalEvent::Context) {
            let Some(observation) = adapter.observe_context(adapter.spec.kind, &payload) else {
                rejected += 1;
                rows.push(rejected_row(
                    line_number,
                    event_name,
