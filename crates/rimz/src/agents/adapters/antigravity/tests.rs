...
fn hook_install_round_trips_existing_and_absent_statuslines() {
    for (name, original, change) in [
        ...
        assert_eq!(
            files,
            [
                (hooks_path.as_path(), true),
                (settings_path.as_path(), true)
            ]
        );
        ...
        let report = install::uninstall(&hooks_path, &settings_path).unwrap();
        assert_eq!(report.removed_events, ANTIGRAVITY_EVENT_NAMES);
        let files: Vec<_> = report
            .files
            .iter()
            .map(|file| (file.path.as_path(), file.existed))
...
fn launch_resume_permissions_and_model_preset_match_agy() {
    ...
    for (mode, expected) in [
        ...
        assert_eq!(
            AntigravityAdapter.spec().launch.permission_args(mode),
            expected
        );
    }
  ...
  sl  local_address rem_address   st tx_queue rx_queue tr tm->when retrnsmt uid timeout inode\n\
   0: 00000000000000000000000001000000:18EB 00000000000000000000000000000000:0000 0A 0:0 00:0 0 501 0 67890";
    ...
    fn paired_usage_probe_revalidates_one_endpoint_for_both_rpcs() {
        ...
        assert!(AntigravityAdapter.spec().capabilities.direct_account_usage);
    }
    #[test]
