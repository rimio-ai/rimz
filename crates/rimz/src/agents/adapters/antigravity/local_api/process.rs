...
pub(in crate::agents::adapters::antigravity) fn candidate_identity_matches(
    candidate: &Candidate,
...
pub(in crate::agents::adapters::antigravity) fn identity_matches(
    process_uid: Option<u32>,
    expected_uid: u32,
    executable: &Path,
    argv0: &OsStr,
...
#[cfg(any(target_os = "linux", test))]
pub(in crate::agents::adapters::antigravity) fn socket_inode(target: &str) -> Option<String> {
    target
        .strip_prefix("socket:[")?
        .strip_suffix(']')
...
#[cfg(any(target_os = "linux", test))]
pub(in crate::agents::adapters::antigravity) fn parse_proc_net(
    table: &str,
...
#[cfg(any(target_os = "macos", test))]
pub(in crate::agents::adapters::antigravity) fn parse_lsof(output: &str) -> Vec<LoopbackEndpoint> {
    let mut endpoints = BTreeSet::new();
