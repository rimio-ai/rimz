...
fn suffix_streaming_is_exactly_once_torn_safe_and_resets_after_truncation() {
    ...
    assert_eq!(
        cursor.messages(
            Some(&path_text),
            None,
            crate::agents::definition_by_kind("droid").unwrap()
        ),
...
fn neutral_malformed_pid_and_launch_surfaces_are_explicit() {
    ...
    let definition = DroidAdapter.spec();
    assert!(definition.runs_as("droid"));
    assert!(definition.runs_as("droid-aarch64-unknown-linux-gnu"));
    ...
    assert_eq!(
        DroidAdapter.spec().launch.fork_command("sess-1"),
        Some(vec![
            "droid".to_owned(),
            "--fork".to_owned(),
            "sess-1".to_owned()
