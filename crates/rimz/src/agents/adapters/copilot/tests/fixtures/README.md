...
Captured `github.copilot.cost` was a finite per-chat `0.0`, while `invoke_agent` carried no session-cumulative dollar. OTel therefore supplies no dollars; the managed statusline supplies the live local estimate and finalized shutdown history supplies the separate historical estimate.
