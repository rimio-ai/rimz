...
/// Orphaned children retain their continuation slot in a separate namespace;
...
fn fold_seats<'a>(
    ...
    for child in children {
        ...
        let key = parent.map(slot).unwrap_or_else(|| {
            let mut key = slot(child);
            key.slot = Slot::Orphan(Box::new(key.slot));
            key
        });
