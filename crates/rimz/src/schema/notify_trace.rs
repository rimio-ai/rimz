//! Notification trace record schema.
//!
//! A human/debugging timeline that answers, for any tab-bell episode, *who sent
//! it and what caused it*: the producer's emitted notifications, each renderer's
//! tab-bell decision, and the unread mark/clear transitions behind them. Records
//! are append-only JSONL under the workspace state directory, never read by
//! correctness code.
