...
- **One pane, one writer.** Every RimZ pane write (`message`, `answer`, `pane send`) runs inside `mux::PaneWriter`, one per-pane user-scoped runtime advisory lock held across the entire write batch, from the first byte through Enter, including compact-first commands and paced segments. Steer waits for an in-flight write; it interrupts the agent's turn, not another writer. A fresh boundary send also parks behind an unexpired non-`Resume` claim for its card.
