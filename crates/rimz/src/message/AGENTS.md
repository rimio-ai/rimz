...
- **Acknowledgements follow submitted text.** A submit that contains a record confirms it. Stray composer text around RimZ's envelope is direct input and is never a reason to write the pane again; reported text that contains no record confirms nothing. A late acknowledgement stays valid while the record is `Queued`; only a new claim supersedes it.
