...
- **One pane, one writer.** Every RimZ pane write (`message`, `answer`, `pane send`) runs inside `mux::PaneWriter`, one per-pane room-state advisory lock held across the entire write batch, from the first byte through Enter, including compact-first commands and paced segments. Steer waits for an in-flight write; it writes into the live turn now, without disrupting another writer. Interrupt holds its one writer from the paste through Enter, the interrupt delay, and the interrupt key, so no other write lands between its prompt and its key. A fresh boundary send also parks behind an unexpired non-`Resume` claim for its card.
- **One card, one FIFO queue.** Records key on `(kind, agent_id)` with `agent_name` folding a provisional `launch_*` id into the session it registers as. `msg_` id string order is FIFO order; `Resume` control messages occupy a separate lane.
