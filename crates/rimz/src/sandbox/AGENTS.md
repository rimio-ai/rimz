...
- `TmpView` owns host-to-agent output path mapping and the reverse mapping for caller-supplied paths: under sandbox isolation the owner's scratch dir becomes `/tmp/scratchpad/…` and other room tmp paths become `/tmp/…`; they stay host paths otherwise. The host state path remains accessible; room tmp is separate from host `/tmp`, not hidden from the host.
