//! Rimz-owned storage measurement: symlink-safe byte walks plus the account
...
mod tests {
    ...
    fn measure_under_reports_roots_and_total() {
        ...
        fs::write(state.join("ledger.json"), b"{}").unwrap();

        let storage = measure_under(&[
            (StorageKind::State, state.clone()),
            (StorageKind::Runtime, runtime.clone()),
        ]);

        assert_eq!(storage.roots[0].kind, StorageKind::State);
        assert_eq!(storage.roots[0].path, state);
        assert!(storage.roots[0].present);
        assert!(storage.roots[0].bytes >= 2);
        assert_eq!(storage.roots[1].kind, StorageKind::Runtime);
        assert_eq!(storage.roots[1].path, runtime);
        assert!(!storage.roots[1].present);
        assert_eq!(storage.roots[1].bytes, 0);
        assert_eq!(
            storage.total_bytes(),
            storage
                .roots
