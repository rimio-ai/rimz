...
/// [`Store::repair_event_log`] heals, distinct from environment errors it
/// cannot.
fn publish_hit_corruption(err: &StoreErr) -> bool {
    matches!(
        err,
        StoreErr::Snapshot(snapshot::SnapshotErr::EventLog(log_err)) if log_err.is_corruption()
    )
}
...
impl Store {
    ...
    fn publish_snapshot_now(&self) {
        ...
        if !publish_hit_corruption(&err) {
            warn!(error = %err, "snapshot publish failed after store commit");
            return;
        }
    ...
    pub(super) fn wake_sidebars_for_event_best_effort(&self, event: &EventEnvelope) {
        if let Err(err) = wakeup::wake_sidebars_for_event(&self.inner.runtime, event) {
            warn!(
                ...
                error = %err,
                "sidebar wakeup failed after store event commit"
            );
        }
    }
}
