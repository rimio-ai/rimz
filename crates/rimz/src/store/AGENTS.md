...
- [`writer/queue.rs`](./writer/queue.rs) owns all three message surfaces together: the live `messages.jsonl`, terminal text in the sibling `history.jsonl`, and terminal outcomes in the event log. Archive retention stays with event-log rotation, global orphan-temp collection with GC, and cache-local temp-sibling cleanup with [`atomic.rs`](./atomic.rs).
