...
- Every mutator and its public intent/outcome vocabulary lands under [`writer.rs`](./writer.rs): the façade owns the `commit` primitive, and `writer/` owns the debounce, lifecycle, publish, reset, queue, and reap branches. Reads on the `Store` handle stay lock-free; snapshot schema is consumed through its owning module.
