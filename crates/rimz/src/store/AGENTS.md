...
- Every store mutator and its public intent/outcome vocabulary lands under [`writer.rs`](./writer.rs): the façade owns the ordinary commit and log-boundary primitives, and `writer/` owns the debounce, lifecycle, publish, reset, queue, and reap branches. **`store` owns every record it persists, reaches nothing in `harness`, and reaches `diag` downward for the bind diagnostics it constructs; disk mechanics, usage measurement included, sit below it in `disk`; `harness` is policy over store records and reaches down.** The supervised-run record lives in [`run.rs`](./run.rs): schema, durable codec, `RunRecord::mark_terminal`, and the terminal wake sender; `harness::run` owns the transitions and their workspace lock, and `harness::run_wake` binds the waiter that sender reaches. The signal vocabulary the event log persists lives in [`event.rs`](./event.rs) beside its payload; `harness::schedule` owns selectors, watch outcomes and firing. Reads on the `Store` handle stay lock-free; snapshot schema is consumed through its owning module.
...
- [`message.rs`](./message.rs) owns the durable record vocabulary, submitted-prompt header grammar (`HeaderKind`, `parse_message_header`, `split_batched_prompt`), prompt-origin classification including the conversation/system split (`MessageSender::is_conversation`), FIFO/claim/batch selection, and `align_submitted_prompt`, which maps one submitted pane prompt back onto the records written as its batch; it strips the headers `address` composes, so both spell a notice's type through `HarnessNotice::header_type`. [`message/codec.rs`](./message/codec.rs) owns the live queue and history codec. [`writer/queue.rs`](./writer/queue.rs) owns all three message surfaces together: the live `messages.jsonl`, terminal text in the sibling `history.jsonl`, and terminal outcomes in the event log, with status transitions under the workspace lock and their audit events. Archive retention stays with event-log rotation, global orphan-temp collection with GC, and cache-local temp-sibling cleanup with [`disk/atomic.rs`](../disk/atomic.rs).
