...
fn run_direct_web(remote: &RemoteConnect, client_size: Option<(u16, u16)>) -> Result<()> {
    ...
    let spec =
        rimz::remote::web::web_tunnel_spec(&remote.target, forward_port, payload.tunnel()?.0);
...
fn run_supervised_web(remote: &RemoteConnect, client_size: Option<(u16, u16)>) -> Result<()> {
    ...
    let mut link = LinkSupervisor::for_web(plan, control);
    if !link.connect()? {
        return Ok(());
    }
    ...
    loop {
        let round_control = link.control().map(Path::to_path_buf);
...
fn parse_web_payload(
    ...
    let credential = payload.tunnel()?.1.clone();
...
mod tests {
    ...
    fn trusted_header_payload_with_a_credential_tunnels_to_its_upstream_port() {
        ...
        assert_eq!(payload.tunnel().expect("tunnel payload").0, 41820);
    }
    ...
    fn legacy_basic_payload_tunnels_to_its_public_port() {
        ...
        assert_eq!(payload.tunnel().expect("tunnel payload").0, 8200);
    }
