...
enum HooksSubcmd {
    ...
    Install {
        ...
        #[arg(long)]
        dry_run: bool,
        /// Agent name (`claude`, `codex`, `amp`, `copilot`, `kimi`, `pi`, `opencode`, `droid`, `qwen`). Omit to install every detected agent.
        agent: Option<String>,
    },
    /// Remove the adapter's Rimz-managed hook block.
    Uninstall {
        /// Agent name (`claude`, `codex`, `amp`, `copilot`, `kimi`, `pi`, `opencode`, `droid`, `kiro`, `qwen`). Omit to remove every Rimz-managed hook set.
        agent: Option<String>,
    },
}
