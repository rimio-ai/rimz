...
- `wake/` is the self-only face of the scheduler: it resolves the calling live agent for delay and command waits and presents receipts, pending lists, and caller-owned cancellation. It and `loop_cmd/` call `harness::schedule::arm` for delivery construction, signal defaults and guards, dedupe, and detached watcher spawning; neither command imports the other. The shared resolver supports `@me`, including bare `loop add --wake` ([loops.md](../../../../docs/internals/harness/loops.md#wakes)).
