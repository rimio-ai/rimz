...
- Shared CLI-layer modules serve every command: `render/` (output streams, palettes), `spinner`, `send` (shared send flags and outcome presentation), `address`, and the target-resolution helpers in `mod.rs`.
