...
- `wake/` is the caller-pinned face of the scheduler: it mints the `wake-<petname>` machine task, resolves the caller and the delivery target, and starts the hidden `rimz wake watch <name>` child through `child_process::spawn_detached_reaped` with null stdin and stdout, the wake's own `wakes/<name>.log` as its stderr, and `process_group(0)` so the arming caller's group cannot take the watcher down with it. Trigger validation, signal firing, and the run record stay in `harness::schedule` ([loops.md](../../../../docs/internals/harness/loops.md#wakes)).
