...
fn docs_url(path_and_anchor: &str) -> String {
    format!("{REPO_URL}/blob/main/docs/{path_and_anchor}")
}
...
pub(super) fn render_markdown(w: &mut impl Write, report: &Attribution) -> std::io::Result<()> {
    ...
    writeln!(
        ...
        markdown_totals_label(&report.totals)
    )?;
    writeln!(w)?;
    ...
    for (index, group) in report.groups.iter().enumerate() {
        ...
        for member in &group.members {
            ...
            if let Some(effort) = markdown_effort_label(member) {
                writeln!(w, "  - effort: {effort}")?;
            }
            if let Some(calls) = markdown_calls_label(member) {
                writeln!(w, "  - calls: {calls}")?;
            }
            if let Some(messages) = markdown_messages_label(member) {
                writeln!(w, "  - messages: {messages}")?;
            }
            if let Some(tokens) = token_label(member) {
    ...
    writeln!(
        ...
        docs_url("reference/cli/agents.md#attribution")
    )?;
    writeln!(w)?;
    writeln!(w, "</details>")
}
...
fn markdown_summary_subject(report: &Attribution) -> String {
    ...
    match report.groups.as_slice() {
        [
            ...
            format!(
                r#"Implemented by the {rimz} <a href="{}"><code>{}</code> team</a>"#,
                docs_url("guide/teams.md"),
...
fn markdown_effort_label(member: &AttributionMember) -> Option<String> {
    let mut parts = Vec::with_capacity(2);
    if let Some(seconds) = member.active_secs {
        parts.push(format!(
            "{} [active]({})",
            duration_label(seconds),
            docs_url("guide/insight.md#how-the-numbers-are-calculated")
        ));
    }
    if let Some(cost) = member.cost_usd.map(rimz::theme::fmt::dollars2) {
        parts.push(if member.subagents.is_empty() {
            ...
            format!(
                "{cost} incl. [subagents]({})",
                docs_url("reference/cli/subagents.md")
            )
        });
    }
    (!parts.is_empty()).then(|| parts.join(" · "))
...
fn markdown_calls_label(member: &AttributionMember) -> Option<String> {
    let asks_url = docs_url("guide/messaging.md#asks-and-answers");
    let compactions_url = docs_url("guide/messaging.md#land-against-a-fresh-window");
    let mut parts = Vec::with_capacity(3);
    match member.asks {
        0 => {}
        1 => parts.push(format!("1 [ask]({asks_url})")),
        count => parts.push(format!("{count} [asks]({asks_url})")),
    }
    match member.tool_calls {
        0 => {}
        1 => parts.push("1 tool call".to_owned()),
    ...
    match member.compactions {
        0 => {}
        1 => parts.push(format!("1 [compaction]({compactions_url})")),
        count => parts.push(format!("{count} [compactions]({compactions_url})")),
    }
    (!parts.is_empty()).then(|| parts.join(" · "))
}

fn markdown_messages_label(member: &AttributionMember) -> Option<String> {
    [
        (member.messages.from_user, "from the author"),
        (member.messages.from_teammates, "from teammates"),
        (member.messages.to_teammates, "to teammates"),
    ]
    .into_iter()
...
fn markdown_totals_label(totals: &EffortTotals) -> String {
    let mut parts = vec![format!(
        "{} {}",
        totals.agents,
        if totals.agents == 1 {
    ...
    if let Some(active) = totals.active_secs {
        parts.push(format!(
            r#"{} <a href="{}">agent-time</a>"#,
    ...
    let mut human = Vec::with_capacity(2);
    match totals.messages.from_user {
        0 => {}
        1 => human.push("1 message".to_owned()),
        count => human.push(format!("{count} messages")),
    }
    match totals.asks_answered {
        0 => {}
        1 => human.push(format!(
            r#"1 <a href="{}">ask</a> answered"#,
            docs_url("guide/messaging.md#asks-and-answers")
        )),
        count => human.push(format!(
            r#"{count} <a href="{}">asks</a> answered"#,
            docs_url("guide/messaging.md#asks-and-answers")
        )),
    }
    ...
    if human.is_empty() {
        ...
        format!("{totals} — human: {}", human.join(", "))
    }
}
...
mod tests {
    ...
    fn markdown_escapes_values_and_renders_grouped_bullets() {
        ...
        <details>
        <summary>Implemented by <a href="https://github.com/rimio-ai/rimz">RimZ</a> agents — 2 agents · 2h10m <a href="https://github.com/rimio-ai/rimz/blob/main/docs/guide/insight.md#how-the-numbers-are-calculated">agent-time</a> · $2.50 — human: 4 messages, 2 <a href="https://github.com/rimio-ai/rimz/blob/main/docs/guide/messaging.md#asks-and-answers">asks</a> answered</summary>

        **forge team**

        - **plan|ner** — Claude fable&#96;2@high
          - effort: 1h05m [active](https://github.com/rimio-ai/rimz/blob/main/docs/guide/insight.md#how-the-numbers-are-calculated) · $1.25 incl. [subagents](https://github.com/rimio-ai/rimz/blob/main/docs/reference/cli/subagents.md)
          - calls: 2 [asks](https://github.com/rimio-ai/rimz/blob/main/docs/guide/messaging.md#asks-and-answers) · 7 tool calls · 1 [compaction](https://github.com/rimio-ai/rimz/blob/main/docs/guide/messaging.md#land-against-a-fresh-window)
          - messages: 2 from the author · 5 from teammates · 4 to teammates
          - tokens: 1.2k input, 800 output, 2k cache write, 3k cache read
          - subagents: 3 × explorer ($0.55) · 1 × other
    ...
    #[test]
    fn markdown_omits_empty_human_clause() {
        let mut report = report();
        report.totals.messages.from_user = 0;
        report.totals.asks_answered = 0;
        let mut output = Vec::new();
