...
pub(super) fn launch_layout(args: AgentsArgs, globals: &GlobalFlags) -> Result<()> {
    ...
    let cwd = launch.cwd;
    let branch_glyph = rimz::sidebar_pane::render::worktree_branch_glyph(&machine_config.sidebar);
    let title = rimz::agents_spec::default_tab_title(
        &layout,
        &cwd,
...
pub(super) fn reject_launch_flags_without_spec(args: &AgentsArgs) -> Result<()> {
    if !args.passthrough.is_empty() {
        bail!("missing agent layout spec before `--`");
    }
