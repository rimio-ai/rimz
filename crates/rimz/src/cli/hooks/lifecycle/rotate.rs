...
pub(super) fn spawn_auto_rotation_if_due(workspace: &ResolvedWorkspace, store: &Store) {
    let Ok(meta) = std::fs::metadata(&store.paths().events_log) else {
        return;
    };
    if !auto_rotation_size_due(meta.len()) {
        return;
    }
    if !auto_rotation_stamp_due(auto_rotate_stamp_age(store)) {
        return;
    }
    touch_auto_rotate_stamp(store);
...
pub(super) fn auto_rotate_stamp_age(store: &Store) -> Option<std::time::Duration> {
    let modified = std::fs::metadata(store.paths().locks_dir.join(AUTO_ROTATE_STAMP))
        .ok()?
        .modified()
        .ok()?;
...
pub(super) fn touch_auto_rotate_stamp(store: &Store) {
    let _ = std::fs::write(store.paths().locks_dir.join(AUTO_ROTATE_STAMP), b"");
}
