...
- A development build launched in a room whose producer is the installed binary joins as a consumer and reads that producer's published caches, so a bumped cache version reads as empty until the installed binary is replaced. That blank is the version gate working, and it is the first thing a cache-version change looks like from inside the room: prove such a change by publishing a cache file at the new version yourself, or by installing the build.
