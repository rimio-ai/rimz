...
//! it until either a run-complete wakeup arrives from the ledger writer or the
//! cap fires. Socket paths are derived from run ids so binder and ledger writer
//! share one source of truth.
//!
//! Validation is by `(workspace_id, run_id)`, per
//! `docs/internals/sidebar/ledger.md`. Frames that fail validation are logged
...
/// Wakeup frame the ledger writer sends to a per-run socket when a supervised
