...
- [`link.rs`](./link.rs) — the health JSONL protocol, the tier classifier over `ids::LinkTier`, and terminal-session transitions; [`reachability.rs`](./reachability.rs) — endpoint discovery and reconnect-wait state; [`recovery.rs`](./recovery.rs) — panel timing and checkpoints.
- [`forward.rs`](./forward.rs) — listener discovery and live ControlMaster forwards; [`setup.rs`](./setup.rs) — argv builders whose I/O belongs to the CLI; [`web.rs`](./web.rs) — browser-tunnel argv plus the one socket exception: `bind_local_relay` and `reserve_forward_port` bind loopback listeners to pick a free local port (the relay keeps its listener; the forward port is probed and released); [`tty.rs`](./tty.rs) — terminal-state hygiene.
