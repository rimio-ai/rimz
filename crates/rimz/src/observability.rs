...
//! [`BREADCRUMB_TARGET`] and [`ScopeFacts`] are always present so the callsites
//! that seed breadcrumbs and command scope need no `#[cfg]`. With the feature
//! on, `reporting` is the live impl; off, `disabled` is a no-op with the same
//! surface, so `main.rs` and `cli/mod.rs` are identical in both builds.
