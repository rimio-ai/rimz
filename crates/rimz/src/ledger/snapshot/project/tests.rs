...
fn registered_at_stamps_first_event_then_carries_forward() {
    let workspace = WorkspaceId::from_project_root(Path::new("/tmp/x"));
    let lifecycle = |secs_after_epoch: i64, params: serde_json::Value| {
...
fn rebirth_boundary_unstamps_prior_panes_in_log_order() {
    ...
    let workspace = WorkspaceId::from_project_root(Path::new("/tmp/x"));
    let stamped_start = |agent_id: &str, pane: &str| {
        EventEnvelope::new(
            workspace.clone(),
            "session",
            "codex",
    ...
    let agents = reduce_agent_states(&[
        stamped_start("sess-old", "zellij:terminal_6"),
        boundary.clone(),
        stamped_start("sess-new", "zellij:terminal_6"),
    ]);
    let by_id = |id: &str| {
        agents
            .iter()
    ...
    let restamped = reduce_agent_states(&[
        stamped_start("sess-old", "zellij:terminal_6"),
        boundary,
        stamped_start("sess-old", "zellij:terminal_9"),
    ]);
    assert_eq!(
        restamped[0]
            .pane
