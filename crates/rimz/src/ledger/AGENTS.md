...
- The feed store owns [`pending_terminal.rs`](./pending_terminal.rs); shared age pruning lives in [`atomic.rs`](./atomic.rs). The message queue owns one live `messages.jsonl` file, records terminal message text in the sibling `history.jsonl`, and records terminal outcomes in the event log.
