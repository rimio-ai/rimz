...
fn attribution_credits_exited_team_members_and_transcript_spend() {
    ...
    assert!(markdown.contains(
        r#"<a href="https://github.com/rimio-ai/rimz/blob/main/docs/guide/teams.md"><code>forge</code> team</a>"#
    ));
    assert!(markdown.contains(">agent-time</a>"));
    assert!(markdown.contains("human: 1 message, 1 <a href="));
    assert!(markdown.contains("- **planner** — Codex `gpt-5.5@high`"));
    assert!(markdown.contains("  - calls: 1 [ask]("));
    assert!(markdown.contains("  - messages: 1 from the author · 1 to teammates"));
    assert!(markdown.contains("  - tokens: "));
    assert!(markdown.contains("<sub>Generated by [`rimz agents attribution`]("));
}
